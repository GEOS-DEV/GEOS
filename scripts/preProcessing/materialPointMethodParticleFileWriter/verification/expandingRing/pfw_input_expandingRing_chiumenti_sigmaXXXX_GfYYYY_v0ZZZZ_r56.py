...
initialRadialVelocity = ZZZZ #0.01592
...
def make_objects():
    ...
    # poisson disk sampling is really slow for a giant empty finely resolved domain.
    # generate points in a disk instead.

    rmin = 0.5*innerDiameter - 0.25
    rmax = 0.5*outerDiameter + 0.25
    flawsize = 5*DZ
    A = np.pi*(rmax*rmax - rmin*rmin)
    npts = int(A/(0.25*np.pi*flawsize**2))
    vpts = []
    for n in range(npts):
        r = np.random.uniform(rmin,rmax)
        t = np.random.uniform(0.,2.*np.pi)
        vpts.append([r*np.cos(t),r*np.sin(t),0.])
