...
for i,file in enumerate(job_dirs):
  ...
  try:
    ...
    if readBoxSums:
      ...
      Bvm = np.sqrt(1.5*( ( Bsxx-Bsyy)**2 + (Bsyy-Bszz)**2 +(Bsxx-Bszz)**2 ))    # von Mises Stress
      ...
      axes[3].plot(Btime,Bvm,linestyle='-',color=colors[i],linewidth=3,alpha=0.5,label=filename+"_Bvm")
...
axes[3].set_ylabel(r'vm stress')
axes[3].tick_params(axis='both', which='major')
axes[3].tick_params(axis='both', which='minor')
axes[3].legend(bbox_to_anchor=(1.22,1), loc="upper left",fontsize=str(legend_font_size))
axes[3].grid()


fig.tight_layout()
fig.savefig("geoTXCcreep.png", bbox_inches="tight")
