...
sampleDiameter = 4 # target diameter
sampleLength = 4 # target length (cylinder) , mm
impactVelocity = 1.5 # mm/us

stopTime = sampleLength/impactVelocity 

aluminumDensity = 2.7
aluminumInitialYield = 0.030      # Johnson–Cook A parameter: initial yield strength
aluminumYoungsModulus = 70.0
aluminumPoissonsRatio = 0.3

# Additional Johnson–Cook parameters:
aluminumHardeningModulus = 0.020          # B parameter: hardening modulus
aluminumStrainHardeningExponent = 0.40    # n parameter: strain hardening exponent
aluminumStrainRateCoeff = 0.015           # C parameter: strain rate sensitivity coefficient
aluminumThermalSofteningExponent = 1.0      # m parameter: thermal softening exponent
aluminumMeltingTemperature = 933          # Melting temperature (Kelvin)
aluminumReferenceTemperature = 293        # Reference temperature (Kelvin)
...
cppx=12   # cells per partition in each direction
cppy=12
cppz=12
...
<JohnsonCook
    ...
    defaultInitialYieldStrength=""" + '"' + str(aluminumInitialYield) + '"' + """
    defaultHardeningModulus=""" + '"' + str(aluminumHardeningModulus) + '"' + """
    defaultStrainHardeningExponent=""" + '"' + str(aluminumStrainHardeningExponent) + '"' + """
    defaultStrainRateCoeff=""" + '"' + str(aluminumStrainRateCoeff) + '"' + """
    defaultThermalSofteningExponent=""" + '"' + str(aluminumThermalSofteningExponent) + '"' + """
    defaultMeltingTemperature=""" + '"' + str(aluminumMeltingTemperature) + '"' + """
    defaultReferenceTemperature=""" + '"' + str(aluminumReferenceTemperature) + '"' + """/>
