...
refStrainRate=1e-10
rateSensitivity=0.17 # gives best results
m2=0.0
...
pfw["xmin"] = (1-domainSize)*sampleLength  #0.0 # mm
pfw["xmax"] = domainX # mm
pfw["ymin"] = -(domainSize/2)*domainY # mm
pfw["ymax"] = (domainSize/2)*domainY # mm

#DX = (pfw["xmax"] - pfw["xmin"])/pfw["xpar"]/cppx

DX = (domainX)/pfw["xpar"]/cppx

#DX1=(pfw["xmax"] - pfw["xmin"])/(2*1)/cppx #initial 

#domainZ=(2/3)*(5*DX1)
pfw["zmin"] =-(domainSize/2)*domainZ # mm
pfw["zmax"] = (domainSize/2)*domainZ # mm
...
def make_objects():
    ...
    impactor = geom.sphere('impactor',
        ...
        x0=[  sampleLength + 0.5*impacterDiameter,0.0,0.0], ## removed pfw["xmin"] +
    ...
    target = geom.cylinder('target',
        ...
        x1=[ 0.0,0.0,0.0],   #changed  pfw["xmin"] to 0.0
        x2=[ 0.0+sampleLength,0.0,0.0],
    ...
    weibullTarget = geom.voronoiWeibullBoxWrapper('weibullWrapper',
        subObject=target,
        x0=[ 0.0, pfw["ymin"], pfw["zmin"] ], #changed  pfw["xmin"] to 0.0
...
pfw["absorbingDampingFactor"]=[500,0,500,500,0,0] 
