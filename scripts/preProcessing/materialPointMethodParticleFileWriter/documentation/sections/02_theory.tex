...
Moving and contact faces are enforced in two phases.  The solver first updates all constrained boundary-node values on all faces and accumulates reactions.  It then rebuilds the moving/contact buffer-node values for all faces using the now-final boundary-node data.  This ordering makes the mirror extension independent of whether the opposite face has already been visited in the face loop.  The distinction is normally negligible in well-resolved domains, but it is important for wide-stencil particle types in very thin non-periodic directions.  If a direction contains only one physical cell after the two non-periodic buffer cells are removed, the source node for the buffer extension on one face may be the boundary node on the opposite face; a one-pass boundary-plus-buffer loop can then give different results on the minus and plus faces simply because the faces are processed in the order $x^-,x^+,y^-,y^+,z^-,z^+$.
