...
\subsection{Reaction output and diagnostics}
\index{reactionHistory!PFW}

Boundary reactions are enabled from PFW with \texttt{reactionHistory} and scheduled with \texttt{reactionWriteInterval}.  The output is written by the solver to \path{reactionHistory.csv} and contains time, domain deformation components, current box lengths, six face reactions, and the current diagonal \texttt{domainL} values.
\begin{lstlisting}[language=Python,caption={Reaction-history controls.}]
pfw["reactionHistory"] = 1
pfw["reactionWriteInterval"] = 1.0e-6
# Optional alternative reaction measure:
pfw["useInteralForceAsFaceReaction"] = 1
\end{lstlisting}
The key name \texttt{useInteralForceAsFaceReaction} preserves the spelling used by the current solver attribute.
