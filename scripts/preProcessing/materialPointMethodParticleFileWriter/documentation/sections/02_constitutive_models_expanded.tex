...
\end{equation}
where \(\kappa\) is the equivalent plastic strain-like history variable and \(\lambda_{eff}=\max(\lambda_{max},1)\).  The maximum principal stretch is computed from the right stretch tensor obtained from \(F^TF\), so rigid-body rotation does not enter the hardening measure.
...
\end{equation}
where \(p_t\) is positive in hydrostatic tension.  The implemented return is non-associated: the trial mean stress is retained and the deviatoric stress is radially scaled to the pressure-adjusted scalar surface.
...
lambda_max = maxPrincipalStretchFrom(F^T F)
for fixed-point iteration:
  kappa = norm(plasticStrainOld + plasticStrainIncrement)
  sigma_f = Y(T,Xc) + softening(T,kappa) + H(T)*stretchMeasure(lambda_max)
  phi = q_trial + eta(T)*p_trial - sigma_f
  if phi <= 0 and first iteration: break elastic
  q_return = min(q_trial, max(sigma_f - eta(T)*p_trial, 0))
