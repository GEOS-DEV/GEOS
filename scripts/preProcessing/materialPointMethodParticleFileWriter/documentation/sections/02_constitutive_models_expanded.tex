...
\index{temperature!surface-informed polymer model}

\texttt{SurfaceInformedPolymer} is a reference-temperature, corotational polymer plasticity model for explicit MPM.  It preserves the elastic predictor and deviatoric radial-return structure used by the other small-strain/corotational solid models, but replaces parameter-specific logistic thermal factors with one normalized thermal scale and adds a weak pressure-sensitive strength correction.

The thermal scale is normalized at the supplied transition/reference temperature,
\begin{equation}
  S_T(T_g)=1,
\end{equation}
\begin{equation}
  \log S_T(T)=a_c\max(T_g-T,0)-a_h\max(T-T_g,0)
  -J\left[\frac{1}{1+\exp[-(T-T_g)/w_T]}-\frac{1}{2}\right].
  \label{eq:surface-informed-temperature-scale}
\end{equation}
Elastic moduli and yield strength may also depend on crystallinity,
\begin{equation}
  C_E=1+\beta_E(X_c-X_{c,g})A_X(T),\qquad
  C_Y=1+\beta_Y(X_c-X_{c,g})A_X(T),
\end{equation}
\begin{equation}
...
\begin{equation}
  K=K_gS_TC_E,\qquad G=G_gS_TC_E,\qquad Y=Y_gS_TC_Y,
\end{equation}
while the hardening slope follows
\begin{equation}
  H(T)=H_g S_T(T)^{p_H}.
\end{equation}
The scalar strength before pressure correction is
\begin{equation}
  \sigma_f^0 = Y(T,X_c) + S_{soft,g}S_T
  \exp\left[-\left(\frac{\kappa}{r_1}\right)^{r_2}\right]
  + H(T)\left(\lambda_{eff}^2-\frac{1}{\lambda_{eff}}\right),
\end{equation}
where \(\kappa\) is the equivalent plastic strain-like history variable and \(\lambda_{eff}\) is the larger of the tensile maximum principal stretch and the isochoric maximum stretch.  The stretch is computed from the right stretch tensor obtained from \(F^TF\), so rigid-body rotation does not enter the hardening measure.  Hydrostatic compression therefore does not activate chain hardening, while constrained compression and shear under pressure can still reach the maximum-stretch failure limit.

The pressure-sensitive yield function is
\begin{equation}
  \Phi=q+\eta(T)p_t-\sigma_f^0\le 0,\qquad
  \eta(T)=\eta_0\exp\left[-\frac{1}{2}\left(\frac{T-T_g}{w_\eta}\right)^2\right],
\end{equation}
where \(p_t\) is positive in hydrostatic tension and \(p_c\) is the optional compressive pressure-strengthening cap.  The implemented return is non-associated: the trial mean stress is retained and the deviatoric stress is radially scaled to the pressure-adjusted scalar surface.
...
scale K and G with S_T(T) and C_E(T,X_c)
stress_trial = elasticIsotropicUpdate(oldStress, strainIncrement)
(p_trial, q_trial, n_trial) = stressInvariants(stress_trial)
lambda_chain = max(maxPrincipalStretchFrom(F^T F), isochoricMaxPrincipalStretchFrom(F^T F))
p_eff = max(p_trial, -compressivePressureStrengtheningCap)
for fixed-point iteration:
  kappa = norm(plasticStrainOld + plasticStrainIncrement)
  sigma_f = Y(T,Xc) + softening(T,kappa) + H(T)*stretchMeasure(lambda_chain)
  phi = q_trial + eta(T)*p_eff - sigma_f
  if phi <= 0 and first iteration: break elastic
  q_return = min(q_trial, max(sigma_f - eta(T)*p_eff, 0))
  stress = recomposeStress(p_trial, q_return, n_trial)
  plasticStrainIncrement = strainIncrement - elasticStrainIncrement(stress-oldStress)
store stress, plastic strain, equivalent plastic strain, yield strength, damage
\end{lstlisting}
