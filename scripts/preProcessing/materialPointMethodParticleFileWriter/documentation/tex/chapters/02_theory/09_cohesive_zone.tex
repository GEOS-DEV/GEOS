...
For a cohesive interface, the key geometric objects for each interacting pair are the two material fields, denoted $A$ and $B$, a reference surface normal $\mathbf{N}^0$, a current surface normal $\mathbf{n}$, and a reference surface area $A^0$.  GEOS-MPM stores these data in \texttt{CohesiveZoneRegion} fields and in particle-level surface fields.  The region owns a list of unique physical cohesive nodes, active node/field slots, and binary field pairs.  Reference positions and partitioning normals are stored per physical node, while side-specific reference normals and areas are stored per active field slot and shared by every pair using that slot.  Cohesive particles store compact slot indices and reference shape-function weights that couple the particle to those cohesive nodes.
...
The implementation supports an arbitrary runtime number of velocity fields.  During initialization, particles with a cohesive surface flag and a matching \texttt{CZTag} are partitioned using the explicit surface normal, the particle contact group, and the particle's damage-field partitioning.  At every physical grid node, each occupied pair of fields becomes one binary cohesive constitutive point.  The topology is sparse: constitutive history is stored only for active field pairs, surface kinematics and reference geometry are stored once for each active $(\text{node},\text{field})$ slot, and reference position data are stored once per physical cohesive node.  Consequently, memory is independent of the total number of contact groups except for groups that are actually present together at cohesive nodes.
...
for each CohesiveZoneRegion R that is not initialized:
    ...
    split selected particle contributions onto their runtime velocity fields
    form every occupied binary field pair at each cohesive node
    collect sparse (node, field pair) keys across MPI ranks
    deduplicate physical nodes and (node, field) slots
...
The principal implementation limitations are: initialization depends on consistent particle surface flags and CZ tags; every simultaneously occupied field pair at a cohesive node is treated as a binary cohesive constitutive point; misorientation-dependent bicrystal strengths require a valid misorientation state; and once cohesive damage reaches completion the interface is handed off to contact rather than continuing to carry cohesive traction.  These limitations should be reflected in verification tests for new cohesive-zone use cases.
