...
Two additional guards can suppress separability.  If \texttt{treatFullyDamagedAsSingleField=1} and the grid damage-gradient magnitude is very small, the code treats the fields as inseparable to avoid arbitrary internal separation planes in fully damaged or undamaged material.  If \texttt{thinFeatureDFGThreshold} is set below its large default value, a DFG pair with very small field-center spacing relative to the neighbor radius is also forced to be inseparable.  The thin-feature guard uses
\begin{equation}
