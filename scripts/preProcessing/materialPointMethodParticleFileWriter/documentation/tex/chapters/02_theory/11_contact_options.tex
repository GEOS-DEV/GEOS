...
For simultaneous nodal contact, set
\texttt{contactSolver=ProjectedGaussSeidel}.  The optional
\texttt{contactPGSUseLogisticRegressionForMultifield=1} policy counts the
active contact fields at each node.  Nodes with more than two active fields
use a separate logistic-regression normal for each field pair, while one-pair
nodes retain the configured \texttt{contactNormalType}.  Enabling this policy
also requests the nodal particle-neighbor list required by the fit.  It is
disabled by default, and rigid-body contact retains its mapped-normal policy
because its color-based field indices are not logistic-regression classes.
...
The projected Gauss--Seidel path applies the same Coulomb law to the coupled
node solve.  After each normal-impulse update it removes the normal component
of the accumulated tangential impulse and projects the remainder onto
\begin{equation}
  \|\mathbf{J}_{t,AB}\| \le
  \mu_{AB}\max\!\left(0,J_{n,AB}-J_{n,AB}^{\mathrm{bias}}\right).
\end{equation}
All pair constraints at a node update the same working field velocities, so
frictional pairs that share a field are converged together rather than applied
as independent one-pass corrections.
...
\texttt{contactSolver} & Selects the legacy one-pass \texttt{Pairwise} update or the coupled \texttt{ProjectedGaussSeidel} node solve. \\
...
\texttt{contactPGSUseLogisticRegressionForMultifield} & When nonzero, uses pair-specific logistic-regression normals at PGS nodes with more than two active contact fields. \\
\texttt{contactPGSMaximumIterations}, \texttt{contactPGSVelocityTolerance} & Set the sweep limit and projected-velocity convergence tolerance for the node-local PGS solve. \\
\texttt{contactPGSRelaxation}, \texttt{contactPGSRequireConvergence} & Set the PGS relaxation factor and whether a nonconverged node is fatal. \\
