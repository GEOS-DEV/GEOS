...
\subsection{VP-Hencky domain scaling, reset, and subdivision controls}
\index{VP-Hencky!PFW controls}
\index{particle splitting!PFW controls}
\pfwidx{cpdiDomainScaling}\pfwidx{cpdiDomainScalingType}\pfwidx{domainResetType}\pfwidx{subdivideParticles}\pfwidx{resetDefGradForFullyDamagedParticles}\pfwidx{resetDefGradForMeltedParticles}\pfwidx{oversizedParticleTreatment}\pfwidx{oversizedParticleResetRankParticleCountThreshold}\pfwidx{defGradResetMaxParticleDomainToGridCellRatio}\pfwidx{plotUnscaledParticles}

The domain-scaling and reset algorithms are selected independently.  The compatibility defaults are \texttt{cpdiDomainScalingType="homel"} and \texttt{domainResetType="isotropicPolar"}.  Select \texttt{vpHencky} to preserve particle-domain measure whenever the neighbor-radius diagonal bound is geometrically feasible.  Section~\ref{subsec:cpdi-domain-scaling} gives the integration-domain algorithm, and Section~\ref{subsec:robust-defgrad-reset} gives the physical deformation-gradient reset.

\begin{lstlisting}[language=Python,caption={Linear CPDI with VP-Hencky integration-domain scaling.}]
pfw["cpdiDomainScaling"] = 1
pfw["cpdiDomainScalingType"] = "vpHencky"   # default is "homel"

# Omit this key, or use -1, for the conditional default.  It resolves to 1
# for vpHencky scaling of linear CPDI particles and to 0 otherwise.
pfw["subdivideParticles"] = -1

# Optional: write the physical F-based domain rather than the scaled
# integration domain in plot output.
pfw["plotUnscaledParticles"] = 1
\end{lstlisting}

The conditional subdivision default is deliberately tied to linear CPDI integration.  B-spline particles use point quadrature, so their stored domain vectors do not control B-spline support; deformation-gradient reset alone also leaves subdivision off by default.  Explicit values of 0 and 1 always disable or enable the option, respectively.

\begin{lstlisting}[language=Python,caption={VP-Hencky reset for melted or fully damaged material.}]
pfw["domainResetType"] = "vpHencky"        # default is "isotropicPolar"
pfw["resetDefGradForMeltedParticles"] = 1
pfw["resetDefGradForFullyDamagedParticles"] = 1

# Optional for reset-only or B-spline cases.  With the conditional default,
# splitting remains off; set 1 to split when fixed volume and the maximum
# diagonal cannot be satisfied simultaneously.
pfw["subdivideParticles"] = 0               # use 1 to opt in
\end{lstlisting}

Fluid subregions whose constitutive catalog is \texttt{Gas} or \texttt{Liquid} are automatically eligible for the selected reset type.  The reset keeps the reference particle-domain vectors fixed, regenerates the current domain from the new deformation gradient, and rebases surface, cohesive, and material-direction reference data consistently; it should still be used only where the constitutive formulation does not require the discarded finite deviatoric stretch.

\begin{lstlisting}[language=Python,caption={Split oversized particle domains with rank-count reset fallback.}]
pfw["defGradResetMaxParticleDomainToGridCellRatio"] = 1.5
pfw["oversizedParticleTreatment"] = "split"

# Optional count-control fallback.  Ranks at or below this local particle count
# keep splitting oversized domains; ranks above it use deformation-gradient reset.
pfw["oversizedParticleResetRankParticleCountThreshold"] = 2000000
\end{lstlisting}
...
\pfwidx{cpdiDomainScaling}\pfwidx{damageFieldPartitioning}\pfwidx{needsNeighborList}\pfwidx{separabilityMinDamage}\pfwidx{maxSingleFieldStateFractionForSeparability}\pfwidx{resetDefGradForFullyDamagedParticles}\pfwidx{disableSurfaceNormalsAndPositionsOnCPDIScaling}\pfwidx{plotUnscaledParticles}\pfwidx{particleFileFields}
...
Initial surface flags are supplied explicitly through geometry painting or the \texttt{SurfaceFlag} particle-file field.  DFG separability uses the same mapping normal used for field partitioning: an explicit particle surface normal when available, otherwise the particle damage gradient.
...
The \texttt{surfaceQualityThreshold} value is compared with the mapping-normal tensor quality using a \(\ge\) check.  A value of 0 disables this quality restriction, while a value of 1 requires the field pair to be fully covered by one coherent normal axis.  The \texttt{thinFeatureDFGThreshold} guard suppresses separation when \(\|\mathbf{x}_A-\mathbf{x}_B\|^2 < (\texttt{thinFeatureDFGThreshold}\,r_n)^2\), where \(r_n\) is the neighbor radius.
...
CPDI and DFG damage & \texttt{cpdiDomainScaling}, \texttt{damageFieldPartitioning}, \texttt{needsNeighborList}, \texttt{separabilityMinDamage}, \texttt{maxSingleFieldStateFractionForSeparability} & CPDI/FLIP damage example\\
Painted surface flags & \texttt{particleFileFields}, \texttt{surfaceQualityThreshold}, \texttt{thinFeatureDFGThreshold} & Notched-bar example with mapping-normal quality and thin-feature guards\\
