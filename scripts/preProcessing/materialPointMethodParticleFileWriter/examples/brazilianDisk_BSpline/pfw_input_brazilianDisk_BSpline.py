...
# Purpose: Brazilian disk compression using the single-point B-spline particle type.
...
"""
Brazilian disk SinglePointBSpline example for GEOS MPM.
...
# cpdiDomainScaling is relevant for CPDI particle-domain scaling. This
# SinglePointBSpline variant uses particle centers and the cubic B-spline
# interpolation stencil instead, so the CPDI scaling flag is disabled.
...
# This example keeps the FMPM grid-to-particle update but changes the
# particle mapping type to SinglePointBSpline below. In GEOS/PFW,
# is the CPDI default used by the companion Brazilian disk examples.
...
# boundary/surface particles. RVector is retained for particle-domain output;
# SinglePointBSpline interpolation itself uses the particle center and B-spline stencil.
...
disk1 = geom.cylinder(
    ...
    mat = 0,
    group = 0,
    particleType = 1,  # SinglePointBSpline; PFW writes this to GEOS ParticleMesh.
)
