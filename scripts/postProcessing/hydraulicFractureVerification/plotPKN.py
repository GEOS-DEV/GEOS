...
t_sim, press0 = np.loadtxt(prefix + '_pressure0.curve',
                           skiprows=1,
                           unpack=True)
...
for i in range(0, len(soln_time)):
    ...
    soln_Length[i] = 0.39 * pow(Eprime * pow(q, 3) / (mu * pow(h, 4)),
                                0.2) * pow(soln_time[i], 0.8)
    #soln_press0[i] = soln_aper0[i] * Eprime / ( 2 * h )
    #soln_press0[i] = 2.5 * pow( pow(G,4)*mu*q*q/(4*pow(1-nu,4)*pow(h,6))*soln_time[i],0.2 )
    soln_press0[i] = 2 / h * pow(mu * q * pow(Eprime, 3) * soln_Length[i] / PI,
                                 0.25)
