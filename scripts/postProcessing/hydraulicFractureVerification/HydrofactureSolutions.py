...
class HydrofractureSolutions:
    ...
    def Solutions( self, mu, E, q, KI, time, time_radplots, distanceFromInjection ):
        ...
        return [ rho, w_tough, p_tough, w_viscous, p_viscous, PvsTime_Tdom, RvsTime_Tdom, w0vsTime_Tdom, P_Vdom, R_Vdom, w0_Vdom ]
...
class KGDSolutions(HydrofractureSolutions):
    ...
    def PI_m0(self, rho):
        ...
        A2 = 0.39650
        B = 0.06858
        PIStar1 = 1.0 / (3.0 * math.pi) * scipy.special.beta(0.5, 2.0/3.0) * scipy.special.hyp2f1(-1.0/6.0, 1, 0.5, rho*rho)
        PIStar2 = 3.0 / (20.0 * math.pi) * scipy.special.beta(-1.5, 8.0/3.0) *( 2.0/3.0 * rho*rho * scipy.special.hyp2f1(-1.0/6.0, 2, 1.5, rho*rho) - 0.5* scipy.special.hyp2f1(-7.0/6.0, 1, 0.5, rho*rho) )
...
class PennySolutions(HydrofractureSolutions):
    ...
    def Omega_m0_bar(self, rho ):
        ...
        C1 = 6.846e-1
        C2 = 7.098e-2
        ot1 = ( math.sqrt(70.0)/3.0*C1 + 4.0*math.sqrt(5.0)/9.0 * C2 * ( 13*rho-6 ) ) * pow( 1.0 - rho, 2.0/3.0 )
