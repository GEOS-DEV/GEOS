...
if [[ "${RUN_INTEGRATED_TESTS}" = true ]]; then
  ...
  ATS_CMAKE_ARGS="-DATS_ARGUMENTS=\"--machine openmpi --ats f \"np<=2\" --ats openmpi_mpirun=/usr/bin/mpirun --ats openmpi_args=--allow-run-as-root --ats openmpi_procspernode=4 --ats openmpi_maxprocs=4\" -DPython3_ROOT_DIR=${ATS_PYTHON_HOME}"
fi
