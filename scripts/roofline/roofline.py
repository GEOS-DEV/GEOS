...
def roofline(LABELS, FLOPS, AIL1, AIL2, AIHBM):
  ...
  for i in range(len(AIL1)):
    ax.plot(float(AIL1[i]),float(FLOPS[i]),c=colors[i],marker=styles[0],linestyle='None',ms=markersize,markerfacecolor='none',markeredgewidth=markerwidth,label=LABELS[i])
