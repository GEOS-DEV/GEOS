...
def collect_files(root_folder,ext):
    ...
    and returns all .extension files in any subfolders
    Returns: a Pandas DataFrame with casenames, runtimes, success_flag
    '''
    abs_input_file_list=[]
...
def parse_and_search(allfiles, todo_token, n_lines_before=5, n_lines_after=5):
    ...
    with open(log_output, 'w') as f:
        ...
        summary = 'Found {} files containing {} (case insensitive) '.format(cnt, todo_token.upper())
...
def main(argv):
    ...
    parse_and_search(all_files,token,3, 3)
