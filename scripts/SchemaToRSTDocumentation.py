...
def buildAttributeMap(root_node, xsd='{http://www.w3.org/2001/XMLSchema}'):
    ...
    for child_node in root_node.findall(xsd + 'complexType'):
        ...
        for attribute_node in child_node.findall(xsd + 'attribute'):
            ...
            att_type = format_type_data(attribute_node.get('type',
                                                           default=' '))
...
def buildTableValues(type_map, link_string='XML', include_defaults=True):
    table_headers = [
        'Name', 'Type', 'Default', 'Registered On', 'Registered By',
        'Description'
    ]
    ...
    table_values = [[' ' for jj in range(0, N_cols)]
        ...
        for ii in range(0, N_rows):
            ...
            for jj in range(1, len(table_headers)):
                ...
                if k in type_map[att_name]:
                    ...
                    if ('Registered' in k):
                        table_row[jj] = ", ".join([
                            ':ref:`%s_%s`' % (link_string, x)
                            for x in table_row[jj]
                        ])
...
complete_output = os.path.join(schema_dir,
...
input_repeated_keys = [
    input_keys[ii] for ii in range(0, len(input_keys))
    if input_keys_count[ii] > 1
]
...
other_repeated_keys = [
    other_keys[ii] for ii in range(0, len(other_keys))
    if other_keys_count[ii] > 1
]
...
with open('%s.rst' % (complete_output), 'w') as output_handle:
    ...
    for type_name in sorted(other_attribute_map.keys()):
        ...
        writeTableRST('%s/%s_other.rst' % (output_folder, type_name),
                      table_values)
...
if len(untouched_files):
    ...
    raise ValueError('Obsolete autogenerated .rst table files detected:%s' %
                     (tmp))
