...
double VerySmoothBump_tt(double freq, double t, double* par, int npar, int* ipar, int nipar )
{
  ...
  if (t*freq < 0)
    ...
    tmp = freq*freq*
          ( -1024*90*pow(t*freq,8) + 5120*72*pow(t*freq,7) - 10240*56*pow(t*freq,6) + 10240*42*pow(t*freq,5) - 5120*30*pow(t*freq,4) + 1024*20*pow(t*freq,3) );
  return tmp;
}
...
double VerySmoothBump_ttt(double freq, double t, double* par, int npar, int* ipar, int nipar )
{
  ...
  if (t*freq < 0)
    ...
    tmp = freq*freq*freq*
          ( -1024*90*8*
            pow(t*freq,7) + 5120*72*7*pow(t*freq,6) - 10240*56*6*pow(t*freq,5) + 10240*42*5*pow(t*freq,4) - 5120*30*4*pow(t*freq,3) + 1024*20*3*pow(t*freq,2) );
  return tmp;
}
...
double Triangle_om(double freq, double t, double* par, int npar, int* ipar, int nipar )
{
  ...
  if (t*freq < 0)
    ...
    tmp = 2*8./
          pow(M_PI,
              2)*
          (M_PI*freq*t*(cos(M_PI*(t*freq)) - cos(3*M_PI*(t*freq))/3 + cos(5*M_PI*(t*freq))/5 - cos(7*M_PI*(t*freq))/7) +
           (sin(M_PI*(t*freq)) - sin(3*M_PI*(t*freq))/9 + sin(5*M_PI*(t*freq))/25 - sin(7*M_PI*(t*freq))/49) );
...
//std::cout <<  "t= " << t << " npts " << npts << " k= " << k << "arg = " << arg
// <<  std::endl;
...
double C6SmoothBump_tttt(double freq, double t, double* par, int npar, int* ipar, int nipar )
{
  ...
  if (t*freq < 0)
    ...
    tmp = 51480*freq*freq*freq*freq*
          (12*42*(pow(t*freq*(1-t*freq),5)-5*(1-2*freq*t)*(1-2*freq*t)*pow(t*freq*(1-t*freq),4))+840*pow((1-2*t*freq)*t*freq*(1-t*freq),4) );
  return tmp;
}
