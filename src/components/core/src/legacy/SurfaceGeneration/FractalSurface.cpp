                                     ...
                                     Array2dT<SurfaceKernel*>& curr, localIndex& icurr)
{
  ...
  for(int j = 0 ; j < nj ; j++)
  {
    ...
    for(int k = 0 ; k < nk ; k++)
    {
      ...
      //std::cout << "kernel " << curr(j,k)->m_x[0] << " " << curr(j,k)->m_x[1]
      // << " " << ilevel << " " << curr(j,k)->Value() << std::endl;
    }
  }
                                     ...
                                     const localIndex, Array2dT<SurfaceKernel*>& curr)
{
  ...
  for (int j = 0 ; j < nj ; j++)
  {
    for (int k = 0 ; k < nk ; k++)
    {
      ...
      //std::cout << "kernel " << curr(j,k)->m_x[0] << " " << curr(j,k)->m_x[1]
      // << " " << ilevel << " " << curr(j,k)->IncrementSum(0) << std::endl;
    }
  }
}
                                ...
                                array< Array2dT<SurfaceKernel*> >& vals)
{
  ...
  for (int j = 0 ; j < nj ; j++) //for each cell at the finest level - j
  {
    ...
    {
      array<array<SurfaceKernel*> >& currentVal = m_values(j, k); // get the
                                                                  // list of
                                                                  // kernels for
                                                                  // each level
      int nper = 1;
