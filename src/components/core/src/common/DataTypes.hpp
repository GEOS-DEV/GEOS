...
//    }
    ...
    inline static void f( arrayView1d<R2SymTensor>& lhs, integer const component, real64 const & rhs )
    {
      for( localIndex a=0 ; a<lhs.size() ; ++a )
      {
        lhs[a].Data()[component] = rhs;
      }
    }

    template< typename T1, typename T2 >
    inline static void f( arrayView1d<T1> & lhs, integer const, T2 const & rhs )
    {
//      lhs = rhs;
    }
