...
static constexpr char const * PyGroup_getSolverDocString =
  "get_solver(self, path, default, /)\n"
  "--\n\n"
  "Return the `Solver` at the relative path ``path``; ``default`` is optional.\n\n"
  "If no wrapper exists and ``default`` is not given, raise a ``ValueError``;\n"
  "otherwise return ``default``.\n"
  "\n"
  "Parameters\n"
  "__________\n"
  "path : str\n"
  "    The relative path of the solver to return.\n"
  "\n"
  "Returns\n"
  "_______\n"
  "Solver\n"
...
static PyObject * PyGroup_getSolver( PyGroup * const self, PyObject * const args ) noexcept
{
  ...
  try
  {
    geosx::EventManager & eventManager = geosx::g_state->getProblemManager().getEventManager();
    ...
    {
      return createNewPyWaveSolver( subEvent );
    }
    else
    {
      return createNewPySolver( subEvent );
    }
  }
  // If the path isn't valid then either getGroupByPath or getWrapperBase will throw a std::domain_error
  catch( std::domain_error const & e )
