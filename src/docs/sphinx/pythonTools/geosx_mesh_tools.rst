...
The `geosx_mesh_tools` python package includes tools for converting meshes from common formats (abaqus, etc.) to those that can be read by GEOSX (gmsh, vtk).
See :ref:`PythonToolsSetup` for details on setup instructions, and :ref:`ExternalMeshUsage` for a detailed description of how to use external meshes in GEOSX.
...
.. note::
    For vtk format meshes, the user also needs to determine the region ID numbers and names of nodesets to import into GEOSX.
