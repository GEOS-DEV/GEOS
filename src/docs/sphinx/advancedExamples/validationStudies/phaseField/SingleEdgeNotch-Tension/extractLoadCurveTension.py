#!/usr/bin/env python3
"""Reduce the GEOS TimeHistory output of the single-edge notched tension benchmark to a
light load curve.
...
directory once the benchmark has been run and its history file copied here:
    ...
    python3 extractLoadCurveTension.py
...
HISTORY_FILE = "singleEdgeNotchTension_topBoundary.hdf5"
OUTPUT_FILE = "loadCurve.csv"

# Candidate locations, in order: the output directory of the run (geosx -o Tension),
# then the directory of the deck itself when the run was launched without -o.
HISTORY_CANDIDATES = [os.path.join(DECK_DIR, "Tension", HISTORY_FILE),
...
# tension test is driven by index 1 (sigma_yy), and the imposed displacement is its
# component 1.
STRESS_COMPONENT = 1
...
def main():
    ...
    header = ("Load curve of the single-edge notched benchmark under tension\n"
              "Extracted from {} by extractLoadCurveTension.py\n"
