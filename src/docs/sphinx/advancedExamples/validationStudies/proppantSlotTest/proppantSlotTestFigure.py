...
def main():
    eTime, eLength, eArea = np.loadtxt("experiment-data.txt",
                                       skiprows=0,
                                       unpack=True)
    esTime, esArea = np.loadtxt("experiment-data2.txt",
                                skiprows=0,
                                unpack=True)

    dTime, dLength, dArea = np.loadtxt("model-results.txt",
                                       skiprows=1,
                                       unpack=True)
    ...
    ax[0, 0].plot(dTime, (dLength - offSize) / totalLength,
                  ...
                  linestyle='-')
    ax[0, 0].set_ylabel('Normalized Bank Length',
    ...
    ax[0, 1].plot(dTime,
                  ...
                  label='GEOSX-30/50 mesh',
                  linestyle='-')
    ax[0, 1].set_ylabel('Normalized Bank Area',
                        multialignment='center',
                        weight="bold")
    ...
    ax[1, 0].plot(sTime,
                  ...
                  label='GEOSX-30/50 mesh',
                  linestyle='-')
    ax[1, 0].set_ylabel('Normalized Suspended Proppant Area',
                        multialignment='center',
                        weight="bold")
    ...
    ax[1, 1].plot(dTime, (dArea + sArea) / totalCellNum,
                  ...
                  label='GEOSX-30/50 mesh',
                  linestyle='-')
    ax[1, 1].set_ylabel('Normalized Total Propped Area',
                        multialignment='center',
                        weight="bold")
