...
class Analytical:
    ...
    def computeRadialStress(self, x):
        return (self.Stress[0] + self.Stress[1]) / 2. * (
            1. - (self.rw / x)**2) + (self.Stress[0] - self.Stress[1]) / 2. * (
                1. - 4. * (self.rw / x)**2 + 3. * (self.rw / x)**4) * cos(
    ...
    def computeShearDisp(self, x):
        return (self.rw)**2 / x / 2. / self.G * (
            self.Stress[0] - self.Stress[1]) / 2. * (2. * (1. - 2. * self.nu) +
                                                     (self.rw / x)**2) * sin(
                                                         2. * self.theta)
...
def getCompressiveStressFromXML(xmlFilePath):
    ...
    found_stress = False
    for elem in param:
        if elem.get("name") == "WellLoad" and elem.get(
                "tractionType") == "normal":
...
def main():
    ...
    for i in range(0, len(zcord_elm)):
        if abs(zcord_elm[i] / 0.5 - 1.) < 0.01 and abs(
                xcord_elm[i] * tan(theta) / ycord_elm[i] - 1.) < 0.01:
    ...
    for i in range(0, len(zcord_node)):
        if abs(zcord_node[i] / 1.0 -
               1.) < 0.01 and abs(xcord_node[i] * tan(theta) /
    ...
    AnalyticalSolution = Analytical(mechanicalParameters, rw, Stress, Pw,
                                    theta)
    ...
    for xCell in x_analytical:
        srr_analytical[i] = AnalyticalSolution.computeRadialStress(
            xCell) / 1.0e6
        s00_analytical[i] = AnalyticalSolution.computeHoopStress(xCell) / 1.0e6
        sr0_analytical[i] = AnalyticalSolution.computeShearStress(
            xCell) / 1.0e6
    ...
    ax[1].semilogx(x_analytical,
                   ur_analytical,
                   lw=lw,
                   alpha=0.5,
                   color=cmap(0),
                   label=r'u$_{r}$ - Analytical')
    ax[1].semilogx(rlist_node,
                   u_r,
                   'o',
                   color=cmap(0),
                   markersize=msize,
