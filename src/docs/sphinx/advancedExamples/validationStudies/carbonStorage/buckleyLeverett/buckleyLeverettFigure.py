...
def computeFractionalFlow( phaseVolFrac, \
    ...
    for i in range( 0, 2 ):
        ...
        dPhaseMob_dS[i] = phaseRelPermExp[i]*phaseRelPermMaxValue[i] / phaseVisc[i] * (scaledPhaseVolFrac**(phaseRelPermExp[i]-1)) / scale

    fractionalFlow = phaseMob[0] / (phaseMob[0]+phaseMob[1])    
    dFractionalFlow_dS = (dPhaseMob_dS[0]*phaseMob[1] + phaseMob[0]*dPhaseMob_dS[1]) / ((phaseMob[0] + phaseMob[1])**2)
