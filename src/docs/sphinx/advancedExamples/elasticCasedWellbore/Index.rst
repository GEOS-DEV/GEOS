...
Analytical results of the radial and hoop stresses, :math:`\sigma_{rr}` and :math:`\sigma_{\theta\theta}`, in casing, cement sheath and rock are expressed as:
...
Far-field boundary are fixed in displacements. The normal traction on the inner face of the casing is defined by ``Traction`` field specification. The nodeset generated by the internal wellbore generator for the inner face is ``rneg``. The traction type for defining a casing test pressure that is normal to the casing inner face is ``normal``. The negative sign of the scale value is because of the negative sign convention for compressional stress.
