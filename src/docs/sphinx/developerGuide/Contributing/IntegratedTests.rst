...
A family of integrated tests with the ``_mgr`` name token (e.g. ``PoroElastic_Terzaghi_mgr_smoke``)
...
* **Baselines for** ``_mgr`` **tests are always generated by builds without hypredrive**
...
* The restartcheck ``atol``/``rtol`` of each ``_mgr`` test is the equivalence criterion.
...
* Each ``_mgr`` deck sets ``logLevel="1"`` on its ``LinearSolverParameters`` so the log carries
...
* Restart checks compare solution fields only, so a solver-quality regression (more
  iterations, same answer) passes them silently. ``scripts/compareLinearSolverIterations.py``
  closes this gap: ``harvest`` the ATS working directory after each pass and ``compare``
  the two JSON files — it fails when total iterations differ by more than 10%, the
