...
- It is also possible to develop directly in the cloud using `GitHub codespaces <https://docs.github.com/en/codespaces>`_.
  ...
  To make it work, select the version of the TPL you need by replacing the `DEFINE_ME` token in this `Dockerfile <https://github.com/GEOSX/GEOS/blob/develop/.devcontainer/Dockerfile>`_ before creating your ``codespace`` instance.
