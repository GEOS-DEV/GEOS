    ...
    if model.get("material") == "DruckerPrager":
       ...
       yieldParameters["q_iniYield"] = q_iniYield
         ...
         for i in range(0,len(p_CSL)):
             if alpha**2*p_CSL[i]*(2.0*alpha*pc0/(alpha+1.0)-p_CSL[i])-alpha**2*(alpha-1.0)/(alpha+1.0)*pc0**2<0.0:
                qlist2[i]=0.0
             else:
                qlist2[i]=M*pow(alpha**2*p_CSL[i]*(2.0*alpha*pc0/(alpha+1.0)-p_CSL[i])-alpha**2*(alpha-1.0)/(alpha+1.0)*pc0**2,0.5)
         ...
         qlist_DE = M*pow(alpha**2*plist_DE*(2.0*alpha*pc0/(alpha+1.0)-plist_DE)-alpha**2*(alpha-1.0)/(alpha+1.0)*pc0**2,0.5)
...
def getMechanicalParametersFromXML( xmlFilePath ):
    ...
    if model.get("material") == "DruckerPrager":
       ...
       mechanicalParameters["frictionAngle"] = float(param.get("defaultFrictionAngle"))
         ...
         mechanicalParameters = dict.fromkeys(["bulkModulus", "shearModulus", "cohesion", "initialFrictionAngle", "residualFrictionAngle"])
         mechanicalParameters["bulkModulus"] = float(param.get("defaultBulkModulus"))
         mechanicalParameters["shearModulus"] = float(param.get("defaultShearModulus"))
         mechanicalParameters["cohesion"] = float(param.get("defaultCohesion"))
         ...
         mechanicalParameters = dict.fromkeys(["bulkModulus", "shearModulus", "preConsolidationPressure", "shapeParameter", "cslSlope"])
         mechanicalParameters["bulkModulus"] = float(param.get("defaultBulkModulus"))
         mechanicalParameters["shearModulus"] = float(param.get("defaultShearModulus"))
         ...
         mechanicalParameters = dict.fromkeys(["shearModulus", "preConsolidationPressure", "cslSlope", "recompressionIndex"])
         mechanicalParameters["shearModulus"] = float(param.get("defaultShearModulus"))
         mechanicalParameters["preConsolidationPressure"] = float(param.get("defaultPreConsolidationPressure"))
         mechanicalParameters["cslSlope"] = float(param.get("defaultCslSlope"))
...
def main():
    ...
    ax[0].plot(-ax_strain*100, q_num, 'o', color=cmap(0), mec = 'b', markersize=msize, alpha=malpha, label='Triaxial Driver')
    ...
    ax[2].plot(-ax_strain*100, -strain_vol*100, 'o', color=cmap(0), mec = 'b', markersize=msize, alpha=malpha, label='Triaxial Driver')
