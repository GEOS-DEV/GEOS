...
Since we have imported a mesh with only one region, we can set the attribute ``cellBlocks`` to ``{ all }``
(we could set it to ``{ hexahedra }`` as the mesh has only hexahedral cells).
