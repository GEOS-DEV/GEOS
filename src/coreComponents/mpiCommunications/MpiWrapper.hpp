...
//  static int Bcast( void * buffer, int count, MPI_Datatype datatype, int root, MPI_Comm comm );
  ...
  static std::pair< U, U > PrefixSum( T const & value, MPI_Comm comm = MPI_COMM_GEOSX );
...
template< typename U, typename T >
std::pair< U, U > MpiWrapper::PrefixSum( T const & value, MPI_Comm comm )
{
  array1d< T > gather;
  ...
  int const mpiSize = Comm_size( comm );
  int const mpiRank = Comm_rank( comm );
