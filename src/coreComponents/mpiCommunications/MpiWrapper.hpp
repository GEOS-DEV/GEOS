...
public:
  ...
  static int Cart_create(MPI_Comm comm_old,
                         int ndims,
                         const int dims[],
                         const int periods[],
                         int reorder,
  ...
  static int Waitany(int count,
                     MPI_Request array_of_requests[],
                     int *indx,
                     MPI_Status *status);

  static int Waitsome(int count,
                      MPI_Request array_of_requests[],
                      int *outcount,
                      int array_of_indices[],
                      MPI_Status array_of_statuses[]);

  static int Waitall(int count,
                     MPI_Request array_of_requests[],
                     MPI_Status array_of_statuses[]);
   ...
   */
  static int ActiveWaitSomePartialPhase(
   ...
   * @return MPI_SUCCESS or and MPI_ERROR from internal calls to MPI_WaitAny.
   */
  static int ActiveWaitSomeCompletePhase(
    const int participants,
    std::vector<std::function<MPI_Request(int)>> const &phases);
   ...
   * @return MPI_SUCCESS or and MPI_ERROR from internal calls to MPI_WaitAny.
   */
  static int ActiveWaitOrderedCompletePhase(
    const int participants,
    std::vector<std::function<MPI_Request(int)>> const &phases);
  ///@}
   ...
   */
  template <typename T>
  static void allGather(T const myValue,
                        array1d<T> &allValues,
   ...
   */
  template <typename T>
  static int allReduce(T const *sendbuf,
                       T *recvbuf,
                       int count,
                       MPI_Op op,
                       MPI_Comm comm);
   ...
   */
  template <typename T> static MPI_Datatype getMpiType();
   ...
   */
  template <typename U, typename T> static U PrefixSum(T const value);
...
template <> inline MPI_Datatype MpiWrapper::getMpiType<float>()
{
  return MPI_FLOAT;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<double>()
{
  return MPI_DOUBLE;
}

template <> inline MPI_Datatype MpiWrapper::getMpiType<char>()
{
  return MPI_CHAR;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<signed char>()
{
  return MPI_SIGNED_CHAR;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<unsigned char>()
{
  return MPI_UNSIGNED_CHAR;
}

template <> inline MPI_Datatype MpiWrapper::getMpiType<int>()
{
  return MPI_INT;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<long int>()
{
  return MPI_LONG;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<long long int>()
{
  return MPI_LONG_LONG;
}

template <> inline MPI_Datatype MpiWrapper::getMpiType<unsigned int>()
{
  return MPI_UNSIGNED;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<unsigned long int>()
{
  return MPI_UNSIGNED_LONG;
}
template <> inline MPI_Datatype MpiWrapper::getMpiType<unsigned long long int>()
{
  return MPI_UNSIGNED_LONG_LONG;
}

template <> inline MPI_Datatype MpiWrapper::getMpiType<bool>()
{
  return MPI_CXX_BOOL;
}
...
template <typename T>
void MpiWrapper::allGather(T const myValue,
                           array1d<T> &allValues,
                           MPI_Comm MPI_PARAM(comm))
{
#ifdef GEOSX_USE_MPI
...
template <typename T>
void MpiWrapper::Broadcast(T &MPI_PARAM(value),
...
template <>
inline void MpiWrapper::Broadcast<std::string>(std::string &MPI_PARAM(value),
                                               int MPI_PARAM(srcRank),
                                               MPI_Comm MPI_PARAM(comm))
{
#ifdef GEOSX_USE_MPI
...
template <typename U, typename T> U MpiWrapper::PrefixSum(T const value)
{
  U localResult;
...
template <typename T> T MpiWrapper::Sum(T const &value, MPI_Comm comm)
{
  return MpiWrapper::Reduce(value, Reduction::Sum, comm);
}

template <typename T> T MpiWrapper::Min(T const &value, MPI_Comm comm)
{
  return MpiWrapper::Reduce(value, Reduction::Min, comm);
}

template <typename T> T MpiWrapper::Max(T const &value, MPI_Comm comm)
{
