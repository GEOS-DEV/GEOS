...
void NeighborCommunicator::MPI_iSendReceive(int const commID, MPI_Comm mpiComm)
{
...
inline int GhostSize(NodeManager& nodeManager,
...
void NeighborCommunicator::PrepareAndSendGhosts(
...
void NeighborCommunicator::UnpackGhosts(MeshLevel& mesh, int const commID)
{
  GEOSX_MARK_FUNCTION;
...
int NeighborCommunicator::PackCommSizeForSync(
...
void NeighborCommunicator::PackCommBufferForSync(
  std::map<string, string_array> const& fieldNames,
  MeshLevel const& mesh,
  int const commID,
  bool on_device)
