                                     ...
                                     array1d< NeighborCommunicator > & neighbors )
{
  ...
  GEOSX_MARK_BEGIN( "GHOST_LOOP" );
  {
    ...
    auto send = [&] ( int idx )
      {
        // GEOSX_LOG_RANK("ghost_send_0 for" << neighbors[idx].NeighborRank());
        neighbors[idx].PrepareAndSendGhosts( false, 1, meshLevel, commID );
        // GEOSX_LOG_RANK("ghost_send_1 for" << neighbors[idx].NeighborRank());
        return neighbors[idx].GetSizeRecvRequest( commID );
      };
    auto post_recv = [&] ( int idx )
      {
        // GEOSX_LOG_RANK("post_recv_0 for" << neighbors[idx].NeighborRank());
        neighbors[idx].PostRecv( commID );
        // GEOSX_LOG_RANK("post_recv_1 for" << neighbors[idx].NeighborRank());
        return neighbors[idx].GetRecvRequest( commID );
      };
    auto proc_recv = [&] ( int idx )
      {
        // GEOSX_LOG_RANK("ghost_unpack_0 for" << neighbors[idx].NeighborRank());
        neighbors[idx].UnpackGhosts( meshLevel, commID );
        // GEOSX_LOG_RANK("ghost_unpack_1 for" << neighbors[idx].NeighborRank());
  ...
  //   shifted from the waitall in unpackandrebuildsync lists since the processes are more 'in-sync' when
