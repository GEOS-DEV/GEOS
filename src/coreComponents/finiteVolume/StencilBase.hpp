...
public:
   ...
   */
  virtual void
  move( LvArray::MemorySpace const space );
   ...
   */
  virtual bool
  zero( localIndex const connectorIndex );
   ...
   */
  typename LEAFCLASSTRAITS::IndexContainerViewConstType const &
  getElementRegionIndices() const
  {
    return m_elementRegionIndices.toViewConst();
  }
   ...
   */
  typename LEAFCLASSTRAITS::IndexContainerViewConstType const &
  getElementSubRegionIndices() const
  {
    return m_elementSubRegionIndices.toViewConst();
  }
   ...
   */
  typename LEAFCLASSTRAITS::IndexContainerViewConstType const &
  getElementIndices() const
  {
    return m_elementIndices.toViewConst();
  }
   ...
   * @return A view to const
   */
  typename LEAFCLASSTRAITS::WeightContainerViewConstType const &
  getWeights() const
  {
...
template< typename LEAFCLASSTRAITS, typename LEAFCLASS >
void
StencilBase< LEAFCLASSTRAITS, LEAFCLASS >::reserve( localIndex const size )
{
...
template< typename LEAFCLASSTRAITS, typename LEAFCLASS >
bool
StencilBase< LEAFCLASSTRAITS, LEAFCLASS >::zero( localIndex const connectorIndex )
{
  return executeOnMapValue(
    m_connectorIndices,
    connectorIndex,
    [&]( localIndex const connectionListIndex ) {
      for( localIndex i = 0;
           i < static_cast< LEAFCLASS * >( this )->stencilSize( connectorIndex );
           ++i )
      {
...
template< typename LEAFCLASSTRAITS, typename LEAFCLASS >
void
StencilBase< LEAFCLASSTRAITS, LEAFCLASS >::setName( string const & name )
{
...
template< typename LEAFCLASSTRAITS, typename LEAFCLASS >
void
StencilBase< LEAFCLASSTRAITS, LEAFCLASS >::move( LvArray::MemorySpace const space )
{
