...
/**
 * @file MatrixFreePreconditionerTwoLevel.cpp
 */

#include "MatrixFreePreconditionerTwoLevel.hpp"
...
MatrixFreePreconditionerTwoLevel< LAI >::
MatrixFreePreconditionerTwoLevel ( LinearOperator< Vector > const & operatorFine,
  ...
  : Base(),
    m_fineOperator( operatorFine ),
    m_coarseMat( matCoarse ),
    m_diagInv( diagonalInverse )
{
  ...
  if( params.solverType == LinearSolverParameters::SolverType::direct )
  {
    ...
    GEOSX_LOG_RANK("Direct coarse solver: done!");
  }
  else
  {
  ...
  fineTmp.create( m_diagInv.localSize(), m_diagInv.comm() );
  fineTmp2.create( m_diagInv.localSize(), m_diagInv.comm() );
  fineRhs.create( m_diagInv.localSize(), m_diagInv.comm() );
  coarseTmp.create( m_coarseMat.numLocalCols(), m_coarseMat.comm() );
  coarseRhs.create( m_coarseMat.numLocalCols(), m_coarseMat.comm() );
}

template< typename LAI >
MatrixFreePreconditionerTwoLevel< LAI >::~MatrixFreePreconditionerTwoLevel() = default;

template< typename LAI >
void MatrixFreePreconditionerTwoLevel< LAI >::apply( Vector const & src,
                                           Vector & dst ) const
{
  ...
  //GEOSX_LOG_RANK("\n\n\nWriting first residual\n\n\n");
  //fineRhs.write("fineRhs.mtx");
  // GEOSX_LOG_RANK("--- presmoothing");
  m_diagInv.pointwiseProduct( fineRhs, fineTmp );
  //GEOSX_LOG_RANK("\n\n\nWriting smoothed residual\n\n\n");
  //fineTmp.write("fineTmp.mtx");
  dst.axpy( 1.0, fineTmp );
  //GEOSX_LOG_RANK("\n\n\nAccumulating smoothed residual\n\n\n");
  //dst.write("dst.mtx");
  //m_fineOperator.residual( fineTmp, fineRhs, fineRhs );
  m_fineOperator.apply( fineTmp, fineTmp2 );
  fineRhs.axpy( -1.0, fineTmp2 );
  //GEOSX_LOG_RANK_VAR( fineRhs.norm2() );
  //GEOSX_LOG_RANK("\n\n\nRelaxed smoothed residual\n\n\n");
  //fineRhs.write("fineRhs2.mtx");

  // ... Coarse-grid correction
  if( 1 )
  {
    //GEOSX_LOG_RANK("--- coarse grid correction");
...
//template< typename LAI >
//void MatrixFreePreconditionerTwoLevel< LAI >::clear()
//{
//  Base::clear();
//  m_precond->clear();
//  m_matSC.reset();
...
template class MatrixFreePreconditionerTwoLevel< TrilinosInterface >;
#endif

#ifdef GEOSX_USE_HYPRE
template class MatrixFreePreconditionerTwoLevel< HypreInterface >;
#endif
