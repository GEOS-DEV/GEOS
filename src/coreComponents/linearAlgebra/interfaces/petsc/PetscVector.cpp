...
PetscVector::PetscVector() : VectorBase(), m_vec {} { }

PetscVector::PetscVector(PetscVector const &src) : PetscVector()
{
  *this = src;
}

PetscVector::PetscVector(PetscVector &&src) noexcept : PetscVector()
{
  *this = std::move(src);
}
...
void PetscVector::createWithLocalSize(localIndex const localSize,
                                      MPI_Comm const &comm)
{
...
void PetscVector::createWithGlobalSize(globalIndex const globalSize,
                                       MPI_Comm const &comm)
{
  GEOSX_LAI_ASSERT(closed());
...
void PetscVector::create(arrayView1d<real64 const> const &localValues,
                         MPI_Comm const &comm)
{
  GEOSX_LAI_ASSERT(closed());
  reset();
...
bool PetscVector::created() const { return m_vec != nullptr; }

void PetscVector::set(globalIndex const globalRow, real64 const value)
{
  GEOSX_LAI_ASSERT(!closed());
  GEOSX_LAI_CHECK_ERROR(VecSetValue(m_vec, globalRow, value, INSERT_VALUES));
}

void PetscVector::add(globalIndex const globalRow, real64 const value)
{
  GEOSX_LAI_ASSERT(!closed());
...
void PetscVector::axpby(real64 const alpha, PetscVector const &x, real64 const beta)
{
  GEOSX_LAI_ASSERT(ready());
  GEOSX_LAI_ASSERT(x.ready());
  GEOSX_LAI_ASSERT_EQ(globalSize(), x.globalSize());
...
void PetscVector::write(string const &filename, LAIOutputFormat const format) const
{
  GEOSX_LAI_ASSERT(ready());
