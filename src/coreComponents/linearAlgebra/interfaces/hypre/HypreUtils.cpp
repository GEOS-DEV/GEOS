                           ...
                           HYPRE_ParVector x )
{
  ...
  if( normType > 0 )
  {
    ...
    GEOSX_LAI_CHECK_ERROR( hypre_ParCSRComputeL1Norms( A, normType, nullptr, &data->norms ) );
  }
  return 0;
}
                          ...
                          HYPRE_ParVector x )
{
  ...
  {
    GEOSX_LAI_CHECK_ERROR( hypre_ParCSRMaxEigEstimateCG( A, 1, data->eigNumIter, &max_eig, &min_eig ) );
  }
  else
  {
    GEOSX_LAI_CHECK_ERROR( hypre_ParCSRMaxEigEstimate( A, 1, &max_eig, &min_eig ) );
  }
