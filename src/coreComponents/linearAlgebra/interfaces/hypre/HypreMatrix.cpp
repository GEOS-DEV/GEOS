...
HypreMatrix::HypreMatrix(HypreMatrix const &src) : HypreMatrix()
{
#if 0
...
bool HypreMatrix::created() const { return m_ij_mat != nullptr; }
...
void HypreMatrix::apply(HypreVector const &src, HypreVector &dst) const
{
  GEOSX_LAI_ASSERT(ready());
  GEOSX_LAI_ASSERT(src.ready());
...
void HypreMatrix::applyTranspose(Vector const &src, Vector &dst) const
{
  GEOSX_LAI_ASSERT(ready());
  GEOSX_LAI_ASSERT(src.ready());
  GEOSX_LAI_ASSERT(dst.ready());
...
void HypreMatrix::multiply(HypreMatrix const &src, HypreMatrix &dst) const
{
  GEOSX_LAI_ASSERT(ready());
  GEOSX_LAI_ASSERT(src.ready());
...
void HypreMatrix::multiplyPtAP(HypreMatrix const &P, HypreMatrix &dst) const
{
  // TODO: figure out how to make this work
#if 1
  GEOSX_LAI_ASSERT(ready());
...
HYPRE_IJMatrix const &HypreMatrix::unwrappedIJ() const { return m_ij_mat; }
...
void HypreMatrix::write(string const &filename, LAIOutputFormat const format) const
{
  GEOSX_LAI_ASSERT(ready());
