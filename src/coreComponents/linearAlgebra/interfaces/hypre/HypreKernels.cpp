                        ...
                        real64 const factor )
{
  ...
  forAll< execPolicy >( hypre_CSRMatrixNumNonzeros( mat ), [=] GEOS_HOST_DEVICE ( HYPRE_Int const i )
  {
    va[i] *= factor;
  } );
}
                      ...
                      hypre_Vector const * const vec )
{
  ...
  forAll< execPolicy >( csr.nrow, [=] GEOS_HOST_DEVICE ( HYPRE_Int const localRow )
  {
...
real64 computeMaxNorm( hypre_CSRMatrix const * const mat )
{
  ...
  forAll< execPolicy >( hypre_CSRMatrixNumNonzeros( mat ), [=] GEOS_HOST_DEVICE ( HYPRE_Int const k )
  {
                       ...
                       globalIndex const firstLocalRow )
{
  ...
  RAJA::ReduceMax< ReducePolicy< execPolicy >, real64 > maxAbsElement( 0.0 );
  forAll< execPolicy >( rowIndices.size(), [=] GEOS_HOST_DEVICE ( localIndex const i )
  {
