...
static void finalize(HYPRE_IJVector &ij_vector, HYPRE_ParVector &par_vector)
{
...
HypreVector::HypreVector() : VectorBase(), m_ij_vector {}, m_par_vector {} { }

// Copy constructor
HypreVector::HypreVector(HypreVector const &src) : HypreVector()
{
  *this = src;
}

// Move constructor
HypreVector::HypreVector(HypreVector &&src) noexcept : HypreVector()
{
  *this = std::move(src);
}
...
void HypreVector::set(globalIndex const globalRow, real64 const value)
{
  GEOSX_LAI_ASSERT(!closed());
...
void HypreVector::add(globalIndex const globalRow, real64 const value)
{
  GEOSX_LAI_ASSERT(!closed());
  GEOSX_LAI_CHECK_ERROR(
...
void HypreVector::axpy(real64 const alpha, HypreVector const &x)
{
  GEOSX_LAI_ASSERT(ready());
  GEOSX_LAI_ASSERT(x.ready());
  GEOSX_LAI_ASSERT_EQ(globalSize(), x.globalSize());
...
void HypreVector::axpby(real64 const alpha, HypreVector const &x, real64 const beta)
{
  scale(beta);
  axpy(alpha, x);
}
...
void HypreVector::write(string const &filename, LAIOutputFormat const format) const
{
  GEOSX_LAI_ASSERT(ready());
  switch(format)
  {
...
HYPRE_ParVector const &HypreVector::unwrapped() const { return m_par_vector; }

HYPRE_IJVector const &HypreVector::unwrappedIJ() const { return m_ij_vector; }
