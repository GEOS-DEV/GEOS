...
EpetraVector::EpetraVector() : VectorBase(), m_vector {} { }

EpetraVector::EpetraVector(EpetraVector const& src) : EpetraVector()
{
  *this = src;
}

EpetraVector::EpetraVector(EpetraVector&& src) noexcept : EpetraVector()
{
  *this = std::move(src);
}
...
bool EpetraVector::created() const { return bool(m_vector); }
...
void EpetraVector::set(globalIndex const globalRowIndex, real64 const value)
{
  GEOSX_LAI_ASSERT(!closed());
...
void EpetraVector::add(globalIndex const globalRowIndex, real64 const value)
{
  GEOSX_LAI_ASSERT(!closed());
  GEOSX_LAI_CHECK_ERROR(
...
void EpetraVector::zero() { set(0.0); }
...
void EpetraVector::axpy(real64 const alpha, EpetraVector const& x)
{
  GEOSX_LAI_ASSERT(ready());
  GEOSX_LAI_ASSERT(x.ready());
  GEOSX_LAI_ASSERT_EQ(globalSize(), x.globalSize());
...
void EpetraVector::write(string const& filename, LAIOutputFormat const format) const
{
  GEOSX_LAI_ASSERT(ready());
  switch(format)
  {
