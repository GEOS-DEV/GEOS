...
auto identity = []( real64 const x ){ return x; };
...
auto reciprocal = []( real64 const x ){ return 1.0 / x; };
...
void compareVectors( VEC const & x,
                     ...
                     OP_Y const op_y = ops::identity )
{
  EXPECT_TRUE( MpiWrapper::Comm_compare( y.getComm(), x.getComm() ) );
...
TYPED_TEST_P( VectorTest, createWithLocalSize )
{
  ...
  int const nproc = MpiWrapper::Comm_size( comm );
  ...
  EXPECT_TRUE( x.ready() );
  EXPECT_TRUE( MpiWrapper::Comm_compare( x.getComm(), comm ) );
...
TYPED_TEST_P( VectorTest, createWithLocalValues )
{
  using Vector = typename TypeParam::ParallelVector;
  ...
  Vector x;
  x.createWithLocalValues( valuesInitial, MPI_COMM_GEOSX );

  EXPECT_TRUE( x.ready() );
  EXPECT_TRUE( MpiWrapper::Comm_compare( x.getComm(), MPI_COMM_GEOSX ) );
  EXPECT_EQ( x.localSize(), localSize );
  EXPECT_EQ( x.globalSize(), globalSize );
...
TYPED_TEST_P( VectorTest, copyConstruction )
{
  ...
  Vector x;
  x.createWithLocalValues( localValues, MPI_COMM_GEOSX );
...
TYPED_TEST_P( VectorTest, moveConstruction )
{
  ...
  EXPECT_TRUE( y.ready() );
  EXPECT_TRUE( MpiWrapper::Comm_compare( y.getComm(), MPI_COMM_GEOSX ) );
...
TYPED_TEST_P( VectorTest, dotProduct )
{
  ...
  Vector x;
  x.createWithLocalValues( xValues, MPI_COMM_GEOSX );

  Vector y;
  y.createWithLocalValues( yValues, MPI_COMM_GEOSX );
  y.reciprocal();
...
INSTANTIATE_TYPED_TEST_SUITE_P( TrilinosTpetra, VectorTest, TrilinosTpetraInterface, );
#endif
