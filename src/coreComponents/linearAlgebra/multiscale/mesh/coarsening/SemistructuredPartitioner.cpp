                 ...
                 FUNC && weightFunc )
{
  ...
  auto const structIndex = mesh.cellManager().getExtrinsicData< extrinsicMeshData::StructuredIndex >().toViewConst();
  ...
  forAll< parallelHostPolicy >( numCells, [=, rowCounts = rowCounts.toView()] ( localIndex const i )
  {
    ...
    GEOSX_ASSERT_EQ( structIndex[cellIdx][1], layerIndex );
                                                ...
                                                arrayView1d< localIndex > const & partition )
{
  ...
  auto const structIndex = cellManager.getExtrinsicData< extrinsicMeshData::StructuredIndex >().toViewConst();
  GEOSX_ERROR_IF_LT_MSG( structIndex.size( 1 ), 2, "Not enough structured indices" );
  ...
  {
    GEOSX_ASSERT_GE( structIndex[i][0], 0 );
    GEOSX_ASSERT_GE( structIndex[i][1], 0 );
  ...
  GEOSX_ASSERT_EQ( numCells % numCellsZ, 0 );
  ...
  else
  {
    ...
    {
      if( m_params.graph.preserveRegions && mesh.cellManager().hasWrapper( meshData::OrigElementRegion::key() ) )
      {
        arrayView1d< localIndex const > const region = mesh.cellManager().getExtrinsicData< meshData::OrigElementRegion >();
...
#else
        GEOSX_THROW( "Attempted to use METIS partition, but GEOSX is not built with METIS support.", InputError );
#endif
        break;
      }
      case Method::scotch:
...
#else
        GEOSX_THROW( "Attempted to use Scotch partition, but GEOSX is not built with Scotch support.", InputError );
#endif
        break;
      }
      default:
      {
        GEOSX_ERROR( "Unrecognized graph partitioner type" );
      }
    }
  }
...
void SemistructuredPartitioner::setCoarseData( MeshLevel & coarseMesh ) const
{
  array2d< integer > & structIndex =
    coarseMesh.cellManager().registerExtrinsicData< extrinsicMeshData::StructuredIndex >( {} ).reference();
