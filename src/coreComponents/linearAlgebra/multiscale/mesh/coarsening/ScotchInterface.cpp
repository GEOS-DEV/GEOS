                ...
                arrayView1d< int64_t > const & partition )
{
  ...
  GEOS_SCOTCH_CHECK( SCOTCH_graphInit( gr ) );
  ...
  GEOS_SCOTCH_CHECK( SCOTCH_graphBuild( gr,           // graphptr
                                        0,            // baseval
                                        numVerts,     // vertnbr
                                        offsets,      // verttab
                                        nullptr,      // vendtab
  ...
  // TODO: maybe remove?
  GEOS_SCOTCH_CHECK( SCOTCH_graphCheck( gr ) );

  SCOTCH_Strat * const strategy = SCOTCH_stratAlloc();
  GEOS_SCOTCH_CHECK( SCOTCH_stratInit( strategy ) );

  GEOS_SCOTCH_CHECK( SCOTCH_graphPart( gr, numPart, strategy, partition.data() ) );
