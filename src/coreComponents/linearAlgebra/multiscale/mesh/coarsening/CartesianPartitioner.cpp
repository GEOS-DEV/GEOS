                                           ...
                                           arrayView1d< localIndex > const & partition )
{
  ...
  GEOSX_THROW_IF_NE_MSG( m_params.ratio.size(), 3,
                         ...
                         InputError );
  GEOSX_THROW_IF( !cellManager.hasExtrinsicData< extrinsicMeshData::StructuredIndex >(),
                  "Cartesian partitioning is only compatible with InternalMesh",
                  InputError );

  arrayView2d< integer const > const cartIndex =
    cellManager.getExtrinsicData< extrinsicMeshData::StructuredIndex >().toViewConst();
