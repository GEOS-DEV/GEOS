                   ...
                   MeshObjectManager & msNodeManager )
{
  ...
  meshUtils::fillArrayByDstIndex< parallelHostPolicy >( nodeLocalIndex,
                                                        msNodeManager.getExtrinsicData< meshData::OrigNodeIndex >().toView(),
                                                        []( auto _ ){ return _; } );
  meshUtils::fillArrayByDstIndex< parallelHostPolicy >( nodeLocalIndex,
                          ...
                          MeshObjectManager & msNodeManager )
{
  ...
  geosx::MeshLevel & mesh = domain.getMeshBody( support.meshBodyName ).getMeshLevel( support.meshLevelName );
                   ...
                   MeshObjectManager & msCellManager )
{
                                                          ...
                                                          ElementSubRegionBase const & subRegion )
  {
    ...
    meshUtils::fillArrayByDstIndex< parallelHostPolicy >( cellLocalIndex,
                                                          msCellManager.getExtrinsicData< meshData::OrigElementRegion >().toView(),
                                                          [er]( auto ){ return er; } );
    meshUtils::fillArrayByDstIndex< parallelHostPolicy >( cellLocalIndex,
                                                          msCellManager.getExtrinsicData< meshData::OrigElementSubRegion >().toView(),
                                                          [esr]( auto ){ return esr; } );
    meshUtils::fillArrayByDstIndex< parallelHostPolicy >( cellLocalIndex,
                                                          msCellManager.getExtrinsicData< meshData::OrigElementIndex >().toView(),
                                                          []( auto _ ){ return _; } );
    meshUtils::fillArrayByDstIndex< parallelHostPolicy >( cellLocalIndex,
    ...
    if( subRegion.hasExtrinsicData< extrinsicMeshData::StructuredIndex >() )
    {
      arrayView2d< integer const > const origStructIndex = subRegion.getExtrinsicData< extrinsicMeshData::StructuredIndex >();
      arrayView2d< integer > const msStructIndex = [&]()
      {
        if( msCellManager.hasWrapper( extrinsicMeshData::StructuredIndex::key() ) )
        {
          return msCellManager.getExtrinsicData< extrinsicMeshData::StructuredIndex >().toView();
        }
        else
        {
          array2d< integer > & msCartIndexArray =
            msCellManager.registerExtrinsicData< extrinsicMeshData::StructuredIndex >( "multiscale::MeshLevel" ).reference();
                               ...
                               Span< DofSupport const > const support )
{
  ...
  GEOSX_ERROR_IF_NE_MSG( support.size(), 1,
                         GEOSX_FMT( "[MsRSB] {}: multiple domain bodies/levels not supported", m_name ) );
  m_domain = &domain;
  m_support = support[0];
  geosx::MeshLevel & sourceMesh = domain.getMeshBody( m_support.meshBodyName ).getMeshLevel( m_support.meshLevelName );
...
{
  geosx::MeshLevel & sourceMesh = m_domain->getMeshBody( m_support.meshBodyName ).getMeshLevel( m_support.meshLevelName );
...
void MeshLevel::writeCellDataCoarse( std::vector< string > const & fieldNames, int depth ) const
{
  ...
  arrayView1d< localIndex const > const coarseCellIndex =
    m_fineMesh->cellManager().getExtrinsicData< meshData::CoarseCellLocalIndex >();
