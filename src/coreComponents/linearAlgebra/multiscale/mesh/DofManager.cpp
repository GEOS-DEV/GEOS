...
{
  GEOSX_ERROR_IF( m_domain == nullptr, "Domain has not been set" );
  GEOSX_ERROR_IF( m_reordered, "Cannot add fields after reorderByRank() has been called." );
  GEOSX_ERROR_IF( fieldExists( fieldName ), GEOSX_FMT( "Duplicate field declared: '{}'.", fieldName ) );
  GEOSX_ERROR_IF_LT_MSG( components, 1, "Invalid number of components specified." );
