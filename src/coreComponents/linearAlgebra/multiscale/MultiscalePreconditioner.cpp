...
void MultiscalePreconditioner< LAI >::logMessage( integer const minLevel, string const & msg ) const
{
  GEOSX_LOG_RANK_0_IF( m_params.logLevel >= minLevel,
                       GEOSX_FMT( "[Multiscale] {}: {}", m_params.multiscale.label, msg ) );
}

template< typename LAI >
void MultiscalePreconditioner< LAI >::computeLevel( integer const level ) const
{
  CALI_CXX_MARK_SCOPE( GEOSX_FMT( "Level {}", level ).c_str() );
  logMessage( 3, GEOSX_FMT( "computing operators for level {}", level ) );
...
void MultiscalePreconditioner< LAI >::setup( Matrix const & mat )
{
  ...
  // setup coarse solver
  {
    GEOSX_MARK_SCOPE( coarse solver );
...
public:
  ...
  ~NormTracker()
  {
    if( m_active )
    {
      ...
      GEOSX_LOG_RANK_0( GEOSX_FMT( "\tLevel {} {}: {:e} -> {:e} | x {:>9.6f}",
                                             ...
                                             Vector & dst ) const
{
  ...
  // down phase
  {
    GEOSX_MARK_SCOPE( v-cycle down phase );
    for( int levelIndex = 0; levelIndex < numLevels - 1; ++levelIndex )
    {
      CALI_CXX_MARK_SCOPE( GEOSX_FMT( "Level {}", levelIndex ).c_str() );
  ...
  // up phase
  {
    GEOSX_MARK_SCOPE( v-cycle up phase );
    for( int levelIndex = numLevels - 2; levelIndex >= 0; --levelIndex )
    {
      ...
      {
        GEOSX_MARK_SCOPE( prolongation );
        coarse.builder->prolongation().apply( coarse.sol, fine.tmp );
      }

      {
        GEOSX_MARK_SCOPE( residual );
