  ...
  : Base( std::move( name ), std::move( params ) )
{
  ...
  for( LinearSolverParameters const * const p : m_params.block.subParams )
  {
    ...
    GEOS_LOG_RANK_0_IF( m_params.multiscale.debugLevel >= 2, GEOS_FMT( "[MsRSB] {}: adding field {}: {}", m_name, m_fields.size(), m_fields.back() ) );
  }
  ...
  for( size_t blockIdx = 0; blockIdx < m_builders.size(); ++blockIdx )
  {
    ...
    if( updateBlock )
    {
                                     ...
                                     rowOffset]( localIndex const localRow )
      {
        ...
        auto const blocksCount = blockVals.size();
        auto const coupledCount = coupledVals.size();
