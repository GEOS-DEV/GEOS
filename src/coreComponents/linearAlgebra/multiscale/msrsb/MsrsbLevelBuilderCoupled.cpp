...
{
  GEOSX_ASSERT( !m_params.block.subParams.empty() );
  for( LinearSolverParameters const * const p : m_params.block.subParams )
  {
    ...
    string levelName = GEOSX_FMT( "{}_{}", m_name, p->multiscale.label );
...
void MsrsbLevelBuilderCoupled< LAI >::createSmoothers()
{
  auto const makeSmoother = [&]( auto const getSmoother ) -> std::unique_ptr< PreconditionerBase< LAI > >
  {
    ...
    {
      GEOSX_ERROR_IF_NE_MSG( m_params.block.subParams.size(), 2, "More than 2 blocks are not supported yet" );
      auto smoother = std::make_unique< BlockPreconditioner< LAI > >( m_params.block );
      for( integer i = 0; i < m_params.block.subParams.size(); ++i )
      {
        LinearSolverParameters params;
        params.preconditionerType = m_params.block.subParams[i]->multiscale.smoother.type;
        geosx::DofManager::SubComponent comp{ m_params.block.subParams[i]->multiscale.fieldName, { m_builders[i]->numComp(), true } };
        smoother->setupBlock( i, { comp }, getSmoother( *m_builders[i] ) );
        smoother->setProlongation( i, m_selectors[i] );
...
{
  GEOSX_ASSERT_EQ( m_prolongationBlocks.size(), m_dofManager.numFields() );
                                                             ...
                                                             Matrix const & fineMat )
{
  ...
  {
    Matrix fineBlock;
    fineMat.multiplyPtAP( fine.m_selectors[blockIdx], fineBlock );
  ...
  initializeCommon( fine.dofManager().domain(), fineMat.comm() );
  createSmoothers();
...
bool MsrsbLevelBuilderCoupled< LAI >::updateProlongation( Matrix const & fineMatrix )
{
  ...
  for( size_t blockIdx = 0; blockIdx < m_builders.size(); ++blockIdx )
  {
    Matrix fineBlock;
    {
      GEOSX_MARK_SCOPE( extract blocks );
    ...
    if( updateBlock )
    {
                                     ...
                                     rowOffset]( localIndex const localRow )
      {
        ...
        GEOSX_ASSERT_EQ( blockVals.size(), coupledVals.size() );
  ...
  if( update )
  {
    GEOSX_MARK_SCOPE( create LAI object );
    GEOSX_LOG_RANK_0_IF( m_params.multiscale.debugLevel >= 2, GEOSX_FMT( "[MsRSB] {}: updating coupled prolongation", m_name ) );
  ...
  {
    m_prolongation.write( GEOSX_FMT( "{}_P_{}.mtx", m_name, "conv" ), LAIOutputFormat::MATRIX_MARKET );
  }
...
MsrsbLevelBuilderCoupled< LAI >::makeCoarseSolver() const
{
  ...
  {
    GEOSX_ERROR_IF_NE_MSG( m_builders.size(), 2, "More than 2 blocks are not supported yet" );
    auto solver = std::make_unique< BlockPreconditioner< LAI > >( params.block );
    for( integer i = 0; i < static_cast< integer >( m_builders.size() ); ++i )
    {
      geosx::DofManager::SubComponent comp{ params.block.subParams[i]->multiscale.fieldName, { m_builders[i]->numComp(), true } };
