...
{
  GEOSX_MARK_FUNCTION;
  GEOSX_UNUSED_VAR( coarse );

  ArrayOfSets< localIndex > const cellConn =
    msrsb::buildLocalConnectivity( fine.cellManager(), true, fine.nodeManager(), true );

  auto const coarseCells = fine.cellManager().getExtrinsicData< meshData::CoarseCellLocalIndex >().toViewConst();
                          ...
                          integer const numLayers )
{
  ...
  auto const coarseNodes = coarse.nodeManager().getExtrinsicData< meshData::FineNodeLocalIndex >().toViewConst();
               ...
               LinearSolverParameters::Multiscale const & params )
{
  switch( params.msrsb.support )
  {
    ...
    default:
    {
      GEOSX_ERROR( "Unrecognized support region algorithm" );
      return {};
    }
  }
}
                                                      ...
                                                      Matrix const & fineMat )
{
  ...
  auto & fine = dynamicCast< MsrsbLevelBuilder< LAI > & >( fine_level );
  m_fineLevel = &fine;
...
{
  GEOSX_MARK_SCOPE( build iteration matrix );
...
integer iterateBasis( Matrix const & jacobiMatrix,
                      ...
                      Matrix & prolongation )
{
  GEOSX_MARK_SCOPE( iterate basis );
  ...
  auto const computeAndLogConvergenceNorm = [&]()
  {
    ...
    GEOSX_LOG_RANK_0_IF( debugLevel >= 2, GEOSX_FMT( "[MsRSB] {}: iter = {}, conv = {:e}", name, iter, norm ) );
  };
  ...
  GEOSX_LOG_RANK_0_IF( debugLevel >= 1,
                       GEOSX_FMT( "[MsRSB] {}: {} in {} iterations",
...
bool MsrsbLevelBuilder< LAI >::updateProlongation( Matrix const & fineMatrix )
{
  ...
  GEOSX_LOG_RANK_0_IF( m_params.multiscale.debugLevel >= 2, GEOSX_FMT( "[MsRSB] {}: building iteration matrix", m_name ) );
  ...
  GEOSX_LOG_RANK_0_IF( m_params.multiscale.debugLevel >= 2, GEOSX_FMT( "[MsRSB] {}: performing basis iteration", m_name ) );
