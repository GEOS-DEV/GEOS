...
public:
   ...
   * @copydoc block(localIndex const) const
   */
  VECTOR& operator()(localIndex const blockIndex) { return block(blockIndex); }
...
protected:
   ...
   */
  explicit BlockVectorView(localIndex const nBlocks) : m_vectors(nBlocks) { }
   ...
   */
  void resize(localIndex const size) { m_vectors.resize(size); }
   ...
   */
  void setPointer(localIndex i, VECTOR* vec) { m_vectors(i) = vec; }
...
template <typename VECTOR> void BlockVectorView<VECTOR>::zero()
{
  for(localIndex i = 0; i < blockSize(); i++)
  {
    block(i).zero();
...
template <typename VECTOR> real64 BlockVectorView<VECTOR>::norm2() const
{
  real64 accum = 0;
  for(localIndex i = 0; i < blockSize(); i++)
  {
...
template <typename VECTOR> real64 BlockVectorView<VECTOR>::normInf() const
{
  real64 result = 0.0;
  for(localIndex i = 0; i < blockSize(); i++)
  {
...
template <typename VECTOR>
void BlockVectorView<VECTOR>::axpy(real64 const alpha, BlockVectorView const& x)
{
...
template <typename VECTOR> localIndex BlockVectorView<VECTOR>::blockSize() const
{
...
template <typename VECTOR> localIndex BlockVectorView<VECTOR>::localSize() const
{
  localIndex size = 0;
  for(localIndex i = 0; i < blockSize(); i++)
  {
