...
public:
   ...
   */
  static const string CatalogName() { return "CellBlock"; }
   ...
   * @param[in] X an arrayView of (const) node positions
   */
  inline void CalculateCellVolumesKernel(
    localIndex const k,
    arrayView2d<real64 const, nodes::REFERENCE_POSITION_USD> const& X) const
  {
   ...
   */
  template <typename T> T& AddProperty(string const& propertyName)
  {
   ...
   */
  template <typename LAMBDA> void forExternalProperties(LAMBDA&& lambda)
  {
