...
public:
  ...
  CellElementRegionSelector( dataRepository::Group const & cellBlocks,
                             map< integer, std::set< string > > const & cellBlocksRegion );
...
private:
  ...
  map< string, stdVector< CellElementRegion const * > > m_cellBlocksOwners;

  /// @brief A map that link every region attribute values to the CellElementRegion(s) that references it.
  map< string, stdVector< CellElementRegion const * > > m_regionAttributesOwners;
   ...
   */
  map< string, std::set< string > const & > m_regionAttributesCellBlocks;
