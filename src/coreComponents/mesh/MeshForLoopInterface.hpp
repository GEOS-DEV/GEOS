...
void for_elems( MeshLevel const * const mesh, LAMBDA && body )
{
  ...
  elementRegions = elemManager->GetGroup( ElementRegionManager::groupKeyStruct::elementRegionsGroup );
...
// template<typename NUMBER=real64, class EXEC_POLICY=serialPolicy, class REDUCE_POLICY=reducePolicy, typename
// LAMBDA=void>
...
// #define FOR_ELEMS_IN_SUBREGION( SUBREGION, INDEX ) for_elems_in_subRegion( SUBREGION, [=] ( localIndex const INDEX )
// mutable -> void
#endif
...
/*
   ...
   for( auto const & regionPair : elementRegions->GetSubGroups() )
   {
    ...
    FiniteElementSpace const * const feDiscretization =
       feDiscretizationManager->GetGroup<FiniteElementSpace>(numMethodName);

    dataRepository::Group const * const elementSubRegions =
       elementRegion->GetGroup(dataRepository::keys::elementSubRegions);
    for( auto & iterCellBlocks : elementSubRegions->GetSubGroups() )
    {
      ...
      auto const & constitutiveMap = cellBlock->getReference< std::pair< array2d<localIndex>,array2d<localIndex> >
         >(CellBlockSubRegion::viewKeyStruct::constitutiveMapString);
   //      RAJA::View< localIndex const, RAJA::Layout<2> > constitutiveMapView( reinterpret_cast<localIndex
      const*>(constitutiveMap.second.data()),
   ...
   //                                                                           constitutiveMap.second.size(1) );
      ...
      auto const & constitutiveGrouping = cellBlock->getReference< map< string, localIndex_array >
         >(CellBlockSubRegion::viewKeyStruct::constitutiveGroupingString);
      arrayView2d<localIndex> const & elemsToNodes =
         cellBlock->getWrapper<FixedOneToManyRelation>(cellBlock->viewKeys().nodeList)->reference();//
         getData<array2d<localIndex>>(keys::nodeList);
   ...
   //      localIndex_array const & elementList = constitutiveGroup.second;
        ...
        constitutive::ConstitutiveBase * constitutiveModel =
           constitutiveManager->GetGroup<constitutive::ConstitutiveBase>( constitutiveName );
        ...
        ///Local copies --------
        arrayView1d<real64> const & meanStress =
           constitutiveModel->GetGroup(std::string("StateData"))->getReference<real64_array>(std::string("MeanStress"));
        arrayView1d<R2SymTensor> const & devStress =
           constitutiveModel->GetGroup(std::string("StateData"))->getReference<r2Sym_array>(std::string("DeviatorStress"));
        //------------------------
