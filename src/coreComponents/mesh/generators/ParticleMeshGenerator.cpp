...
static int getNumElemPerBox( ElementType const elementType )
{
  switch( elementType )
  {
    ...
    default:
    {
      GEOSX_ERROR( "ParticleMeshGenerator: unsupported element type " << elementType );
      return 0;
    }
  }
}
...
void ParticleMeshGenerator::postProcessInput()
{
...

  {
    ...
    if( failFlag )
    {
      GEOSX_ERROR( "vertex/element mismatch ParticleMeshGenerator::ReadXMLPost()" );
    }
    ...
    if( failFlag )
    {
      GEOSX_ERROR( "element/bias mismatch ParticleMeshGenerator::ReadXMLPost()" );
    }
  }
  ...
  if( m_elementType.size() != m_numElePerBox.size() )
  {
    ...
    {
      GEOSX_ERROR( "ParticleMeshGenerator: The number of element types is inconsistent with the number of total block." );
    }
  }
...

  {
    ...
    if( numBlocks != m_regionNames.size() )
    {
      ...
      {
        GEOSX_ERROR( "Incorrect number of regionLayout entries specified in ParticleMeshGenerator::ReadXML()" );
      }
    }
  }
...
void ParticleMeshGenerator::generateMesh( DomainPartition & domain )
{
  ...
  // -------------------------------------------------------------------------------------------------------------------
  // ------------------------------------ BEGIN NEW MPM PARTICLE FILE STUFF --------------------------------------------
  // -------------------------------------------------------------------------------------------------------------------

  std::cout << "MPM particle file name: " << m_filePath << std::endl;
  ...
  CellBlockManager & elementManager = meshBody.getGroup< CellBlockManager >( keys::cellManager );
