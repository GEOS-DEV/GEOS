...
void VTKMeshGenerator::fillCellBlockManager( CellBlockManager & cellBlockManager, SpatialPartition & partition )
{
  ...
  {
    GEOS_LOG_LEVEL_RANK_0( logInfo::VTKSteps, GEOS_FMT( "{} '{}': importing fracture network {}...", catalogName(), getName(), name ) );
    vtk::importFractureNetwork( name, mesh, m_vtkMesh, cellBlockManager );
  }
  for( auto const & [name, mesh]: m_embeddedSurfaceBlockMeshes )
  {
    GEOS_LOG_LEVEL_RANK_0( logInfo::VTKSteps, GEOS_FMT( "{} '{}': importing embedded fracture network {}...", catalogName(), getName(), name ) );
