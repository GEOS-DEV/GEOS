...
namespace geosx
{
WellBlock::WellBlock( const InternalWellGenerator & internalWellGenerator ) :
  ...
  m_nDims( internalWellGenerator.getPhysicalDimensionsNumber() )
{
  ...
  m_numElems = m_numElemsPerSegment * internalWellGenerator.getSegmentToPolyNodeMap().size( 0 );
  ...
  constructPolylineNodeToSegmentMap( internalWellGenerator );

  // detect the head polyline node based on depth
  findPolylineHeadNodeIndex( internalWellGenerator );

  // compute the location and distance from top of the well elements
  discretizePolyline( internalWellGenerator );

  // map the perforations to the well elements
  connectPerforationsToWellElements( internalWellGenerator );

  // make sure that the perforation locations are valid
  checkPerforationLocationsValidity( internalWellGenerator );

  if( internalWellGenerator.getLogLevel() >= 1 )
  {
    debugWellGeometry( internalWellGenerator );
  }

}

void WellBlock::constructPolylineNodeToSegmentMap( const InternalWellGenerator & internalWellGenerator )
{
  const array2d< real64 > & polyNodeCoords = internalWellGenerator.getPolyNodeCoord();
  m_polyNodeToSegmentMap.resize( polyNodeCoords.size( 0 ) );

  bool foundSmallElem = false;
  const array2d< globalIndex > & segmentToPolyNodeMap = internalWellGenerator.getSegmentToPolyNodeMap();


  // loop over the segments
  for( globalIndex iseg = 0; iseg < segmentToPolyNodeMap.size( 0 ); ++iseg )
  {
    globalIndex const ipolyNode_a = segmentToPolyNodeMap[iseg][0];
    globalIndex const ipolyNode_b = segmentToPolyNodeMap[iseg][1];

    real64 vSeg[3] = LVARRAY_TENSOROPS_INIT_LOCAL_3( polyNodeCoords[ipolyNode_a] );
    LvArray::tensorOps::subtract< 3 >( vSeg, polyNodeCoords[ipolyNode_b] );
    ...
    GEOSX_THROW_IF( segmentLength < m_minSegmentLength,
                    "Error in the topology of well '" << internalWellGenerator.getName() <<
                    "': we detected a polyline segment measuring less than " << m_minSegmentLength << "m. \n" <<
                    "You can change the minimum segment length using the field " << InternalWellGenerator::viewKeyStruct::minSegmentLengthString(),
                    InputError );

    GEOSX_THROW_IF( polyNodeCoords[ipolyNode_a][2] < polyNodeCoords[ipolyNode_b][2],
                    "Error in the topology of well '" << internalWellGenerator.getName() <<
                    "': in the polyline, each segment must be going down. \n" <<
                    "This is not the case between polyline nodes " << polyNodeCoords[ipolyNode_a] << " and " << polyNodeCoords[ipolyNode_b],
                    InputError );
  ...
  if( foundSmallElem )
  {
    GEOSX_LOG_RANK_0( "\nWarning: the chosen number of well elements per polyline segment (" << m_numElemsPerSegment <<
                      ") leads to well elements measuring less than " << m_minElemLength << "m in the topology of well '" << internalWellGenerator.getName() << "'.\n" <<
                      ...
                      "You can get rid of this message by changing the field " << InternalWellGenerator::viewKeyStruct::minElementLengthString() );
  }
}

void WellBlock::findPolylineHeadNodeIndex( const InternalWellGenerator & internalWellGenerator )
{
  ...
  const array2d< globalIndex > segmentToPolyNodeMap = internalWellGenerator.getSegmentToPolyNodeMap();  globalIndex const ipolyNode_a = segmentToPolyNodeMap[wellHeadSegId][0];
  globalIndex const ipolyNode_b = segmentToPolyNodeMap[wellHeadSegId][1];
  ...
  const array2d< real64 > & polyNodeCoords = internalWellGenerator.getPolyNodeCoord();
  GEOSX_THROW_IF( !(polyNodeCoords[ipolyNode_a][2] < polyNodeCoords[ipolyNode_b][2])
                  && !(polyNodeCoords[ipolyNode_a][2] > polyNodeCoords[ipolyNode_b][2]),
                  "The head polyline segment cannot be horizontal in well '" << internalWellGenerator.getName()
  ...
  real64 const headZcoord = polyNodeCoords[m_polylineHeadNodeId][2];
  for( globalIndex inode = 0; inode < polyNodeCoords.size( 0 ); ++inode )
  {
    if( inode == m_polylineHeadNodeId )
    {
      continue;
    }
    real64 const currentZcoord = polyNodeCoords[inode][2];
...
void WellBlock::discretizePolyline( const InternalWellGenerator & internalWellGenerator )
{
  ...
  const array2d< real64 > & polyNodeCoords = internalWellGenerator.getPolyNodeCoord();
  LvArray::tensorOps::copy< 3 >( m_nodeCoords[0], polyNodeCoords[ipolyNodeTop] );
  ...
  const array2d< globalIndex > segmentToPolyNodeMap = internalWellGenerator.getSegmentToPolyNodeMap();
  for( globalIndex is = 0; is < segmentToPolyNodeMap.size( 0 ); ++is )
  {

    GEOSX_THROW_IF( isegCurrent == -1,
                    "Invalid segmentToNode map in well " << internalWellGenerator.getName(),
                    InputError );

    globalIndex const ipolyNodeBottom = ( ipolyNodeTop == segmentToPolyNodeMap[isegCurrent][0] )
                                      ? segmentToPolyNodeMap[isegCurrent][1]
                                      : segmentToPolyNodeMap[isegCurrent][0];

    real64 vPoly[3] = LVARRAY_TENSOROPS_INIT_LOCAL_3( polyNodeCoords[ipolyNodeBottom] );
    LvArray::tensorOps::subtract< 3 >( vPoly, polyNodeCoords[ipolyNodeTop] );
    ...
    for( localIndex iw = 0; iw < m_numElemsPerSegment; ++iw )
    {
      ...
      LvArray::tensorOps::add< 3 >( m_elemCenterCoords[iwelemCurrent], polyNodeCoords[ipolyNodeTop] );

      GEOSX_THROW_IF( iwelemCurrent >= m_numElems,
                      "Invalid well topology in well " << internalWellGenerator.getName(),
                      InputError );
      ...
      m_elemToNodesMap[iwelemCurrent][InternalWellGenerator::NodeLocation::TOP]    = iwellNodeTop;
      m_elemToNodesMap[iwelemCurrent][InternalWellGenerator::NodeLocation::BOTTOM] = iwellNodeBottom;
      ...
      LvArray::tensorOps::add< 3 >( m_nodeCoords[iwellNodeBottom], polyNodeCoords[ipolyNodeTop] );
...
void WellBlock::connectPerforationsToWellElements( const InternalWellGenerator & internalWellGenerator )
{
  ...
  for( globalIndex iperf = 0; iperf < m_numPerforations; ++iperf )
  {
    ...
    Perforation const & perf = internalWellGenerator.getGroup< Perforation >( internalWellGenerator.getPerforationList()[iperf] );
    ...
    real64 const wellLength = m_nodeDistFromHead[m_elemToNodesMap[iwelemBottom][InternalWellGenerator::NodeLocation::BOTTOM]];

    GEOSX_THROW_IF( m_perfDistFromHead[iperf] > wellLength,
                    "Distance from perforation " << perf.getName() << " to head is larger than well polyline length for well " << internalWellGenerator.getName() << "\n \n"
    ...
    while( iwelemTop < iwelemBottom )
    {
      ...
      real64 const headToBottomDist =
        m_nodeDistFromHead[m_elemToNodesMap[iwelemMid][InternalWellGenerator::NodeLocation::BOTTOM]];
      ...
      GEOSX_THROW_IF( currentNumSteps > maxNumSteps,
                      "Perforation " << perf.getName() << " cannot be mapped to a well element in well " << internalWellGenerator.getName(),
                      InputError );
    ...
    GEOSX_THROW_IF( iwelemMatched >= m_numElems,
                    "Invalid topology in well " << internalWellGenerator.getName(),
                    InputError );

    m_perfElemId[iperf] = iwelemMatched;

    // compute the physical location of the perforation
    globalIndex const inodeTop    = m_elemToNodesMap[iwelemMatched][InternalWellGenerator::NodeLocation::TOP];
    globalIndex const inodeBottom = m_elemToNodesMap[iwelemMatched][InternalWellGenerator::NodeLocation::BOTTOM];
...
void WellBlock::checkPerforationLocationsValidity( const InternalWellGenerator & internalWellGenerator )
{
  ...
  if( mpiSize > 1 )
  {
    mergePerforations( elemToPerfMap, internalWellGenerator );
  }
  ...
  for( globalIndex iwelem = 0; iwelem < m_numElems; ++iwelem )
  {
    ...
    for( localIndex iwelemPrev = 0; iwelemPrev < m_prevElemId[iwelem].size(); ++iwelemPrev )
    {
      GEOSX_THROW_IF( m_prevElemId[iwelem][iwelemPrev] == -1 && elemToPerfMap[iwelem].size() == 0,
                      "The bottom element of well " << internalWellGenerator.getName() << " does not have a perforation. "
...
globalIndex WellBlock::getNextSegmentIndex( globalIndex topSegId,
                                            globalIndex currentPolyNodeId ) const
{
  ...
  void WellBlock::mergePerforations( array1d< array1d< localIndex > > const & elemToPerfMap, const InternalWellGenerator & internalWellGenerator )
{
    ...
    if( elemToPerfMap[iwelem].size() > 1 )
    {
      ...
      for( localIndex ip = 0; ip < elemToPerfMap[iwelem].size(); ++ip )
      {
        ...
        GEOSX_LOG_RANK_0( "\n \nThe GEOSX wells currently have the following limitation in parallel: \n"
                          ...
                          << " of well " << internalWellGenerator.getName()
...
void WellBlock::debugWellGeometry(const InternalWellGenerator & internalWellGenerator) const
{
  ...
  std::cout << std::endl;
  std::cout << "++++++++++++++++++++++++++" << std::endl;
  std::cout << "InternalWellGenerator = " << internalWellGenerator.getName() << std::endl;
