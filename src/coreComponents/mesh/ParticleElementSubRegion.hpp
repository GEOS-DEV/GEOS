...
/**
 * @class ParticleElementSubRegion
 * Class deriving from ParticleBlock further specializing the element subregion
 * for a particle element. This is the class used in the physics solvers to
 * represent a collection of mesh particle elements
 */
class ParticleElementSubRegion : public ParticleBlock
{
public:

  /// Type of map between particle blocks and embedded elements
   ...
   */
  ParticleElementSubRegion( string const & name, Group * const parent );

  /**
   * @brief Destructor.
   */
  virtual ~ParticleElementSubRegion() override;

  ///@}

  /**
   * @name Helpers for ParticleElementSubRegion construction
   */
  ///@{

  /**
   * @brief Fill the ParticleElementSubRegion by copying those of the source ParticleBlock
  ...
  /**
   * @brief Fill the ParticleElementSubRegion by querying a target set into the faceManager
   ...
   * @param particleElemIndex particle element index
   * @param embSurfIndex embedded surface element index
   */
  void addFracturedElement( localIndex const particleElemIndex,
  ...
  /// viewKey struct for the ParticleElementSubRegion class
  ...
  { return m_fracturedParticles.toViewConst(); }
...
private:
  ...
  SortedArray< localIndex > m_fracturedParticles;

  /// Map from Particle Elements to Embedded Surfaces
