...
void ParMetisPartitioner::setPartitionCounts( unsigned int xPartitions, unsigned int yPartitions, unsigned int zPartitions )
{
  m_numPartitions = xPartitions * yPartitions * zPartitions;
}

void ParMetisPartitioner::setNeighborsRank( const std::vector< int > & neighborsRank )
{
...
array1d< idx_t >
ParMetisPartitioner::partition( ArrayOfArraysView< idx_t const, idx_t > const & graph,
