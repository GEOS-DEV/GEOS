      ...
      forAll< serialPolicy >( ni, [&, particleRank] GEOS_HOST ( localIndex const k )
        {
          ...
          particleRank[pp] = neighbors[n].neighborRank();
        } );
    }
                                                                               ...
                                                                               const real64 & boundaryRadius )
{
  ...
  // Get first-order neighbors from the domain (we'll iterate using getNumFirstOrderNeighbors as the limit)
  int const numFirstOrderNeighbors = domain.getNumFirstOrderNeighbors();
  stdVector< NeighborCommunicator > & neighbors = domain.getNeighbors();
  ...
  particleManager.forParticleSubRegions( [&]( ParticleSubRegion & subRegion )
  {
    ...
    sendCoordinateListToNeighbors( inDomainMasterParticleCoordinates.toView(),          // input: Single list of coordinates sent to all
                                                                                        // neighbors
                                   commData,                                              // input: Solver MPI communicator
                                   particleCoordinatesReceivedFromNeighbors,              // output: List of lists of coordinates received
                                                                                          // from each neighbor.
                                   neighbors,                                             // input: Neighbors to communicate with
                                   numFirstOrderNeighbors );                              // input: Number of first-order neighbors
          ...
          forAll< serialPolicy >( subRegion.size(), [&, particleGlobalID, particleRank] GEOS_HOST ( localIndex const p )
            {
              if( gI == particleGlobalID[p] )
              {
                ...
                particleRank[p] = neighbors[n].neighborRank();
                alreadyHere = true;
              }
            } );
          if( !alreadyHere )
...
{
  // Number of first-order neighboring partitions
  unsigned int nn = numFirstOrderNeighbors;
  ...
  // the mpi request will be different for each send, but the buffer is the same
  {
    ...
    for( size_t n=0; n<nn; n++ )
    {
      ...
      // Perform comms
      neighbors[n].mpiISendReceive( &sizeOfPacked,
                                    1,
                                    sendRequest[n],
                                    &(sizeOfReceived[n]),
                                    1,
  ...
  // Send/receive the buffer containing the list of coordinates
  {
    ...
    for( size_t n=0; n<nn; n++ )
    {
      ...
      neighbors[n].mpiISendReceive( sendBuffer.data(),   // TODO: This can't be sendBufferPtr, why not? I guess cuz sendBufferPtr gets
                                                                 ...
                                                                 int numFirstOrderNeighbors )
{
  ...
  // send the list of local indices to each neighbor using an asynchronous send
  {
    ...
    for( size_t n=0; n<nn; n++ )
    {
      ...
      // Perform comms
      neighbors[n].mpiISendReceive( &(sizeOfPacked[n]),
                                    1,
                                    sendRequest[n],
                                    &(sizeOfReceived[n]),
                                    1,
  ...
  // Send/receive the buffer containing the list of local indices
  {
    ...
    for( size_t n=0; n<nn; n++ )
    {
      ...
      // Perform comms
      receiveBuffer[n].resize( sizeOfReceived[n] );
      neighbors[n].mpiISendReceive( sendBuffer[n].data(),   // TODO: This can't be sendBufferPtr, why not? I guess cuz sendBufferPtr gets
                                                            // incremented (moved) during packing.
                                    sizeOfPacked[n],
                                    sendRequest[n],
                                    receiveBuffer[n].data(),
