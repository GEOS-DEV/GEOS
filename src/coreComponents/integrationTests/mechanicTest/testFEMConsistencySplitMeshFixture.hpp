...
<Problem>
 <Mesh>
   <VTKMesh name="mesh1" checkEulerCharacteristic="1" useGlobalIds="1" file=")xml" << meshFile <<
 ...
 <FieldSpecifications>
   ...
   <Traction name="ypos_traction" setNames="{ yposFace }" objectPath="faceManager" 
             tractionType="normal" scale=")xml" << s_yy <<
      R"xml("/>
   <Traction name="zpos_traction" setNames="{ zposFace }" objectPath="faceManager" 
              tractionType="normal" scale=")xml" << s_zz <<
      R"xml("/>

    <!-- Pre-populate averageStress on volume elements so that implicitStepSetup
         calls initializeTractionFromAdjacentCellStress, which computes the exact
         per-element traction t_local = R^T * sigma * n.
         Voigt order: [s_xx, s_yy, s_zz, s_yz, s_xz, s_xy] -->
    <FieldSpecification name="initial_stress_xx" initialCondition="1"
                        component="0"
                        setNames="{ all }"
                        ...
                        scale=")xml" << s_xx <<
      R"xml("/>
    <FieldSpecification name="initial_stress_yy" initialCondition="1"
                        component="1"
                        setNames="{ all }"
                        objectPath="ElementRegions/Region/*"
                        fieldName="averageStress"
                        scale=")xml" << s_yy <<
      R"xml("/>
    <FieldSpecification name="initial_stress_zz" initialCondition="1"
                        component="2"
                        setNames="{ all }"
                        objectPath="ElementRegions/Region/*"
                        fieldName="averageStress"
