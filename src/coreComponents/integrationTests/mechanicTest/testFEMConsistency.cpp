...
constexpr real64 tolerance = 1.0e-8;
...
<Problem>
  ...
  <FieldSpecifications>
    ...
    <Traction name="xpos_traction" setNames="{ xposFace }" objectPath="faceManager" tractionType="normal" scale=")xml" << s_xx << R"xml("/>
    <Traction name="ypos_traction" setNames="{ yposFace }" objectPath="faceManager" tractionType="normal" scale=")xml" << s_yy << R"xml("/>
    <Traction name="zpos_traction" setNames="{ zposFace }" objectPath="faceManager" tractionType="normal" scale=")xml" << s_zz << R"xml("/>
  </FieldSpecifications>
  <Tasks>
    ...
    // Scoped state to ensure cleanup
    {
        ...
        fractureRegion.forElementSubRegions< FaceElementSubRegion >( [&]( FaceElementSubRegion & subRegion )
        {
            ...
            for( localIndex k=0; k<faces.size(); ++k )
            {
                ...
                // 1. Compute Geometric Normal
                auto const & fnodes = faceNodes[f];
                if( fnodes.size() < 3 ) continue;
                ...
                // 3. Get average stress from cell subregion
