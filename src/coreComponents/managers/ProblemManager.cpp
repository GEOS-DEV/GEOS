...
ProblemManager::ProblemManager( const std::string & name, Group * const parent ) :
  ...
  m_functionManager( nullptr )
{
  ...
  RegisterGroup< FieldSpecificationManager >(
    groupKeys.fieldSpecificationManager.Key(),
  ...
  m_physicsSolverManager =
    RegisterGroup< PhysicsSolverManager >( groupKeys.physicsSolverManager );
  ...
  commandLine->registerWrapper< string >( viewKeys.problemName.Key() )
    ->setRestartFlags( RestartFlags::WRITE )
    ->setDescription(
      "Used in writing the output files, if not specified defaults to the name "
      "of the input file." );

  commandLine->registerWrapper< string >( viewKeys.outputDirectory.Key() )
    ->setRestartFlags( RestartFlags::WRITE )
    ->setDescription(
      "Directory in which to put the output files, if not specified defaults "
  ...
  commandLine->registerWrapper< integer >( viewKeys.useNonblockingMPI.Key() )
    ...
    ->setRestartFlags( RestartFlags::WRITE )
    ->setDescription(
      "Whether to prefer using non-blocking MPI communication where "
      "implemented (results in non-deterministic DOF numbering)." );

  commandLine->registerWrapper< integer >( viewKeys.suppressPinned.Key() )
    ->setApplyDefaultValue( 0 )
    ->setRestartFlags( RestartFlags::WRITE )
    ->setDescription(
      "Whether to disallow using pinned memory allocations for MPI "
...
Group *
ProblemManager::CreateChild( string const & GEOSX_UNUSED_PARAM( childKey ),
...
ProblemManager::ProblemSetup()
{
  ...
  RegisterDataOnMeshRecursive(
    GetGroup< DomainPartition >( groupKeys.domain )->getMeshBodies() );
...
ProblemManager::ParseCommandLineInput()
{
  ...
  commandLine->getReference< std::string >( viewKeys.restartFileName ) =
    opts.restartFileName;
  commandLine->getReference< integer >( viewKeys.beginFromRestart ) =
    opts.beginFromRestart;
  commandLine->getReference< integer >( viewKeys.xPartitionsOverride ) =
    opts.xPartitionsOverride;
  commandLine->getReference< integer >( viewKeys.yPartitionsOverride ) =
    opts.yPartitionsOverride;
  commandLine->getReference< integer >( viewKeys.zPartitionsOverride ) =
    opts.zPartitionsOverride;
  commandLine->getReference< integer >( viewKeys.overridePartitionNumbers ) =
    opts.overridePartitionNumbers;
  commandLine->getReference< integer >( viewKeys.useNonblockingMPI ) =
    opts.useNonblockingMPI;
  commandLine->getReference< integer >( viewKeys.suppressPinned ) =
    opts.suppressPinned;

  std::string & inputFileName =
    commandLine->getReference< std::string >( viewKeys.inputFileName );
  inputFileName = opts.inputFileName;

  std::string & schemaName =
    commandLine->getReference< std::string >( viewKeys.schemaFileName );
  schemaName = opts.schemaName;

  std::string & problemName =
    commandLine->getReference< std::string >( viewKeys.problemName );
  problemName = opts.problemName;

  std::string & outputDirectory =
    commandLine->getReference< std::string >( viewKeys.outputDirectory );
...
bool
ProblemManager::ParseRestart( std::string & restartFileName )
{
  CommandLineOptions const & opts = getCommandLineOptions();
  ...
  if( beginFromRestart == 1 )
  {
    ...
    GEOSX_ERROR_IF( dir_contents.size() == 0,
                    "Directory gotten from " << restartFileName << " " << dirname
                                             << " is empty." );
    ...
    GEOSX_ERROR_IF( !match_found,
                    "No matches found for pattern "
                      << basename << " in directory " << dirname << "." );
...
ProblemManager::GenerateDocumentation()
{
  ...
  if( !schemaName.empty() )
  {
    ...
    schemaUtilities::ConvertDocumentationToSchema( ( schemaName + ".other" ).c_str(),
                                                   this,
                                                   1 );
  }
}
...
void
ProblemManager::SetSchemaDeviations( xmlWrapper::xmlNode schemaRoot,
                                     ...
                                     integer documentationType )
{
  ...
  schemaUtilities::SchemaConstruction( m_functionManager,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
  ...
  schemaUtilities::SchemaConstruction( &bcManager,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
  ...
  schemaUtilities::SchemaConstruction( constitutiveManager,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
  ...
  schemaUtilities::SchemaConstruction( elementManager,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
  ...
  schemaUtilities::SchemaConstruction( IncludedList,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
  ...
  schemaUtilities::SchemaConstruction( parameterList,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
  ...
  for( std::string const & machineName : { "quartz", "lassen" } )
  {
    ...
    run->registerWrapper< std::string >( "name" )
      ->setInputFlag( InputFlags::REQUIRED )
      ->setDescription( "The name of this benchmark." );

    run->registerWrapper< int >( "nodes" )
      ->setInputFlag( InputFlags::REQUIRED )
      ->setDescription( "The number of nodes needed to run the benchmark." );

    run->registerWrapper< int >( "tasksPerNode" )
      ->setInputFlag( InputFlags::REQUIRED )
      ->setDescription(
        "The number of tasks per node to run the benchmark with." );

    run->registerWrapper< int >( "threadsPerTask" )
      ->setInputFlag( InputFlags::OPTIONAL )
      ->setDescription(
        "The number of threads per task to run the benchmark with." );

    run->registerWrapper< int >( "timeLimit" )
      ->setInputFlag( InputFlags::OPTIONAL )
      ->setDescription( "The time limit of the benchmark." );

    run->registerWrapper< std::string >( "args" )
      ->setInputFlag( InputFlags::OPTIONAL )
      ->setDescription( "Any extra command line arguments to pass to GEOSX." );

    run->registerWrapper< std::string >( "autoPartition" )
      ->setInputFlag( InputFlags::OPTIONAL )
      ->setDescription(
        "May be 'Off' or 'On', if 'On' partitioning arguments are created "
        "automatically. Default is Off." );

    run->registerWrapper< array1d< int > >( "strongScaling" )
      ->setInputFlag( InputFlags::OPTIONAL )
      ->setDescription(
        "Repeat the benchmark N times, scaling the number of nodes in the "
        "benchmark by these values." );
  }

  schemaUtilities::SchemaConstruction( benchmarks,
                                       schemaRoot,
                                       targetChoiceNode,
                                       documentationType );
}
...
ProblemManager::ParseInputFile()
{
  ...
  PyObject * pPreprocessorFunction =
    PyObject_GetAttrString( pModule, "PreprocessGEOSXML" );
  PyObject * pPreprocessorInputStr = Py_BuildValue( "(s)", inputFileName.c_str() );
  PyObject * pKeywordDict =
    Py_BuildValue( "{s:s}", "schema", getenv( "GPAC_SCHEMA" ) );
  PyObject * pPreprocessorResult =
    PyObject_Call( pPreprocessorFunction, pPreprocessorInputStr, pKeywordDict );
...
#else
  GEOSX_LOG_RANK_0(
    "GEOS must be configured to use Python to use parameters, symbolic math, "
    "etc. in input files" );
#endif
  ...
  xmlDocument.append_child( xmlWrapper::filePathString )
    .append_attribute( xmlWrapper::filePathString ) = path.c_str();
...
void
ProblemManager::PostProcessInput()
{
  DomainPartition * domain = getDomainPartition();

  Group const * commandLine = GetGroup< Group >( groupKeys.commandLine );
  integer const & xparCL =
    commandLine->getReference< integer >( viewKeys.xPartitionsOverride );
  integer const & yparCL =
    commandLine->getReference< integer >( viewKeys.yPartitionsOverride );
  integer const & zparCL =
    commandLine->getReference< integer >( viewKeys.zPartitionsOverride );

  integer const & suppressPinned =
    commandLine->getReference< integer >( viewKeys.suppressPinned );
...
void
ProblemManager::InitializationOrder( string_array & order )
{
...
ProblemManager::GenerateMesh()
{
  ...
  for( localIndex a = 0; a < meshBodies->numSubGroups(); ++a )
  {
    ...
    for( localIndex b = 0; b < meshBody->numSubGroups(); ++b )
    {
      ...
      GeometricObjectManager * geometricObjects =
  ...
  integer const & useNonblockingMPI =
    commandLine->getReference< integer >( viewKeys.useNonblockingMPI );
...
ProblemManager::ApplyNumericalMethods()
{
  ...
  map< std::pair< string, string >, localIndex > const regionQuadrature =
    calculateRegionQuadrature( *meshBodies );

  setRegionQuadrature( *meshBodies, *constitutiveManager, regionQuadrature );
}

map< std::pair< string, string >, localIndex >
ProblemManager::calculateRegionQuadrature( Group & meshBodies )
{
  NumericalMethodsManager const * const numericalMethodManager =
    GetGroup< NumericalMethodsManager >( groupKeys.numericalMethodsManager.Key() );

  map< std::pair< string, string >, localIndex > regionQuadrature;

  for( localIndex solverIndex = 0;
       solverIndex < m_physicsSolverManager->numSubGroups();
       ++solverIndex )
  {
    SolverBase const * const solver =
      m_physicsSolverManager->GetGroup< SolverBase >( solverIndex );
          ...
          for( auto const & regionName : targetRegions )
          {
            ...
            if( feDiscretization != nullptr )
            {
              elemRegion->forElementSubRegions< CellElementSubRegion,
                ...
                [&]( auto & subRegion ) {
                  string const elementTypeString =
                    subRegion.GetElementTypeString();

                  std::unique_ptr< finiteElement::FiniteElementBase > newFE =
                    feDiscretization->factory( elementTypeString );

                  finiteElement::FiniteElementBase & fe =
                    subRegion
                      .template registerWrapper< finiteElement::FiniteElementBase >(
                        discretizationName,
                        std::move( newFE ) )
                  ...
                  finiteElement::dispatch3D( fe, [&]( auto & finiteElement ) {
                    ...
                    feDiscretization->CalculateShapeFunctionGradients(
                      X,
                      &subRegion,
                      finiteElement );

                    localIndex & numQuadraturePointsInList =
                      regionQuadrature[std::make_pair( regionName,
                                                       subRegion.getName() )];

                    numQuadraturePointsInList =
            ...
            else  //if( fvFluxApprox != nullptr )
            {
              elemRegion->forElementSubRegions( [&]( auto & subRegion ) {
                localIndex & numQuadraturePointsInList =
                  regionQuadrature[std::make_pair( regionName, subRegion.getName() )];
...
void
ProblemManager::setRegionQuadrature(
  Group & meshBodies,
  ConstitutiveManager const & constitutiveManager,
  map< std::pair< string, string >, localIndex > const & regionQuadrature )
{
  for( localIndex a = 0; a < meshBodies.GetSubGroups().size(); ++a )
  {
    ...
    for( localIndex b = 0; b < meshBody->numSubGroups(); ++b )
    {
      ...
      elemManager->forElementSubRegionsComplete(
        [&]( localIndex const,
             localIndex const,
          ...
          TYPEOFREF( regionQuadrature )::const_iterator rqIter =
            regionQuadrature.find( std::make_pair( regionName, subRegionName ) );
          if( rqIter != regionQuadrature.end() )
          {
            localIndex const quadratureSize = rqIter->second;
            for( auto & materialName : materialList )
            {
              constitutiveManager.HangConstitutiveRelation( materialName,
                                                            &elemSubRegion,
                                                            quadratureSize );
              GEOSX_LOG_RANK_0( "  " << regionName << "/" << subRegionName << "/"
                                     << materialName << " is allocated with "
                                     << quadratureSize << " quadrature points." );
            }
          }
          else
          {
            GEOSX_LOG_RANK_0(
              "  " << regionName << "/" << subRegionName
                   << ") does not have a discretization associated with it." );
          }
        } );
    }
  }
...
ProblemManager::getDomainPartition() const
{
  return GetGroup< DomainPartition >( keys::domain );
}
...
ProblemManager::ReadRestartOverwrite()
{
  ...
  this->postRestartInitializationRecursive(
    GetGroup< DomainPartition >( keys::domain ) );
}
