...
public:
  ///Default constructor
  Table() : m_set( false ), m_zeroGradient( false ), m_interpolation( TableInterpolation::linear ), m_size( 0 ), m_p(), m_mult( T_dim ),
    m_x( T_dim )
  {
  }
...
private:
  ...
  template<class ARRAY>
  T Lookup( const ARRAY & xx, const unsigned int dim, unsigned int & i,
            const TableInterpolation::Order interpolate = TableInterpolation::linear ) const
  {
