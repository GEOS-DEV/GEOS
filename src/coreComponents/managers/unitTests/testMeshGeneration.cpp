  ...
  for ( localIndex i = 0; i < numNodesInX; ++i )
  {
    for ( localIndex j = 0; j < numNodesInY; ++j )
    {
      for ( localIndex k = 0; k < numNodesInZ; ++k )
      {
        ...
        if ( k < numElemsInZ )
        {
          if ( i < numElemsInX && j < numElemsInY ) expectedElems.push_back( elemID );
          if ( i > 0 && j < numElemsInY ) expectedElems.push_back( elemID - elem_dI );
          if ( i > 0 && j > 0 ) expectedElems.push_back( elemID - elem_dI - elem_dJ );
          if ( i < numElemsInX && j > 0 ) expectedElems.push_back( elemID - elem_dJ );
        }

        if ( k > 0 )
        {
          if ( i < numElemsInX && j < numElemsInY ) expectedElems.push_back( elemID - 1 );
          if ( i > 0 && j < numElemsInY ) expectedElems.push_back( elemID - elem_dI - 1 );
          if ( i > 0 && j > 0 ) expectedElems.push_back( elemID - elem_dI - elem_dJ - 1 );
          if ( i < numElemsInX && j > 0 ) expectedElems.push_back( elemID - elem_dJ - 1 );
        }
