...
TEST(FieldSpecification, Recursive)
{
  ...
  ElementRegionBase* reg0 = elemManager->CreateChild("ElementRegion","reg0")->group_cast<ElementRegionBase*>();
  reg0->AddCellBlockName(reg0Hex->getName());
  reg0->AddCellBlockName(reg0Tet->getName());
  ElementRegionBase* reg1 = elemManager->CreateChild("ElementRegion","reg1")->group_cast<ElementRegionBase*>();
