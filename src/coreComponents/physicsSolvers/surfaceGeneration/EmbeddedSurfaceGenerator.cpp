...
void EmbeddedSurfaceGenerator::InitializePostSubGroups(Group *const problemManager)
{
  ...
  ElementRegionManager::ElementViewAccessor<arrayView1d<integer const>> const
    &cellElemGhostRank = elemManager->ConstructArrayViewAccessor<integer, 1>(
...
void EmbeddedSurfaceGenerator::InitializePostInitialConditions_PreSubGroups(
  Group *const GEOSX_UNUSED_PARAM(problemManager))
{
  // I don't think there is  much to do here.
}

void EmbeddedSurfaceGenerator::postRestartInitialization(
...
real64 EmbeddedSurfaceGenerator::SolverStep(
