...
/**
 * @file AggregationSinglePhaseFlow.cpp
 */

#include "AggregationSinglePhaseFlow.hpp"
...
AggregationSinglePhaseFlow::AggregationSinglePhaseFlow( const std::string& name,
                                  ManagedGroup * const parent ):
  SinglePhaseFlow(name, parent)
{
  m_numDofPerCell = 1;
...
void AggregationSinglePhaseFlow::RegisterDataOnMesh(ManagedGroup * const MeshBodies)
{
...
void AggregationSinglePhaseFlow::InitializePreSubGroups(ManagedGroup * const rootGroup)
{
...
void AggregationSinglePhaseFlow::UpdateConstitutiveModels(DomainPartition * const domain)
{
...
void AggregationSinglePhaseFlow::InitializePostInitialConditions_PreSubGroups( ManagedGroup * const rootGroup )
{
...
void AggregationSinglePhaseFlow::ImplicitStepSetup( real64 const& time_n,
                                         real64 const& dt,
...
void AggregationSinglePhaseFlow::ImplicitStepComplete( real64 const & time_n,
                                            real64 const & dt,
                                            DomainPartition * const domain )
{
...
void AggregationSinglePhaseFlow::SetNumRowsAndTrilinosIndices( MeshLevel * const meshLevel,
...
void AggregationSinglePhaseFlow::SetupSystem ( DomainPartition * const domain,
                                         EpetraBlockSystem * const blockSystem )
{
  ...
  std::map<string, string_array > fieldNames;
  fieldNames["elems"].push_back(viewKeysAggregationSinglePhaseFlow.blockLocalDofNumber.Key());
  CommunicationTools::
...
void AggregationSinglePhaseFlow::SetSparsityPattern( DomainPartition const * const domain,
...
void AggregationSinglePhaseFlow::AssembleSystem( DomainPartition * const domain,
                                      EpetraBlockSystem * const blockSystem,
                                      real64 const time_n,
                                      real64 const dt )
{
  ...
  if( verboseLevel() >= 2 )
  {
    GEOS_LOG_RANK("After AggregationSinglePhaseFlow::AssembleSystem");
    GEOS_LOG_RANK("\nJacobian:\n" << *jacobian);
    GEOS_LOG_RANK("\nResidual:\n" << *residual);
  }
}
...
template< bool ISPORO >
void AggregationSinglePhaseFlow::AssembleAccumulationTerms( DomainPartition const * const domain,
...
void AggregationSinglePhaseFlow::AssembleFluxTerms( DomainPartition const * const domain,
                                         Epetra_FECrsMatrix * const jacobian,
                                         Epetra_FEVector * const residual,
                                         real64 const time_n,
                                         real64 const dt )
...
void AggregationSinglePhaseFlow::ApplyBoundaryConditions( DomainPartition * const domain,
                                               EpetraBlockSystem * const blockSystem,
                                               real64 const time_n,
                                               real64 const dt )
{
  ...
  if (verboseLevel() >= 2)
  {
    ...
    GEOS_LOG_RANK( "After AggregationSinglePhaseFlow::ApplyBoundaryConditions" );
    GEOS_LOG_RANK( "\nJacobian\n" << *jacobian );
    GEOS_LOG_RANK( "\nResidual\n" << *residual );
  }
...
void AggregationSinglePhaseFlow::ApplyDirichletBC_implicit( DomainPartition * domain,
...
void AggregationSinglePhaseFlow::ApplyFaceDirichletBC_implicit(DomainPartition * domain,
                                                    real64 const time_n, real64 const dt,
                                                    EpetraBlockSystem * const blockSystem)
{
  FieldSpecificationManager * fsManager = FieldSpecificationManager::get();
...
void AggregationSinglePhaseFlow::ApplySystemSolution( EpetraBlockSystem const * const blockSystem,
...
void AggregationSinglePhaseFlow::SolveSystem( EpetraBlockSystem * const blockSystem,
                                        SystemSolverParameters const * const params )
{
  ...
  if( verboseLevel() >= 2 )
  {
    GEOS_LOG_RANK("After AggregationSinglePhaseFlow::SolveSystem");
    GEOS_LOG_RANK("\nsolution\n" << *solution);
  }
}

void AggregationSinglePhaseFlow::ResetStateToBeginningOfStep( DomainPartition * const domain )
{
  MeshLevel * const mesh = domain->getMeshBodies()->GetGroup<MeshBody>(0)->getMeshLevel(0);
  ElementRegionManager * const elemManager = mesh->getElemManager();
...
void AggregationSinglePhaseFlow::ResetViews(DomainPartition * const domain)
{
  ...
  if (m_poroElasticFlag)
  {
    ...
    m_bulkModulus = elemManager->ConstructMaterialViewAccessor<array2d<real64>, arrayView2d<real64> >( "BulkModulus",
                                                                                                       constitutiveManager );
...
REGISTER_CATALOG_ENTRY( SolverBase, AggregationSinglePhaseFlow, std::string const &, ManagedGroup * const )
} /* namespace geosx */
