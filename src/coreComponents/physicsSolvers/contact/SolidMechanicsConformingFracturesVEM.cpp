...
void SolidMechanicsConformingFracturesVEM::registerDataOnMesh( Group & meshBodies )
{
...
void SolidMechanicsConformingFracturesVEM::setConstitutiveNames( ElementSubRegionBase & subRegion ) const
{
...
void SolidMechanicsConformingFracturesVEM::initializePreSubGroups()
{
...
void SolidMechanicsConformingFracturesVEM::setupSystem( DomainPartition & domain,
...
void SolidMechanicsConformingFracturesVEM::implicitStepSetup( real64 const & time_n,
                                                 real64 const & dt,
...
void SolidMechanicsConformingFracturesVEM::implicitStepComplete( real64 const & time_n,
                                                    real64 const & dt,
                                                    DomainPartition & domain )
{
...
void SolidMechanicsConformingFracturesVEM::postProcessInput()
{
  m_solidSolver = &this->getParent().getGroup< SolidMechanicsLagrangianFEM >( m_solidSolverName );
  SolverBase::postProcessInput();
}

SolidMechanicsConformingFracturesVEM::~SolidMechanicsConformingFracturesVEM()
{
  // TODO Auto-generated destructor stub
}

void SolidMechanicsConformingFracturesVEM::computeTolerances( DomainPartition & domain ) const
{
  GEOS_MARK_FUNCTION;
...
void SolidMechanicsConformingFracturesVEM::resetStateToBeginningOfStep( DomainPartition & domain )
{
...
void SolidMechanicsConformingFracturesVEM::computeFaceDisplacementJump( DomainPartition & domain )
{
  forDiscretizationOnMeshTargets( domain.getMeshBodies(), [&] ( string const &,
                                                                MeshLevel & mesh,
                                                                arrayView1d< string const > const & regionNames )
...
void SolidMechanicsConformingFracturesVEM::setupDofs( DomainPartition const & domain,
...
void SolidMechanicsConformingFracturesVEM::assembleSystem( real64 const time,
                                              real64 const dt,
                                              DomainPartition & domain,
...
real64 SolidMechanicsConformingFracturesVEM::calculateResidualNorm( real64 const & GEOS_UNUSED_PARAM( time ),
...
void SolidMechanicsConformingFracturesVEM::createPreconditioner( DomainPartition const & domain )
{
  if( m_linearSolverParameters.get().preconditionerType == LinearSolverParameters::PreconditionerType::block )
  {
    ...
    {
      GEOS_ERROR( "SolidMechanicsConformingFracturesVEM::CreatePreconditioner leadingBlockApproximation option " << leadingBlockApproximation << " not supported" );
    }
...
void SolidMechanicsConformingFracturesVEM::computeRotationMatrices( DomainPartition & domain ) const
{
  GEOS_MARK_FUNCTION;
  forDiscretizationOnMeshTargets( domain.getMeshBodies(), [&] ( string const &,
                                                                MeshLevel & mesh,
                                                    ...
                                                    array1d< real64 > & nodalArea ) const
{
  ...
  {
    GEOS_ERROR( "SolidMechanicsConformingFracturesVEM: face with " << numNodesPerFace << " nodes. Only triangles and quadrilaterals are supported." );
  }
}

void SolidMechanicsConformingFracturesVEM::
...
void SolidMechanicsConformingFracturesVEM::assembleStabilization( MeshLevel const & mesh,
...
void SolidMechanicsConformingFracturesVEM::updateState( DomainPartition & domain )
{
  computeFaceDisplacementJump( domain );
}

bool SolidMechanicsConformingFracturesVEM::resetConfigurationToDefault( DomainPartition & domain ) const
{
  GEOS_MARK_FUNCTION;
...
bool SolidMechanicsConformingFracturesVEM::updateConfiguration( DomainPartition & domain )
{
  GEOS_MARK_FUNCTION;
...
bool SolidMechanicsConformingFracturesVEM::isFractureAllInStickCondition( DomainPartition const & domain ) const
{
...
real64 SolidMechanicsConformingFracturesVEM::setNextDt( real64 const & currentDt,
                                           DomainPartition & domain )
{
...
REGISTER_CATALOG_ENTRY( SolverBase, SolidMechanicsConformingFracturesVEM, string const &, Group * const )
