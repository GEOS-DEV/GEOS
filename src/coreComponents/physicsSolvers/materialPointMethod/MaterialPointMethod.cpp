...
/**
 * @file MaterialPointMethod.cpp
 */

// Source includes
#include "MaterialPointMethod.hpp"
...
MaterialPointMethod::MaterialPointMethod( const string & name,
		Group * const parent ):
...
//  m_effectiveStress( 0 )
{
	m_sendOrReceiveNodes.setName( "MaterialPointMethod::m_sendOrReceiveNodes" );
	m_nonSendOrReceiveNodes.setName( "MaterialPointMethod::m_nonSendOrReceiveNodes" );
	m_targetNodes.setName( "MaterialPointMethod::m_targetNodes" );
...
void MaterialPointMethod::postProcessInput()
{
...
MaterialPointMethod::~MaterialPointMethod()
{
	// TODO Auto-generated destructor stub
}


void MaterialPointMethod::registerDataOnMesh( Group & meshBodies )
{
...
//void MaterialPointMethod::initializePreSubGroups()
//{
...
//real64 MaterialPointMethod::explicitKernelDispatch( PARAMS && ... params )
//{
//	GEOSX_MARK_FUNCTION;
//	real64 rval = 0;
//	//  if( m_strainTheory==0 )
...
void MaterialPointMethod::updateIntrinsicNodalData( DomainPartition * const domain )
{
	GEOSX_MARK_FUNCTION;
...
void MaterialPointMethod::initializePostInitialConditionsPreSubGroups()
{
	DomainPartition & domain = getGlobalState().getProblemManager().getDomainPartition();
	MeshLevel & mesh = domain.getMeshBody( 0 ).getMeshLevel( 0 );
	...
	mass[0] += 0.0;  // TODO just to fix unused error.
...
//	std::set< localIndex > tmpNonSendOrReceiveNodes;
	...
	//        "MaterialPointMethod::m_elemsAttachedToSendOrReceiveNodes["
	//        + std::to_string( er ) + "][" + std::to_string( esr ) + "]" );
	//
	//      elemsNotAttachedToSendOrReceiveNodes.setName(
	//        "MaterialPointMethod::m_elemsNotAttachedToSendOrReceiveNodes["
	//        + std::to_string( er ) + "][" + std::to_string( esr ) + "]" );
	//
	//      arrayView2d< real64 const > const & detJ = elementSubRegion.detJ();
	//      arrayView2d< localIndex const, cells::NODE_MAP_USD > const & elemsToNodes = elementSubRegion.nodeList();
...
real64 MaterialPointMethod::solverStep( real64 const & time_n,
		real64 const & dt,
...
real64 MaterialPointMethod::explicitStep( real64 const & time_n,
		real64 const & dt,
...
#define USE_PHYSICS_LOOP
	...
	//  MaterialPointMethodKernels::velocityUpdate( acc, vel, dt/2 );
	//
	...
	//  MaterialPointMethodKernels::displacementUpdate( vel, uhat, u, dt );
	//
	...
	//  MaterialPointMethodKernels::velocityUpdate( acc, mass, vel, dt / 2, m_sendOrReceiveNodes.toViewConst() );
	//
	//  fsManager.applyFieldValue< parallelDevicePolicy< 1024 > >( time_n, domain, "nodeManager", keys::Velocity );
	//
	...
	//
	//  // apply this over a set
	//  MaterialPointMethodKernels::velocityUpdate( acc, mass, vel, dt / 2, m_nonSendOrReceiveNodes.toViewConst() );
	//
	//  fsManager.applyFieldValue< parallelDevicePolicy< 1024 > >( time_n, domain, "nodeManager", keys::Velocity );
	//
...
REGISTER_CATALOG_ENTRY( SolverBase, MaterialPointMethod, string const &, dataRepository::Group * const )
}
