...
struct RegionStatisticsKernel
{
  struct RegionStatistics
  {
    ...
    RegionStatistics( integer regionCount, integer phaseCount, integer componentCount )
      : m_phaseCount( phaseCount ),
      m_componentCount( componentCount ),
      m_properties( TYPE_END, regionCount, getSize( phaseCount, componentCount )),
      m_elementCounts( regionCount )
    {
      LvArray::forValuesInSlice( m_properties.toSlice(), []( real64 & v ){ v = 0.0; } );
      LvArray::forValuesInSlice( m_elementCounts.toSlice(), []( localIndex & i ){ i = 0; } );
      ...
      LvArray::forValuesInSlice( m_properties[MINIMUM], []( real64 & v ){ v = maxValue; } );
      LvArray::forValuesInSlice( m_properties[MAXIMUM], []( real64 & v ){ v = minValue; } );
    }

    real64 & getProperty( integer region, integer type, integer property, integer phase = 0, integer component = 0 )
    {
      return m_properties( type, region, calculateIndex( property, phase, component ));
    }

    real64 getProperty( integer region, integer type, integer property, integer phase = 0, integer component = 0 ) const
    {
      return m_properties( type, region, calculateIndex( property, phase, component ));
    }
    ...
    arrayView3d< real64 > toView() const
    {
      return m_properties.toView();
    }

    arrayView1d< localIndex > getElementCounts() const
    {
      return m_elementCounts.toView();
    }
...
private:
    ...
    integer calculateIndex( integer property, integer phase, integer component ) const
    {
      return calculateIndex( m_phaseCount, m_componentCount, property, phase, component );
    }
    ...
    integer const m_phaseCount{0};
    integer const m_componentCount{0};
    array3d< real64 > m_properties;
    array1d< localIndex > m_elementCounts;
  };
