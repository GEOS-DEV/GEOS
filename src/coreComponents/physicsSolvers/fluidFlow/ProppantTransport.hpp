...
public:
   ...
   */
  ProppantTransport( const std::string & name, Group * const parent );
  ...
  ProppantTransport &
  operator=( ProppantTransport const & ) = delete;

  /// deleted move operator
  ProppantTransport &
  operator=( ProppantTransport && ) = delete;
  ...
  void
  ResizeFractureFields( MeshLevel & mesh );

  arrayView1d< string const > const &
  proppantModelNames() const
  {
    return m_proppantModelNames;
  }
  ...
  struct viewKeyStruct : FlowSolverBase::viewKeyStruct
  {
    ...
    static constexpr auto deltaProppantConcentrationString =
      "deltaProppantConcentration";
    static constexpr auto componentConcentrationString =
      "componentConcentration";
    static constexpr auto deltaComponentConcentrationString =
      "deltaComponentConcentration";
    static constexpr auto bcComponentConcentrationString =
    ...
    static constexpr auto proppantPackVolumeFractionString =
      "proppantPackVolumeFraction";
    static constexpr auto proppantExcessPackVolumeString =
  ...
  viewKeys()
  {
    return viewKeysProppantTransport;
  }
  viewKeyStruct const &
  viewKeys() const
  {
  ...
  groupKeys()
  {
    return groupKeysProppantTransport;
  }
  ...
  static constexpr localIndex MAX_NUM_COMPONENTS =
    constitutive::ParticleFluidBase::MAX_NUM_COMPONENTS;
  ...
  void
  UpdateProppantMobility( Group & dataGroup );

  /**
   * @brief Function to update proppant pack volume fraction
   */
  void
  UpdateProppantPackVolume( real64 const time_n,
                            real64 const dt,
                            DomainPartition & domain );
...
private:
   ...
   */
  void
  ResetViews( MeshLevel & mesh ) override;
   ...
   */
  void
  UpdateFluidModel( Group & dataGroup, localIndex const targetIndex );

  void
  UpdateComponentDensity( Group & dataGroup, localIndex const targetIndex );

  void
  UpdateProppantModel( Group & dataGroup, localIndex const targetIndex );

  /**
   * @brief Function to update cell-based fluid flux
   */
  void
  UpdateCellBasedFlux( real64 const time_n, DomainPartition & domain );
   ...
   */
  void
  UpdateState( Group & dataGroup, localIndex const targetIndex );
  ...
  ElementRegionManager::ElementViewAccessor< arrayView1d< integer const > >
