...
/**
 * @file DLFlowSolverBase.cpp
 */

#include "DLFlowSolverBase.hpp"
...
void DLFlowSolverBase::registerDataOnMesh( Group & meshBodies )
{
  DLPhysicsSolverBase::registerDataOnMesh( meshBodies );
...
void DLFlowSolverBase::saveConvergedState( ElementSubRegionBase & subRegion ) const
{
...
void DLFlowSolverBase::saveSequentialIterationState( DomainPartition & domain )
{
...
void DLFlowSolverBase::setConstitutiveNamesCallSuper( ElementSubRegionBase & subRegion ) const
{
  DLPhysicsSolverBase::setConstitutiveNamesCallSuper( subRegion );
...
void DLFlowSolverBase::setConstitutiveNames( ElementSubRegionBase & subRegion ) const
{
  GEOS_UNUSED_VAR( subRegion );
}

void DLFlowSolverBase::initializePreSubGroups()
{
  DLPhysicsSolverBase::initializePreSubGroups();
...
void DLFlowSolverBase::checkDiscretizationName() const
{
...
void DLFlowSolverBase::validatePoreVolumes( DomainPartition const & domain ) const
{
...
void DLFlowSolverBase::initializePostInitialConditionsPreSubGroups()
{
  DLPhysicsSolverBase::initializePostInitialConditionsPreSubGroups();
...
void DLFlowSolverBase::initializeState( DomainPartition & domain )
{
...
void DLFlowSolverBase::initializePorosityAndPermeability( MeshLevel & mesh, string_array const & regionNames )
{
...
void DLFlowSolverBase::initializeHydraulicAperture( MeshLevel & mesh, string_array const & regionNames )
{
...
void DLFlowSolverBase::saveInitialPressureAndTemperature( MeshLevel & mesh, string_array const & regionNames )
{
...
void DLFlowSolverBase::updatePorosityAndPermeability( CellElementSubRegion & subRegion ) const
{
  GEOS_MARK_FUNCTION;
...
void DLFlowSolverBase::updatePorosityAndPermeability( SurfaceElementSubRegion & subRegion ) const
{
  GEOS_MARK_FUNCTION;
                                           ...
                                           DomainPartition & domain )
{
  ...
  for( int i = lowerbound; i < upperbound; i++ )
  {
    ...
    m_shared_metadata->m_shared_dofZCoords[i] = m_dofZCoords_values[i-lowerbound];
  ...
  if( MpiWrapper::commRank( MPI_COMM_GEOS ) == 0 )
  {
    ...
    m_shared_metadata->m_t_time_n = time_n;
    m_shared_metadata->m_t_stepDt = dt;
    m_shared_metadata->m_t_step_no = cycleNumber;
    // print testing shared data
    std::cout << "DLFlowSolverBase::shareDLModelInputs " << cycleNumber << " started" << std::endl;
  }
                                           ...
                                           DomainPartition & domain )
{
  ...
  // Wait for Python to signal that output array is ready (only rank 0 will wait)
  if( MpiWrapper::commRank( MPI_COMM_GEOS ) == 0 )
  {
  ...
  if( MpiWrapper::commRank( MPI_COMM_GEOS ) == 0 )
  {
    ...
    std::cout << "m_t_time_n: " << m_shared_metadata->m_t_time_n << std::endl;
    std::cout << "m_t_stepDt: " << m_shared_metadata->m_t_stepDt << std::endl;
...
void DLFlowSolverBase::prepareStencilWeights( DomainPartition & domain ) const
{
  forDiscretizationOnMeshTargets( domain.getMeshBodies(), [&] ( string const &,
                                                                MeshLevel & mesh,
                                                                string_array const & )
...
void DLFlowSolverBase::updateStencilWeights( DomainPartition & domain ) const
{
  forDiscretizationOnMeshTargets( domain.getMeshBodies(), [&] ( string const &,
                                                                MeshLevel & mesh,
                                                                string_array const & )
...
bool DLFlowSolverBase::checkSequentialSolutionIncrements( DomainPartition & GEOS_UNUSED_PARAM( domain ) ) const
{
