...
void CompositionalMultiphaseBase::registerDataOnMesh( Group & meshBodies )
{
  ...
  m_numDofPerCell = m_isothermalFlag ? m_numComponents + 1 : m_numComponents + 2;
  ...
  meshBodies.forSubGroups< MeshBody >( [&]( MeshBody & meshBody )
  {
    ...
    forTargetSubRegions( mesh, [&]( localIndex const targetIndex, ElementSubRegionBase & subRegion )
    {
      ...
      if( !m_isothermalFlag )
      {
        subRegion.registerWrapper< array2d< real64, compflow::LAYOUT_PHASE > >( viewKeyStruct::dPhaseVolumeFraction_dTemperatureString() ).
          setRestartFlags( RestartFlags::NO_WRITE ).
          reference().resizeDimension< 1 >( m_numPhases );
        ...
        subRegion.registerWrapper< array1d< real64 > >( viewKeyStruct::rockInternalEnergyOldString() ).
          setRestartFlags( RestartFlags::NO_WRITE );
      }
...
void CompositionalMultiphaseBase::updateComponentFraction( Group & dataGroup ) const
{
  ...
  < IsothermalCompositionalMultiphaseBaseKernels::ComponentFractionKernel >
    ( m_numComponents,
    dataGroup.size(),
    compDens,
    dCompDens,
                                                             ...
                                                             localIndex const targetIndex ) const
{
  ...
  if( m_isothermalFlag )
  {
    ...
    < IsothermalCompositionalMultiphaseBaseKernels::PhaseVolumeFractionKernel >
      ( m_numComponents,
      m_numPhases,
      dataGroup.size(),
      compDens,
  ...
  {
    // get thermal views
    arrayView3d< real64 const, multifluid::USD_PHASE > const & dPhaseFrac_dTemp = fluid.dPhaseFraction_dTemperature();
    ...
    < ThermalCompositionalMultiphaseBaseKernels::PhaseVolumeFractionKernel >
      ( m_numComponents,
      m_numPhases,
      dataGroup.size(),
      compDens,
...
void CompositionalMultiphaseBase::backupFields( MeshLevel & mesh ) const
{
  ...
  forTargetSubRegions( mesh, [&]( localIndex const targetIndex, ElementSubRegionBase & subRegion )
  {
    ...
    // TODO: move to constitutive model
    if( !m_isothermalFlag )
    {
      SolidBase const & solid = getConstitutiveModel< SolidBase >( subRegion, m_solidModelNames[targetIndex] );

      arrayView3d< real64 const, multifluid::USD_PHASE > const phaseInternalEnergy = fluid.phaseInternalEnergy();
      arrayView2d< real64 const > const rockInternalEnergy = solid.getInternalEnergy();
                                                             ...
                                                             arrayView1d< real64 > const & localRhs ) const
{
                            ...
                            ElementSubRegionBase const & subRegion )
  {
    ...
    {
      IsothermalCompositionalMultiphaseBaseKernels::KernelLaunchSelector1
      < IsothermalCompositionalMultiphaseBaseKernels::AccumulationKernel >
        ( m_numComponents,
        m_numPhases,
        subRegion.size(),
    ...
    else
    {
      ...
      arrayView3d< real64 const, multifluid::USD_PHASE > const & dPhaseDens_dTemp = fluid.dPhaseDensity_dTemperature();
      arrayView4d< real64 const, multifluid::USD_PHASE_COMP > const & dPhaseCompFrac_dTemp = fluid.dPhaseCompFraction_dTemperature();
      arrayView2d< real64 const, compflow::USD_PHASE > const & dPhaseVolFrac_dTemp =
        subRegion.getReference< array2d< real64, compflow::LAYOUT_PHASE > >( viewKeyStruct::dPhaseVolumeFraction_dTemperatureString() );

      arrayView3d< real64 const, multifluid::USD_PHASE > const & phaseInternalEnergy = fluid.phaseInternalEnergy();
      arrayView3d< real64 const, multifluid::USD_PHASE > const & dPhaseInternalEnergy_dPres = fluid.dPhaseInternalEnergy_dPressure();
      arrayView3d< real64 const, multifluid::USD_PHASE > const & dPhaseInternalEnergy_dTemp = fluid.dPhaseInternalEnergy_dTemperature();
      arrayView4d< real64 const, multifluid::USD_PHASE_DC > const & dPhaseInternalEnergy_dComp = fluid.dPhaseInternalEnergy_dGlobalCompFraction();
      arrayView2d< real64 const, compflow::USD_PHASE > const & phaseInternalEnergyOld =
        subRegion.getReference< array2d< real64, compflow::LAYOUT_PHASE > >( viewKeyStruct::phaseInternalEnergyOldString() );

      arrayView2d< real64 const > const & rockInternalEnergy = solid.getInternalEnergy();
      arrayView2d< real64 const > const & dRockInternalEnergy_dTemp = solid.getDinternalEnergy_dTemperature();
      ...
      < ThermalCompositionalMultiphaseBaseKernels::AccumulationKernel >
        ( m_numComponents,
        m_numPhases,
        subRegion.size(),
        dofManager.rankOffset(),
                                                              ...
                                                              arrayView1d< real64 > const & localRhs ) const
{
  ...
  forTargetSubRegions( mesh, [&]( localIndex const targetIndex, ElementSubRegionBase const & subRegion )
  {
    ...
    {
      IsothermalCompositionalMultiphaseBaseKernels::KernelLaunchSelector2
      < IsothermalCompositionalMultiphaseBaseKernels::VolumeBalanceKernel >
        ( numComponents,
        numPhases,
        subRegion.size(),
        dofManager.rankOffset(),
    ...
    else
    {
      ...
      ThermalCompositionalMultiphaseBaseKernels::KernelLaunchSelector2
      < ThermalCompositionalMultiphaseBaseKernels::VolumeBalanceKernel >
        ( numComponents,
        numPhases,
        subRegion.size(),
        dofManager.rankOffset(),
  ...
  forTargetSubRegions< CellElementSubRegion, SurfaceElementSubRegion >( mesh, [&]( localIndex const targetIndex, auto & subRegion )
  {
    ...
    if( !m_isothermalFlag )
    {
      arrayView1d< real64 > const & dTemp =
        subRegion.template getReference< array1d< real64 > >( viewKeyStruct::deltaTemperatureString() );
      dTemp.zero();
    }
...
void CompositionalMultiphaseBase::resetViews( MeshLevel & mesh )
{
...

  {
    ...
    if( !m_isothermalFlag )
    {
      ...
      m_dPhaseVolFrac_dTemp = elemManager.constructArrayViewAccessor< real64, 2, LAYOUT_PHASE >( viewKeyStruct::dPhaseVolumeFraction_dTemperatureString() );
      ...
      m_dPhaseMob_dTemp = elemManager.constructArrayViewAccessor< real64, 2, LAYOUT_PHASE >( viewKeyStruct::dPhaseMobility_dTemperatureString() );
...

  {
    ...
    if( !m_isothermalFlag )
    {
      ...
      m_dPhaseMassDens_dTemp = elemManager.constructMaterialArrayViewAccessor< real64, 3, LAYOUT_PHASE >( keys::dPhaseMassDensity_dTemperatureString(),
                                                                                                          targetRegionNames(),
                                                                                                          fluidModelNames() );
      m_dPhaseMassDens_dTemp.setName( getName() + "/accessors/" + keys::dPhaseMassDensity_dTemperatureString() );

      m_dPhaseCompFrac_dTemp.clear();
      m_dPhaseCompFrac_dTemp = elemManager.constructMaterialArrayViewAccessor< real64, 4, LAYOUT_PHASE_DC >( keys::dPhaseCompFraction_dTemperatureString(),
                                                                                                             targetRegionNames(),
                                                                                                             fluidModelNames() );
      m_dPhaseCompFrac_dTemp.setName( getName() + "/accessors/" + keys::dPhaseCompFraction_dTemperatureString() );

      m_phaseEnthalpy.clear();
      m_phaseEnthalpy = elemManager.constructMaterialArrayViewAccessor< real64, 3, LAYOUT_PHASE >( keys::phaseEnthalpyString(),
                                                                                                   targetRegionNames(),
                                                                                                   fluidModelNames() );
      m_phaseEnthalpy.setName( getName() + "/accessors/" + keys::phaseEnthalpyString() );

      m_dPhaseEnthalpy_dPres.clear();
      m_dPhaseEnthalpy_dPres = elemManager.constructMaterialArrayViewAccessor< real64, 3, LAYOUT_PHASE >( keys::dPhaseEnthalpy_dPressureString(),
                                                                                                          targetRegionNames(),
                                                                                                          fluidModelNames() );
      m_dPhaseEnthalpy_dPres.setName( getName() + "/accessors/" + keys::dPhaseEnthalpy_dPressureString() );

      m_dPhaseEnthalpy_dTemp.clear();
      m_dPhaseEnthalpy_dTemp = elemManager.constructMaterialArrayViewAccessor< real64, 3, LAYOUT_PHASE >( keys::dPhaseEnthalpy_dTemperatureString(),
                                                                                                          targetRegionNames(),
                                                                                                          fluidModelNames() );
      m_dPhaseEnthalpy_dTemp.setName( getName() + "/accessors/" + keys::dPhaseEnthalpy_dTemperatureString() );

      m_dPhaseEnthalpy_dComp.clear();
      m_dPhaseEnthalpy_dComp = elemManager.constructMaterialArrayViewAccessor< real64, 4, LAYOUT_PHASE_DC >( keys::dPhaseEnthalpy_dGlobalCompFractionString(),
                                                                                                             targetRegionNames(),
                                                                                                             fluidModelNames() );
