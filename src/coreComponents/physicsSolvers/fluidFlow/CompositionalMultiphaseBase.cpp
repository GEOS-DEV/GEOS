...
void CompositionalMultiphaseBase::initializePreSubGroups()
{
  ...
  // 4. Initialize and validate the aquifer boundary condition
...
void CompositionalMultiphaseBase::resetViews( MeshLevel & mesh )
{
...

  {
    ...
    {
      m_temperature.clear();
      m_temperature = elemManager.constructArrayViewAccessor< real64, 1 >( temperature::key() );
      m_temperature.setName( getName() + "/accessors/" + temperature::key() );

      m_deltaTemperature.clear();
      m_deltaTemperature = elemManager.constructArrayViewAccessor< real64, 1 >( deltaTemperature::key() );
      m_deltaTemperature.setName( getName() + "/accessors/" + deltaTemperature::key() );

      m_dPhaseVolFrac_dTemp.clear();
      m_dPhaseVolFrac_dTemp = elemManager.constructArrayViewAccessor< real64, 2, LAYOUT_PHASE >( dPhaseVolumeFraction_dTemperature::key() );
      m_dPhaseVolFrac_dTemp.setName( getName() + "/accessors/" + dPhaseVolumeFraction_dTemperature::key() );

      m_dPhaseMob_dTemp.clear();
      m_dPhaseMob_dTemp = elemManager.constructArrayViewAccessor< real64, 2, LAYOUT_PHASE >( dPhaseMobility_dTemperature::key() );
      m_dPhaseMob_dTemp.setName( getName() + "/accessors/" + dPhaseMobility_dTemperature::key() );
    }
  }

  {
    ...
    if( !m_isothermalFlag )
    {
      ...
      m_dPhaseMassDens_dTemp = elemManager.constructMaterialExtrinsicAccessor< dPhaseMassDensity_dTemperature >( targetRegionNames(),
                                                                                                                 fluidModelNames() );
      m_dPhaseMassDens_dTemp.setName( getName() + "/accessors/" + dPhaseMassDensity_dTemperature::key() );

      m_dPhaseCompFrac_dTemp.clear();
      m_dPhaseCompFrac_dTemp = elemManager.constructMaterialExtrinsicAccessor< dPhaseCompFraction_dTemperature >( targetRegionNames(),
                                                                                                                  fluidModelNames() );
      m_dPhaseCompFrac_dTemp.setName( getName() + "/accessors/" + dPhaseCompFraction_dTemperature::key() );

      m_phaseEnthalpy.clear();
      m_phaseEnthalpy = elemManager.constructMaterialExtrinsicAccessor< phaseEnthalpy >( targetRegionNames(),
                                                                                         fluidModelNames() );
      m_phaseEnthalpy.setName( getName() + "/accessors/" + phaseEnthalpy::key() );

      m_dPhaseEnthalpy_dPres.clear();
      m_dPhaseEnthalpy_dPres = elemManager.constructMaterialExtrinsicAccessor< dPhaseEnthalpy_dPressure >( targetRegionNames(),
                                                                                                           fluidModelNames() );
      m_dPhaseEnthalpy_dPres.setName( getName() + "/accessors/" + dPhaseEnthalpy_dPressure::key() );

      m_dPhaseEnthalpy_dTemp.clear();
      m_dPhaseEnthalpy_dTemp = elemManager.constructMaterialExtrinsicAccessor< dPhaseEnthalpy_dTemperature >( targetRegionNames(),
                                                                                                              fluidModelNames() );
      m_dPhaseEnthalpy_dTemp.setName( getName() + "/accessors/" + dPhaseEnthalpy_dTemperature::key() );

      m_dPhaseEnthalpy_dComp.clear();
      m_dPhaseEnthalpy_dComp = elemManager.constructMaterialExtrinsicAccessor< dPhaseEnthalpy_dGlobalCompFraction >( targetRegionNames(),
                                                                                                                     fluidModelNames() );
      m_dPhaseEnthalpy_dComp.setName( getName() + "/accessors/" + dPhaseEnthalpy_dGlobalCompFraction::key() );
    }
  }
  {
