...
/**
 * @file DLSinglePhaseFVM.cpp
 */

#include "DLSinglePhaseFVM.hpp"
...
template< typename BASE >
void DLSinglePhaseFVM< BASE >::initializePreSubGroups()
{
...
DLSinglePhaseFVM< BASE >::createPreconditioner( DomainPartition & domain ) const
{
                                                        ...
                                                        arrayView1d< real64 const > const & localRhs )
{
  ...
  if( m_isThermal )
  {
    ...
    GEOS_LOG_LEVEL_RANK_0_NLR( logInfo::ResidualNorm, GEOS_FMT( "        ( R{} ) = ( {:4.2e} )        ( Renergy ) = ( {:4.2e} )",
                                                                DLFlowSolverBase::coupledSolverAttributePrefix(), globalResidualNorm[0], globalResidualNorm[1] ));
    BASE::getConvergenceStats().setResidualValue( GEOS_FMT( "R{}", DLFlowSolverBase::coupledSolverAttributePrefix()), globalResidualNorm[0] );
  ...
  else
  {
    ...
    GEOS_LOG_LEVEL_RANK_0_NLR( logInfo::ResidualNorm,
                               GEOS_FMT( "        ( R{} ) = ( {:4.2e} )", DLFlowSolverBase::coupledSolverAttributePrefix(), residualNorm ));
    BASE::getConvergenceStats().setResidualValue( GEOS_FMT( "R{}", DLFlowSolverBase::coupledSolverAttributePrefix()), residualNorm );
  }
  return residualNorm;
}
                                            ...
                                            arrayView1d< real64 > const & localRhs )
{
  ...
  string const & dofKey = dofManager.getKey( DLSinglePhaseBase::viewKeyStruct::elemDofFieldString() );
...
// template<>
// void DLSinglePhaseFVM< SinglePhaseProppantBase >::assembleFluxTerms( real64 const dt,
//                                                                    DomainPartition const & domain,
//                                                                    DofManager const & dofManager,
//                                                                    CRSMatrixView< real64, globalIndex const > const & localMatrix,
//                                                                    arrayView1d< real64 > const & localRhs )
...
char const faceBcLogMessage[] =
  "DLSinglePhaseFVM {}: at time {}s, "
  "the <{}> boundary condition '{}' is applied to the face set '{}' in '{}'. "
  "\nThe total number of target faces (including ghost faces) is {}. "
  "\nNote that if this number is equal to zero, the boundary condition will not be applied on this face set.";

GEOS_MAYBE_UNUSED char const incompleteBCLogmessage[] = "DLSinglePhaseFVM {}: at time {}, one or more Face boundary conditions are not complete. "
...
// template<>
// void DLSinglePhaseFVM< SinglePhaseProppantBase >::applyAquiferBC( real64 const GEOS_UNUSED_PARAM( time ),
//                                                                 real64 const GEOS_UNUSED_PARAM( dt ),
...
namespace
{
// typedef DLSinglePhaseFVM< SinglePhaseProppantBase > SinglePhaseFVMProppant;
// REGISTER_CATALOG_ENTRY( PhysicsSolverBase, SinglePhaseFVMProppant, string const &, Group * const )
typedef DLSinglePhaseFVM<> DLSinglePhaseFVM;
REGISTER_CATALOG_ENTRY( PhysicsSolverBase, DLSinglePhaseFVM, string const &, Group * const )
}
} /* namespace geos */
