...
/**
 * @file DLSinglePhaseBase.cpp
 */

#include "DLSinglePhaseBase.hpp"
...
void DLSinglePhaseBase::registerDataOnMesh( Group & meshBodies )
{
  DLFlowSolverBase::registerDataOnMesh( meshBodies );
...
void DLSinglePhaseBase::setConstitutiveNames( ElementSubRegionBase & subRegion ) const
{
  setConstitutiveName< SingleFluidBase >( subRegion, viewKeyStruct::fluidNamesString(), "DL fluid" );

  if( m_isThermal )
  {
    setConstitutiveName< SinglePhaseThermalConductivityBase >( subRegion, viewKeyStruct::thermalConductivityNamesString(), "DL thermal conductivity" );
  }
}

void DLSinglePhaseBase::initializeAquiferBC() const
{
...
void DLSinglePhaseBase::validateConstitutiveModels( DomainPartition & domain ) const
{
  GEOS_MARK_FUNCTION;
...
void DLSinglePhaseBase::initializePreSubGroups()
{
  DLFlowSolverBase::initializePreSubGroups();
...
void DLSinglePhaseBase::updateFluidModel( ObjectManagerBase & dataGroup ) const
{
  GEOS_MARK_FUNCTION;
...
void DLSinglePhaseBase::updateMass( ElementSubRegionBase & subRegion ) const
{
  GEOS_MARK_FUNCTION;
...
void DLSinglePhaseBase::updateEnergy( ElementSubRegionBase & subRegion ) const
{
  GEOS_MARK_FUNCTION;
...
void DLSinglePhaseBase::updateSolidInternalEnergyModel( ObjectManagerBase & dataGroup ) const
{
...
void DLSinglePhaseBase::updateThermalConductivity( ElementSubRegionBase & subRegion ) const
{
...
real64 DLSinglePhaseBase::updateFluidState( ElementSubRegionBase & subRegion ) const
{
...
void DLSinglePhaseBase::updateMobility( ObjectManagerBase & dataGroup ) const
{
  GEOS_MARK_FUNCTION;
...
void DLSinglePhaseBase::initializePostInitialConditionsPreSubGroups()
{
  GEOS_MARK_FUNCTION;

  allowNegativePressure();

  DLFlowSolverBase::initializePostInitialConditionsPreSubGroups();
...
void DLSinglePhaseBase::computeHydrostaticEquilibrium( DomainPartition & domain )
{
  FieldSpecificationManager & fsManager = FieldSpecificationManager::getInstance();
...
void DLSinglePhaseBase::initializeFluidState( MeshLevel & mesh, string_array const & regionNames )
{
...
void DLSinglePhaseBase::initializeThermalState( MeshLevel & mesh, string_array const & regionNames )
{
  mesh.getElemManager().forElementSubRegions< CellElementSubRegion, SurfaceElementSubRegion >( regionNames, [&]( localIndex const,
                                                                                                                 auto & subRegion )
  {
...
void DLSinglePhaseBase::updateState( DomainPartition & domain )
{
  GEOS_MARK_FUNCTION;
...
void DLSinglePhaseBase::resetStateToBeginningOfStep( DomainPartition & domain )
{
  forDiscretizationOnMeshTargets( domain.getMeshBodies(), [&]( string const &,
                                                               MeshLevel & mesh,
                                                               string_array const & regionNames )
...
void DLSinglePhaseBase::saveConvergedState( ElementSubRegionBase & subRegion ) const
{
  DLFlowSolverBase::saveConvergedState( subRegion );
...
void DLSinglePhaseBase::applyDeltaVolume( ElementSubRegionBase & subRegion ) const
{
