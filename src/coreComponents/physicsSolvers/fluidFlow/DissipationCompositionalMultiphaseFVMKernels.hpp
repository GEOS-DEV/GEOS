                    ...
                    StackVariables & stack ) const
  {
    ...
    */
                                           ...
                                           real64 const (&dPhaseFlux_dC)[2][numComp] )
    {
      ...
      // Step 1. Compute potential gradient (only viscous for now)
      real64 potential_gradient = 0;
      for( integer ke = 0; ke < numFluxSupportPoints; ++ke ) 
      {
          potential_gradient += fluxPointCoef[ke] * m_pres_n[seri[ke]][sesri[ke]][sei[ke]];
      }
      potential_gradient = abs(potential_gradient);

      // Step 2. Compute the dissipation flux and its derivative
      for( integer ke = 0; ke < numFluxSupportPoints; ++ke ) {
          for( integer ic = 0; ic < numComp; ++ic ) {
            ...
            if (dz_config) // using z gradient
              dissFlux[ic] += m_kappaDBC * m_omegaDBC * fluxPointCoef[ke] * m_compFrac[er][esr][ei][ic] * potential_gradient;
            else // using rho_c gradeint
              dissFlux[ic] += m_kappaDBC * m_omegaDBC * fluxPointCoef[ke] * m_compDens[er][esr][ei][ic] * potential_gradient;
            ...
            for( integer jc = 0; jc < numComp; ++jc ) 
            {
              ...
              if (dz_config) // using z gradient
                dDissFlux_dC[ke][ic][jc] += m_kappaDBC * m_omegaDBC * fluxPointCoef[ke] * m_dCompFrac_dCompDens[er][esr][ei][ic][jc] * potential_gradient;
              else // using rho_c gradeint
                dDissFlux_dC[ke][ic][jc] += m_kappaDBC * m_omegaDBC * fluxPointCoef[ke] * potential_gradient;
            }
          }
      }
                   ...
                   //CoupledSolidBase const & solid)
  {
      ...
      internal::kernelLaunchSelectorCompSwitch( numComps, [&]( auto NC )
    {
      ...
      KERNEL_TYPE kernel( numPhases, rankOffset, hasCapPressure, stencilWrapper, dofNumberAccessor,
                          ...
                          dt, localMatrix, localRhs, kappa, omega );
