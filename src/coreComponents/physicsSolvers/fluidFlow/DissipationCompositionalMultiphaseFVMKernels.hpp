                    ...
                    StackVariables & stack ) const
  {
                                           ...
                                           real64 const (&dPhaseFlux_dC)[2][numComp] )
    {
      ...
      for( integer ic = 0; ic < numComp; ++ic )
      {
        ...
        stack.localFlux[eqIndex0] +=  dissFlux[ic];
        stack.localFlux[eqIndex1] += -dissFlux[ic];

        for( integer ke = 0; ke < numFluxSupportPoints; ++ke )
        {
          localIndex const localDofIndexPres = k[ke] * numDof;
          stack.localFluxJacobian[eqIndex0][localDofIndexPres] +=  dDissFlux_dP[ke][ic];
          stack.localFluxJacobian[eqIndex1][localDofIndexPres] += -dDissFlux_dP[ke][ic];
        }
      }
