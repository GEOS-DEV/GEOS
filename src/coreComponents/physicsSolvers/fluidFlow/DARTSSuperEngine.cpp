...
void DARTSSuperEngine::computeCFLNumbers( real64 const & dt,
                                          DomainPartition & domain )
{
  GEOSX_MARK_FUNCTION;
...
void DARTSSuperEngine::updatePhaseMobility( ObjectManagerBase & dataGroup, localIndex const targetIndex ) const
{
  GEOSX_MARK_FUNCTION;
...
void DARTSSuperEngine::applyAquiferBC( real64 const time,
                                       real64 const dt,
                                       DofManager const & dofManager,
                                       DomainPartition & domain,
                                       CRSMatrixView< real64, globalIndex const > const & localMatrix,
                                       arrayView1d< real64 > const & localRhs ) const
{
                                                     ...
                                                     string const & )
  {
    ...
    if( bc.getLogLevel() >= 1 && m_nonlinearSolverParameters.m_numNewtonIterations == 0 )
    {
      ...
      string const logMessage = string( "DARTSSuperEngine {}: at time {}s, " )
