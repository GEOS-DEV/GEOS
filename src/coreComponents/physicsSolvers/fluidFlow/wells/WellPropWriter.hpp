...
public:  
  ...
  {
    virtual void write_prop(const integer er, const integer esr, const integer ei,   std::ofstream& stream ) =0;
  };
  ...
  template <typename T> struct typed_1d_perfprop_writer : public perf_prop_writer  
  {
    typedef T value_type;
    typedef typed_prop_wrapper <value_type> prop_wrapper_type;

    typed_1d_perfprop_writer(value_type const &  prop):m_prop(prop){}
    value_type const    m_prop;

    virtual void write_prop(const integer er, const integer esr, const integer ei,  std::ofstream& stream )
    {
      stream << "," <<  m_prop[er][esr][ei];
    }
  };
     template <typename T> struct typed_2d_perf_prop_writer : public perf_prop_writer  
  {
    typedef T value_type;
    typedef typed_prop_wrapper <value_type> prop_wrapper_type;

    typed_2d_perf_prop_writer( value_type & prop,integer dim2):m_prop(prop),m_Dim2(dim2){}
    value_type const m_prop;
 ...
 template <typename T> struct typed_3d_prop_writer : public prop_writer  
  ...
  template <typename T> struct typed_f3d_prop_writer : public prop_writer  
  {
   ...
   template <typename T> struct typed_f3d_perf_prop_writer : public perf_prop_writer  
  {
    typedef T value_type;
    typedef typed_prop_wrapper <value_type> prop_wrapper_type;

    typed_f3d_perf_prop_writer( value_type & prop,integer dim2,integer dim3 ):m_prop(prop),m_Dim2(dim2),m_Dim3(dim3){}
    value_type  m_prop;
    const integer m_Dim2;
    const integer m_Dim3;
  ...
  void registerSegProp(std::string const &name ,const T & prop) 
  {
    ...
    template <typename T >
  void registerPerfProp(std::string const &name ,const T & prop) 
  {
    if ( m_initialized == 0 ) 
    m_perfHeader.push_back(name);
    m_perfPropWriterVec.push_back(new typed_1d_perfprop_writer(prop));
  }
    template <typename T >
  void registerPerfComponentProp(std::string const &name, const T& prop) 
  {
   ...
   m_perfPropWriterVec.push_back(new typed_2d_perf_prop_writer(prop,m_numComponent));
  }
  template <typename T >
   void registerPerfPhaseComponentProp(std::string const &name, const T & prop) 
  {
    if ( m_initialized == 0 ) 
    ...
    m_perfPropWriterVec.push_back(new typed_f3d_perf_prop_writer(prop,m_numPhase,m_numComponent));
  }
  template <typename T >
  ...
  template <typename T >
  void registerPerfPhaseProp(std::string const &name, const T& prop) 
  {
    if ( m_initialized == 0 ) 
    for(const auto& p : m_phaseNames)
    {
    ...
    m_perfPropWriterVec.push_back(new typed_2d_perf_prop_writer(prop,m_numPhase));
  }
