...
public:
  ...
  template< typename T > struct typed_1d_prop_writer : public prop_writer
  {
    ...
    template< typename T > struct typed_1d_prop_writerf : public prop_writer
  {
    typedef T value_type;
    typedef typed_prop_wrapper< value_type > prop_wrapper_type;

    typed_1d_prop_writerf( value_type & prop ): m_prop( prop ){}
    value_type m_prop;
  ...
  template< typename T > struct typed_1d_perfres_prop_writer : public perf_prop_writer
  {
    typedef T value_type;
    typedef typed_prop_wrapper< value_type > prop_wrapper_type;

    typed_1d_perfres_prop_writer( value_type const & prop ): m_prop( prop ){}
    ...
    template< typename T > struct typed_1d_perf_res_prop_writerf : public perf_prop_writer
  {
    typedef T value_type;
    typedef typed_prop_wrapper< value_type > prop_wrapper_type;

    typed_1d_perf_res_prop_writerf( value_type const & prop ): m_prop( prop ){}
    value_type const m_prop;

    virtual void write_prop( const integer g, const integer er, const integer esr, const integer ei, std::ofstream & stream )
    {
      GEOS_UNUSED_VAR( g );
      stream << "," <<  m_prop[er][esr][0][ei];
    }
  };
  template< typename T > struct typed_1d_perf_prop_writer : public perf_prop_writer
  {
    typedef T value_type;
    typedef typed_prop_wrapper< value_type > prop_wrapper_type;

    typed_1d_perf_prop_writer( value_type const & prop ): m_prop( prop ){}
    value_type const m_prop;
  ...
  template< typename T >
  void registerSegPropf( std::string const & name, const T & prop )
  {
    if( m_initialized == 0 )
      m_header.push_back( name );
    m_propWriterVec.push_back( new typed_1d_prop_writerf( prop ));
  }
  template< typename T >
  void registerPerfResProp( std::string const & name, const T & prop )
  {
    ...
    typed_1d_perfres_prop_writer<T> * writer = new typed_1d_perfres_prop_writer( prop );
    m_perfPropWriterVec.push_back(writer);
  }
    template< typename T >
  void registerPerfResPropf( std::string const & name, const T & prop )
  {
    if( m_initialized == 0 )
      m_perfHeader.push_back( name );
    m_perfPropWriterVec.push_back( new typed_1d_perf_res_prop_writerf( prop ));
  }
  template< typename T >
  ...
  template< typename T >
  void registerPerf1dProp( std::vector< std::string > const & name, const T & prop )
  {
    if( m_initialized == 0 )
      for( const auto & c : name )
      {
    ...
    m_perfPropWriterVec.push_back( new typed_1d_perf_prop_writer( prop ));
  }
  template< typename T >
  ...
  void write( real64 time, real64 dt, integer cycle, integer subevent, integer timeStep, integer newtonIter, integer numTimeStepCuts )
  {
    ...
    for( integer j =0; j < m_numPerforations; j++ )
    {
      ...
      if( m_elemGhostRank[iwelem] < 0 )
      {
        ...
        {
          // wrong not 1:1 std::cout << "Writing perf property" << m_perfHeader[cntr] << std::endl;
