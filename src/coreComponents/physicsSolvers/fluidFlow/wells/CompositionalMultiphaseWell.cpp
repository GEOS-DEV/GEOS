...
void CompositionalMultiphaseWell::updateVolRatesForConstraint( WellElementSubRegion & subRegion )
{
  ...
  // fluid data
  constitutive::MultiFluidBase & fluidSeparator =  wellControls.getMultiFluidSeparator();
  integer const isThermal = fluidSeparator.isThermal();
...
void CompositionalMultiphaseWell::chopNegativeDensities( WellElementSubRegion & subRegion )
{
  ...
  forAll< parallelDevicePolicy<> >( subRegion.size(), [=] GEOS_HOST_DEVICE ( localIndex const iwelem )
  {
    /*
       ...
       if( iwelem == 65 )
       {
        std::cout << "tjb dens " << iwelem << " " << ic << " " << wellElemCompDens[iwelem][ic] << " " << wellElemCompDens_n[iwelem][ic] <<
           std::endl;
       }
       }
     */
                                                            ...
                                                            WellElementSubRegion & subRegion )
{
  ...
  if( useEstimator )
  {
    ...
    wellControls.forSubGroups< WellConstraintBase, GROUPTYPES... >( [&]( auto & constraint )
    {
      ...
      if( limitingConstraint != nullptr && constraint.isConstraintActive() )
                                                      ...
                                                      DofManager const & dofManager )
{
  ...
  if( useEstimator )
  {
    ...
    if( wellControls.hasMinimumWHPConstraint())
    {
      std::shared_ptr< MinimumWHPConstraint > whpConstraint = wellControls.getMinWHPConstraint();
      // find limiting constraint
      //wellControls.forSubGroups< MinimumWHPConstraint >( [&]( auto & constraint )

      std::cout << "Use estimator " <<  useEstimator << " Evaluating constraint " << whpConstraint->getName() << std::endl;
      ...
      if( subRegion.isLocallyOwned() )
      {
        ...
        m_flowTable.calculateWHP( currentBHP, currentPhaseVolRate, currentWHP, flowTableSolveState );
      }
      owner = MpiWrapper::max( owner );

      MpiWrapper::broadcast( currentWHP, owner );
      std::cout << wellControls.getName() << " Solve WHP constraint at current bhp " << currentBHP << " whp " << currentWHP << " minwhp " << constraintWHP << " phase rates " << currentPhaseVolRate <<
        ...
        std::endl;
      std::shared_ptr< MinimumBHPConstraint > bhpConstraint=  wellControls.getMinimumBHPConstraintForWHP();
      bhpConstraint->setConstraintActive( false );
      if( currentWHP < constraintWHP )
      {

        std::cout << " tjb WHP constraint violated " << currentWHP << " < " << constraintWHP << " bhp " << currentBHP << " phase rates " << currentPhaseVolRate << " total vol " <<
          currentTotalVolRate <<
          std::endl;
        ...
        for( integer i=0; i <  numRates; ++i )
        {
          ...
          m_flowTable.calculateBHP( currentPhaseVolRate, currentWHP, tableBHP0,
                                    flowTableSolveState );
          std::cout << " tjb Solve at liquid rate " << liqRate0 << " whp " << constraintWHP << " bhp " << bhp0 << " table bhp " << tableBHP0<< " phase rates " << currentPhaseVolRate <<
        ...
        real64 liqRate1 = liquidRates[numRates-1];
        real64 tableBHP1;
        m_flowTable.calculateBHP( currentPhaseVolRate, currentWHP, tableBHP1,
                                  flowTableSolveState );

        std::cout << " tjb Solve at liquid rate " << liqRate1 << " whp  " << constraintWHP << " bhp " << bhp1 << " table bhp " << tableBHP1<< " phase rates " << currentPhaseVolRate <<
...
// Only subregion owner evaluates well control and control changes need to be broadcast to all ranks
      ...
      std::cout << "tjb " << constraint->getName() << " " << constraint->bottomHolePressure() << " " << constraint->phaseVolumeRates() << " " << constraint->totalVolumeRate() << " " <<
        constraint->massRate() <<
        std::endl;
    }
  }
...
void CompositionalMultiphaseWell::solveConstraint( std::shared_ptr< WellConstraintBase > constraint,
                                                   real64 const & time_n,
                                                   real64 const & dt,
                                                   integer const cycleNumber,
                                                   integer const coupledIterationNumber,
                                                   ...
                                                   DofManager const & dofManager )
{
  ...
  if( useEstimator )
  {
    ...
    {
      wellControls.setControl( static_cast< WellControls::Control >(constraint->getControl()) );     // tjb old
      wellControls.setCurrentConstraint( constraint.get() );
      // If a well is opened and then timestep is cut resulting in the well being shut, if the well is opened
// the well initialization code requires control type to by synced
      integer owner = -1;
// Only subregion owner evaluates well control and control changes need to be broadcast to all ranks
...
bool CompositionalMultiphaseWell::evaluateProductionConstraints1( real64 const & time_n,
                                                                  real64 const & dt,
                                                                  integer const cycleNumber,
                                                                  integer const coupledIterationNumber,
                                                                  DomainPartition & domain,
                                                                  ...
                                                                  DofManager const & dofManager )
{
  ...
  this->template solveConstraint< MinimumBHPConstraint >( time_n,
                                                          dt,
                                                          cycleNumber,
                                ...
                                DofManager const & dofManager )
{
  ...
  this->template solveConstraint< MaximumBHPConstraint >( time_n,
                                                          dt,
                                                          cycleNumber,
                                                          coupledIterationNumber,
                                                          domain,
                                                       ...
                                                       DofManager const & dofManager )
{
  ...
  if( useEstimator )
  {
    ...
    std::vector< std::shared_ptr< WellConstraintBase > > constraintList;
    std::shared_ptr< WellConstraintBase >  limitingConstraint = nullptr;
    if( wellControls.isProducer() )
    {
    ...
    else
    {
      ...
      limitingConstraint = wellControls.getMaxBHPConstraint();
    }
    ...
    for( auto const & constraint : constraintList )
    {
      ...
      if( constraint->isConstraintActive()  && constraint->checkViolation( *limitingConstraint, time_n ))
      {
        limitingConstraint=constraint;
        wellControls.setControl( static_cast< WellControls::Control >(constraint->getControl()) );                     // tjb old
        wellControls.setCurrentConstraint( limitingConstraint.get() );
