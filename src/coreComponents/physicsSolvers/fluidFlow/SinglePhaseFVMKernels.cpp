...
GEOSX_HOST_DEVICE
void
FluxKernel::computeTPFA( localIndex const numFluxElems,
                         arraySlice1d< localIndex const > const & stencilElementIndices,
                         real64 const transmissibility,
                         real64 (& dTrans_dPres)[2],
                         arrayView1d< real64 const > const & pres,
                         arrayView1d< real64 const > const & dPres,
                         arrayView1d< real64 const > const & gravCoef,
                         arrayView2d< real64 const > const & dens,
                         ...
                         arraySlice2d< real64 > const & fluxJacobian )
{
  ...
  // Compute flux and fill flux rval
  real64 const fluxVal = mobility * transmissibility * potDif * dt;
  flux[k[0]] += fluxVal;
  flux[k[1]] -= fluxVal;
  ...
  dFlux_dTrans[0] = mobility  * (  1 - dDensMean_dP[0] * ( gravCoef[ei[0]] - gravCoef[ei[1]] ) ) * dt;
  dFlux_dTrans[1] = mobility  * (  1 - dDensMean_dP[0] * ( gravCoef[ei[0]] - gravCoef[ei[1]] ) ) * dt;

  dFlux_dP[0] = mobility * transmissibility * (  1 - dDensMean_dP[0] * ( gravCoef[ei[0]] - gravCoef[ei[1]] ) ) * dt +
      dFlux_dTrans * dTrans_dPres[0];
  dFlux_dP[1] = mobility * transmissibility * ( -1 - dDensMean_dP[1] * ( gravCoef[ei[0]] - gravCoef[ei[1]] ) ) * dt +
      dFlux_dTrans * dTrans_dPres[1];
  dFlux_dP[k_up] += dMobility_dP * transmissibility * potDif * dt;
