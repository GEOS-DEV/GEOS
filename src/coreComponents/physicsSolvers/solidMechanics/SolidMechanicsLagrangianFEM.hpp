...
public:
   ...
   */
  void ApplyDisplacementBC_implicit(
    real64 const time,
    DofManager const& dofManager,
    DomainPartition& domain,
    CRSMatrixView<real64, globalIndex const> const& localMatrix,
  ...
  arrayView1d<string const> const& solidMaterialNames() const
  {
    return m_solidMaterialNames;
  }

  SortedArray<localIndex>& getElemsAttachedToSendOrReceiveNodes(
  ...
  SortedArray<localIndex>& getElemsNotAttachedToSendOrReceiveNodes(
    ElementSubRegionBase& subRegion)
  {
    return subRegion.getReference<SortedArray<localIndex>>(
  ...
  void setEffectiveStress(integer const input) { m_effectiveStress = input; }
...
template <typename CONSTITUTIVE_BASE,
          ...
          class KERNEL_TEMPLATE,
          typename... PARAMS>
void SolidMechanicsLagrangianFEM::AssemblyLaunch(
  DomainPartition& domain,
  DofManager const& dofManager,
  CRSMatrixView<real64, globalIndex const> const& localMatrix,
  arrayView1d<real64> const& localRhs,
