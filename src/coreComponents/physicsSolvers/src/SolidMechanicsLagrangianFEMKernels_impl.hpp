                          ...
                          localIndex nx=2, localIndex ny=2, localIndex nz=2)
{
  ...
  geosx::raja::forall_in_set<Pol>(elemList, noElem, GEOSX_LAMBDA (globalIndex k) {
      ...
      real64 uhat_local_x[NUM_NODES];
      real64 uhat_local_y[NUM_NODES];
      real64 uhat_local_z[NUM_NODES];

      real64 u_local_x[NUM_NODES];
      real64 u_local_y[NUM_NODES];
      real64 u_local_z[NUM_NODES];
      
      real64 f_local_x[NUM_NODES] = {0};
      real64 f_local_y[NUM_NODES] = {0};
      real64 f_local_z[NUM_NODES] = {0};


#if defined(STRUCTURED_GRID)
       localIndex nodeList[NUM_NODES];       
       structuredElemToNodes(nodeList,k,nx,ny,nz);
#else
       const localIndex *nodeList = (&elemsToNodes[NUM_NODES*k]);
#endif       
                                ...
                                localIndex nx=2, localIndex ny=2, localIndex nz=2)
{
  ...
  geosx::raja::forall_in_set<Pol>(elemList, noElem, GEOSX_LAMBDA (globalIndex k) {
      ...
      //Copy element to node list
       ...
       real64 dNdX_x[QUAD_PTS][NUM_NODES];
       real64 dNdX_y[QUAD_PTS][NUM_NODES];
       real64 dNdX_z[QUAD_PTS][NUM_NODES];
                                            ...
                                            localIndex nx=2, localIndex ny=2, localIndex nz=2)
{
  ...
  geosx::raja::forall_in_set<Pol>(elemList, noElem, GEOSX_LAMBDA (globalIndex k) {
       ...
       real64 uhat_local[LOCAL_DIM*NUM_NODES];
       real64 u_local[LOCAL_DIM*NUM_NODES];
       real64 f_local[LOCAL_DIM*NUM_NODES] = {0};
       ...
       //Compute shape function derivatives
       real64 dNdX[QUAD_PTS][NUM_NODES][LOCAL_DIM];

       //Compute shape function derivatives at quadrature points
#if defined(PRE_COMPUTE_P)       
       make_dNdX(nodeList, X, dNdX, P, QUAD_PTS, NUM_NODES);
#else       
       make_dNdX(nodeList, X, dNdX, QUAD_PTS, NUM_NODES);
#endif       
