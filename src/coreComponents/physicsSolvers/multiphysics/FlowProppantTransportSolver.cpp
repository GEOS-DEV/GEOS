...
FlowProppantTransportSolver::FlowProppantTransportSolver( const std::string & name,
                                                          Group * const parent ) :
  ...
  m_proppantSolver {}
{
  registerWrapper( viewKeyStruct::proppantSolverNameString, &m_proppantSolverName )
    ...
    ->setDescription(
      "Name of the proppant transport solver to use in the "
      "flowProppantTransport solver" );

  registerWrapper( viewKeyStruct::flowSolverNameString, &m_flowSolverName )
    ->setInputFlag( InputFlags::REQUIRED )
    ->setDescription(
      "Name of the flow solver to use in the flowProppantTransport solver" );
}

void
FlowProppantTransportSolver::RegisterDataOnMesh( dataRepository::Group * const )
{}

void
FlowProppantTransportSolver::PostProcessInput()
{
  SolverBase::PostProcessInput();

  m_proppantSolver =
    this->getParent()->GetGroup< ProppantTransport >( m_proppantSolverName );
  GEOSX_ERROR_IF( m_proppantSolver == nullptr,
                  "Invalid proppant solver name: " << m_proppantSolverName );

  m_flowSolver = this->getParent()->GetGroup< FlowSolverBase >( m_flowSolverName );
  GEOSX_ERROR_IF( m_flowSolver == nullptr,
                  "Invalid flow solver name: " << m_flowSolverName );
}

FlowProppantTransportSolver::~FlowProppantTransportSolver() = default;

void
FlowProppantTransportSolver::PreStepUpdate( real64 const & time_n,
                                            real64 const & dt,
                                            DomainPartition & domain )
{
  if( time_n <= 0.0 )
  {
    ...
    m_proppantSolver->ResizeFractureFields(
...
void
FlowProppantTransportSolver::PostStepUpdate( real64 const & time_n,
                                             real64 const & dt,
                                             DomainPartition & domain )
{
...
void
FlowProppantTransportSolver::ResetStateToBeginningOfStep( DomainPartition & domain )
{
...
real64
FlowProppantTransportSolver::SolverStep( real64 const & time_n,
                                         real64 const & dt,
                                         ...
                                         DomainPartition & domain )
{
  ...
  while( iter < this->m_nonlinearSolverParameters.m_maxIterNewton )
  {
    ...
    NonlinearSolverParameters const & fluidNonLinearParams =
      m_flowSolver->getNonlinearSolverParameters();
    if( fluidNonLinearParams.m_numNewtonIterations <=
          this->m_nonlinearSolverParameters.m_minIterNewton &&
        iter > 0 )
    {
    ...
    GEOSX_LOG_LEVEL_RANK_0( 1,
                            "\tIteration: " << iter + 1 << ", Proppant Solver: " );
