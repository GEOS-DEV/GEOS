...
#include "PoroelasticPhaseFieldSolver.hpp"
...
PoroelasticPhaseFieldSolver::PoroelasticPhaseFieldSolver( const string & name,
                                                    Group * const parent ):
  SolverBase( name, parent ),
  m_poroelasticSolverName(),
  m_damageSolverName(),
...
void PoroelasticPhaseFieldSolver::implicitStepComplete( real64 const & GEOSX_UNUSED_PARAM( time_n ),
                                                     real64 const & GEOSX_UNUSED_PARAM( dt ),
                                                     DomainPartition & GEOSX_UNUSED_PARAM( domain ) )
{}

void PoroelasticPhaseFieldSolver::postProcessInput()
{
...
real64 PoroelasticPhaseFieldSolver::splitOperatorStep( real64 const & time_n,
                                                    real64 const & dt,
                                                    ...
                                                    DomainPartition & domain )
{
  ...
  poroelasticSolver.setupSystem( domain,
                           poroelasticSolver.getDofManager(),
                           poroelasticSolver.getLocalMatrix(),
                           poroelasticSolver.getLocalRhs(),
			   poroelasticSolver.getLocalSolution(),
			   true );

  damageSolver.implicitStepSetup( time_n, dt, domain );

  poroelasticSolver.implicitStepSetup( time_n, dt, domain );
  ...
  while( iter < solverParams.m_maxIterNewton )
  {
    if( iter == 0 )
    {
      ...
      poroelasticSolver.resetStateToBeginningOfStep( domain );
      resetStateToBeginningOfStep( domain );
    }

    GEOSX_LOG_LEVEL_RANK_0( 1, "\tIteration: " << iter+1 << ", PoroelasticSolver: " );

    //poroelasticSolver.resetStressToBeginningOfStep( domain );
    dtReturnTemporary = poroelasticSolver.nonlinearImplicitStep( time_n,
                                                                 dtReturn,
                                                                 cycleNumber,
                                                                 domain );
    ...
    if( poroelasticSolver.getNonlinearSolverParameters().m_numNewtonIterations == 0 && iter > 0 )
    {
  ...
  GEOSX_ERROR_IF( !isConverged, "PoroelasticPhaseFieldSolver::SplitOperatorStep() did not converge" );

  damageSolver.implicitStepComplete( time_n, dt, domain );
  poroelasticSolver.implicitStepComplete( time_n, dt, domain );
  this->implicitStepComplete( time_n, dt, domain );

  return dtReturn;
}

void PoroelasticPhaseFieldSolver::mapDamageToQuadrature( DomainPartition & domain )
{
...
REGISTER_CATALOG_ENTRY( SolverBase, PoroelasticPhaseFieldSolver, string const &, Group * const )
