      ...
      for( localIndex i = 0; i < elemsToFaces.size( 1 ); ++i )
      {
        ...
        if( facesDomainBoundaryIndicator[f] == 1 && freeSurfaceFaceIndicator[f] != 1 )
        {
          ...
          for( localIndex q = 0; q < numNodesPerFace; ++q )
          {
            ...
            real32 const localIncrementy = density[e] * (velocityVp[e] * std::abs( ny ) + velocityVs[e] * sqrt( pow( nx, 2 ) + pow( nz, 2 ) ) ) * aux;
            real32 const localIncrementz = density[e] * (velocityVp[e] * std::abs( nz ) + velocityVs[e] * sqrt( pow( nx, 2 ) + pow( ny, 2 ) ) ) * aux;
