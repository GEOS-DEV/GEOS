...
/**
 * @brief Implements kernels for solving the acoustic wave equation in the PML region
 * @copydoc geosx::finiteElement::KernelBase
 * @tparam SUBREGION_TYPE The type of subregion that the kernel will act on.
 *
 * ### AcousticPMLKernel Description
 * Implements the KernelBase interface functions required for solving
 * the acoustic wave equation in the PML regions using the
 * "finite element kernel application" functions such as
 * geosx::finiteElement::RegionBasedKernelApplication.
 *
 * The number of degrees of freedom per support point for both
...
template< typename SUBREGION_TYPE,
          typename CONSTITUTIVE_TYPE,
          typename FE_TYPE >
class AcousticPML : public ExplicitAcousticSEM< SUBREGION_TYPE,
                                                CONSTITUTIVE_TYPE,
                                                FE_TYPE >
{
public:
  ...
  /// Alias for the base class;
  using Base = ExplicitAcousticSEM< SUBREGION_TYPE,
                                    CONSTITUTIVE_TYPE,
                                    FE_TYPE >;
...
//*****************************************************************************
   ...
   *   elements to be processed during this kernel launch.
   */
  AcousticPML( NodeManager & nodeManager,
               EdgeManager const & edgeManager,
               FaceManager const & faceManager,
               localIndex const targetRegionIndex,
               SUBREGION_TYPE const & elementSubRegion,
    ...
    Base( nodeManager,
          ...
          inputConstitutiveType,
          dt ),
    m_v_n( nodeManager.getExtrinsicData< extrinsicMeshData::AuxiliaryVar1PML_n >() ),
    m_v_np1( nodeManager.getExtrinsicData< extrinsicMeshData::AuxiliaryVar1PML_np1 >() ),
    m_stiffnessPMLVector( nodeManager.getExtrinsicData< extrinsicMeshData::DivAuxiliaryVar1PML >() )
  {}
...
public:
   ...
   * @copydoc geosx::finiteElement::KernelBase::quadraturePointKernel
   *
   * ### AcousticPML Description
   * Calculates additional stiffness vector for the PML region
   *
   */
  GEOSX_HOST_DEVICE
  GEOSX_FORCE_INLINE
                              ...
                              StackVariables & stack ) const
  {
    ...
    // TODO: copy input and output arrays to temporary arrays
    //FE_TYPE::gradient(gradN, m_p_n[m_elemsToNodes[k][:]], m_v_np1);
    ...
    for( localIndex i=0; i<numNodesPerElem; ++i )
    {
      ...
      for( localIndex j=0; j<numNodesPerElem; ++j )
      {
        ...
        sigmaPrime[1] = 0;
        sigmaPrime[2] = 0;
        real64 const localIncrement = ( sigma[0] * gradN[j][0] + sigmaPrime[0] * N[j] ) * m_v_n[m_elemsToNodes[k][j]][0]
                                    + ( sigma[1] * gradN[j][1] + sigmaPrime[1] * N[j] ) * m_v_n[m_elemsToNodes[k][j]][1]
                                    + ( sigma[2] * gradN[j][2] + sigmaPrime[2] * N[j] ) * m_v_n[m_elemsToNodes[k][j]][2] ;

        RAJA::atomicAdd< parallelDeviceAtomic >( &m_stiffnessPMLVector[m_elemsToNodes[k][i]], a * localIncrement );
      }
    }
  }
...
protected:
  /// The arrays containing the nodal particle velocity array (three components).
  arrayView2d< real64 const > const m_v_n;
  arrayView2d< real64 const > const m_v_np1; // this array will be used as a proxy to store the pressure gradient

  /// The array containing the product of the additional stiffness matrix and the nodal particle velocity.
  arrayView1d< real64 > const m_stiffnessPMLVector;
...
using AcousticPMLFactory = finiteElement::KernelFactory< AcousticPML,
                                                                 real64 >;
