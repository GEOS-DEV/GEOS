...
ElasticWaveEquationSEM::ElasticWaveEquationSEM( const std::string & name,
  ...
  WaveSolverBase( name,
                  parent )
{
  registerWrapper( string( viewKeyStruct::sourceConstantsString()) + "x", &m_sourceConstantsx ).
    setInputFlag( InputFlags::FALSE ).
    setSizedFromParent( 0 ).
    setDescription( "Constant part of the source for the nodes listed in m_sourceNodeIds in x-direction" );

  registerWrapper( string( viewKeyStruct::sourceConstantsString()) + "y", &m_sourceConstantsy ).
    setInputFlag( InputFlags::FALSE ).
    setSizedFromParent( 0 ).
    setDescription( "Constant part of the source for the nodes listed in m_sourceNodeIds in y-direction" );

  registerWrapper( string( viewKeyStruct::sourceConstantsString()) + "z", &m_sourceConstantsz ).
    setInputFlag( InputFlags::FALSE ).
    setSizedFromParent( 0 ).
