...
struct computeMassRhs
{
	  ...
	  arrayView1d< real32 > const rhs)
  {
    ...
    {
      real32 const cst = -forder * pow(perturbation[e], forder);
      ...
      {
        real32 const localIncrement = cst * FE_TYPE::computeMassTerm( q, xLocal ); //* p_dt2[elemsToNodes( e, q )];
 ...
 */
	  ...
	  arrayView1d< real32 const > const velocity)
  {
    ...
    forAll< EXEC_POLICY >( size, [=] GEOS_HOST_DEVICE ( localIndex const k ) 
    {
      real32 const cst = perturbation[k] * pow(velocity[k],2);
      constexpr localIndex numQuadraturePointsPerElem = FE_TYPE::numQuadraturePoints;
	...
	FE_TYPE::computeStiffnessTerm( q, xLocal, [&] ( int i, int j, real64 val )
	{
	  ...
	  RAJA::atomicAdd< parallelDeviceAtomic >( &stiffnessVectorFrechet[elemsToNodes( k, i )], localIncrement );
	  RAJA::atomicAdd< parallelDeviceAtomic >( &rhs_fp1[elemsToNodes( k, i )], localIncrement * cst );
	} );
      }
    } );
  }
