...
AcousticSecondOrderAnisotropicWaveEquationSEM::AcousticSecondOrderAnisotropicWaveEquationSEM( const std::string & name, Group * const parent )
  : WaveSolverBase( name, parent )
{
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::postInputInitialization()
{
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::registerDataOnMesh( Group & meshBodies )
{
  // First, call the parent to register its base fields
  WaveSolverBase::registerDataOnMesh( meshBodies );

  forDiscretizationOnMeshTargets( meshBodies, [&]( string const &, MeshLevel & mesh, string_array const & )
  {
    NodeManager & nodeManager = mesh.getNodeManager();

    // Register all the nodal fields that are common to ALL anisotropic solvers
    ...
    /// PML is not supported for this solver family
    if( m_usePML )
    {
    ...
    {
      // Register element-centered fields common to all anisotropic solvers
...
real32 AcousticSecondOrderAnisotropicWaveEquationSEM::getGlobalMinWavespeed( MeshLevel & mesh, string_array const & regionNames )
{
...
real64 AcousticSecondOrderAnisotropicWaveEquationSEM::explicitStepForward( real64 const & time_n,
                                                                           real64 const & dt,
                                                                           integer cycleNumber,
...
real64 AcousticSecondOrderAnisotropicWaveEquationSEM::explicitStepBackward( real64 const & GEOS_UNUSED_PARAM( time_n ),
                                                                            real64 const & GEOS_UNUSED_PARAM( dt ),
                                                                            integer GEOS_UNUSED_PARAM( cycleNumber ),
                                                                            DomainPartition & GEOS_UNUSED_PARAM( domain ),
                                                                            integer GEOS_UNUSED_PARAM( computeGradient ) )
{
  GEOS_ERROR( "Adjoint option is not supported yet for this solver family." );
  return 0.;
}


real64 AcousticSecondOrderAnisotropicWaveEquationSEM::explicitStepInternal( real64 const & time_n,
                                                                            real64 const & dt,
                                                                            DomainPartition & domain,
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::synchronizeUnknowns( real64 const & time_n,
                                                                         real64 const & dt,
                                                                         DomainPartition & domain,
                                                                         MeshLevel & mesh,
                                                                         string_array const & )
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::precomputeSourceAndReceiverTerm( MeshLevel & baseMesh, MeshLevel & mesh,
                                                                                     string_array const & regionNames )
{
  GEOS_MARK_FUNCTION;
                                                                                                ...
                                                                                                CellElementSubRegion & elementSubRegion )
  {
    GEOS_THROW_IF( elementSubRegion.getElementType() != ElementType::Hexahedron,
                   getDataContext() << ": Invalid type of element, the " << getCatalogName()
                                    << " solver is designed for hexahedral meshes only (C3D8), using the SEM formulation",
                   InputError );
    ...
    finiteElement::FiniteElementDispatchHandler< SEM_FE_TYPES >::dispatch3D( fe, [&]( auto const finiteElement )
    {
      ...
      {
        GEOS_MARK_SCOPE( "AcousticAnisotropic::PrecomputeSourceAndReceiver" );
        PreComputeSourcesAndReceivers::Compute1DSourceAndReceiverConstants< EXEC_POLICY, FE_TYPE >(
          elementSubRegion.size(), facesToNodes, nodeCoords,
          nodeLocalToGlobalMap, elemLocalToGlobalMap, nodesToElements,
          baseElemsToNodes, elemGhostRank, elemsToNodes, elemsToFaces,
          elemCenter, sourceCoordinates, sourceIsAccessible,
          sourceNodeIds, sourceConstants, receiverCoordinates,
          receiverIsLocal, receiverNodeIds, receiverConstants,
          sourceValue, dtCompute, m_timeSourceFrequency,
          m_timeSourceDelay, m_rickerOrder, sourceWaveletTableWrappers,
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::computeUnknowns( real64 const & time_n,
                                                                     real64 const & dt,
                                                                     DomainPartition & GEOS_UNUSED_PARAM( domain ),
                                                                     MeshLevel & mesh,
                                                                     string_array const & regionNames,
                                                                     bool const isForward )
{
  ...
  if( isForward )
  {
    // Use the pure virtual function to apply the subclass-specific kernel
    applyStiffnessKernels( dt, mesh, regionNames );
  }
  else
  {
    // Adjoint
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::prepareNextTimestep( MeshLevel & mesh )
{
  NodeManager & nodeManager = mesh.getNodeManager();
  arrayView1d< real32 > const p_nm1 = nodeManager.getField< acousticvtifields::Pressure_p_nm1 >();
  arrayView1d< real32 > const p_n = nodeManager.getField< acousticvtifields::Pressure_p_n >();
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::addSourceToRightHandSide( real64 const & time_n, arrayView1d< real32 > const rhs )
{
  ...
  forAll< EXEC_POLICY >( sourceConstants.size( 0 ), [=] GEOS_HOST_DEVICE ( localIndex const isrc )
  {
    ...
    {
      real64 const srcValue = useSourceWaveletTables ? sourceWaveletTableWrappers[isrc].compute( &time_n ) :
                              WaveSolverUtils::evaluateRicker( time_n, timeSourceFrequency, timeSourceDelay, rickerOrder );
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::initializePostInitialConditionsPreSubGroups()
{
  GEOS_MARK_FUNCTION;
  {
                                                               ...
                                                               string_array const & regionNames )
  {
    ...
    // Call the virtual method to initialize matrices (implemented by subclasses)
    initializeMatrices( mesh, regionNames );
  } );
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::precomputeSurfaceFieldIndicator( DomainPartition & domain )
{
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::applyFreeSurfaceBC( real64 time, DomainPartition & domain )
{
  GEOS_MARK_FUNCTION;
  FieldSpecificationManager & fsManager = FieldSpecificationManager::getInstance();
  FunctionManager const & functionManager = FunctionManager::getInstance();
...
real64 AcousticSecondOrderAnisotropicWaveEquationSEM::computeTimeStep( real64 & dtOut )
{
  GEOS_ERROR( "This option (Time Step computation) is not supported" );
  dtOut = 0.;
...
void AcousticSecondOrderAnisotropicWaveEquationSEM::applyPML( real64 const GEOS_UNUSED_PARAM( time ), DomainPartition & GEOS_UNUSED_PARAM( domain ))
{
  GEOS_ERROR( "This option is not supported yet" );
  return;
}

void AcousticSecondOrderAnisotropicWaveEquationSEM::initializePML()
{
  GEOS_ERROR( "This option (PML) is not supported" );
  return;
}
