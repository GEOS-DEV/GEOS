...
#include "AcousticSecondOrderVTIFletcher.hpp"
...
AcousticSecondOrderVTIFletcher::AcousticSecondOrderVTIFletcher( const std::string & name, Group * const parent )
  : AcousticSecondOrderAnisotropicVTISEM( name, parent )
{}

void AcousticSecondOrderVTIFletcher::registerDataOnMesh( Group & meshBodies )
{
...
void AcousticSecondOrderVTIFletcher::applyStiffnessKernels( const real64 & dt, MeshLevel & mesh, const string_array & regionNames )
{
...
void AcousticSecondOrderVTIFletcher::initializeMatrices( MeshLevel & mesh, string_array const & regionNames )
{
  ...
  {
    // Check if it's a hexahedral element
    if( elementSubRegion.getElementType() != ElementType::Hexahedron )
    {
      GEOS_LOG_RANK_0( "Skipping non-hexahedral element region: " << elementSubRegion.getName()
                                                                  << " with element type. VTI Fletcher acoustic solver only supports hexahedral elements." );
      return;
    }
...
REGISTER_CATALOG_ENTRY( PhysicsSolverBase, AcousticSecondOrderVTIFletcher, string const &, dataRepository::Group * const )
