...
public:
                              ...
                              StackVariables & stack ) const
  {
    ...
    m_finiteElementSpace.template computeRotatedStiffnessxyTerm( q, stack.tti_tilt, stack.tti_azimuth, stack.xLocal, [&] ( int i, int j, real64 val )
    {
      ...
      real32 const localIncrement_qq = -val * stack.invDensity * stack.vti_sqrtDelta * m_p_n[m_elemsToNodes( k, j )];
    ...
    {
      real32 const localIncrement_pp = -val * stack.invDensity * stack.vti_sqrtDelta * m_q_n[m_elemsToNodes( k, j )];
      stack.stiffnessVectorLocal_p[i] += localIncrement_pp;
