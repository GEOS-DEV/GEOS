...
/**
 * @file AcousticTTIZhangWaveEquationSEMKernel.hpp
 */

#ifndef GEOS_PHYSICSSOLVERS_WAVEPROPAGATION_ACOUSTICTTIZHANGWAVEEQUATIONSEMKERNEL_HPP_
#define GEOS_PHYSICSSOLVERS_WAVEPROPAGATION_ACOUSTICTTIZHANGWAVEEQUATIONSEMKERNEL_HPP_
...
/**
 * @brief Implements kernels for solving the pseudo-acoustic TTI wave equations
 *   explicit central FD method and SEM
 * @copydoc geos::finiteElement::KernelBase
 * @tparam SUBREGION_TYPE The type of subregion that the kernel will act on.
 *
 * ### AcousticTTIZhangWaveEquationSEMKernel Description
 * Implements the KernelBase interface functions required for solving
 * the TTI pseudo-acoustic wave Zhang's set of equations using the
...
//*****************************************************************************
  ...
  ExplicitAcousticTTIZhangSEM( NodeManager & nodeManager,
    ...
    m_tti_dipx( elementSubRegion.template getField< geos::fields::acousticttifields::AcousticDipX >() ),
    m_tti_dipy( elementSubRegion.template getField< geos::fields::acousticttifields::AcousticDipY >() ),
    m_tti_DofTilt( nodeManager.getField< geos::fields::acousticttifields::AcousticDofTilt >() ),
    m_tti_DofAzimuth( nodeManager.getField< geos::fields::acousticttifields::AcousticDofAzimuth >() ),
    m_dt( dt )
  {
...
public:
    ...
    real32 vti_delta_sqrtDelta_Q1[numNodesPerElem]{}; // delta / sqrt(1 + 2*delta)
    real32 vti_sqrtDelta_Q1[numNodesPerElem]{}; // sqrt(1 + 2*delta)

    real32 tti_tilt; //TTI tilt angle
    real32 tti_azimuth; //TTI azimuth angle

    real32 tti_tilt_Q1[numNodesPerElem]{};
    real32 tti_azimuth_Q1[numNodesPerElem]{};
              ...
              StackVariables & stack ) const
  {
    ...
    for( localIndex i=0; i<numNodesPerElem; i++ )
    {
      ...
      stack.vti_sqrtDelta_Q1[i] = std::sqrt( 1 + 2 *delt );
      //stack.vti_delta_sqrtDelta_Q1[i] = stack.vti_sqrtDelta_Q1[i];
      stack.vti_delta_sqrtDelta_Q1[i] = delt / stack.vti_sqrtDelta_Q1[i];
    }
  ...
  {
    // Pseudo Stiffness xy
    m_finiteElementSpace.template computeRotatedStiffnessxyTerm( q, stack.tti_tilt_Q1[q], stack.tti_azimuth_Q1[q], stack.xLocal, [&] ( int i, int j, real64 val )
    {
      ...
      real32 const localIncrement_q = -val * stack.invDensity * stack.vti_sqrtDelta_Q1[q] * m_p_n[m_elemsToNodes( k, j )];
      stack.stiffnessVectorLocal_q[i] += localIncrement_q;
    } );

    // Pseudo-Stiffness z
    m_finiteElementSpace.template computeRotatedStiffnesszTerm( q, stack.tti_tilt_Q1[q], stack.tti_azimuth_Q1[q], stack.xLocal, [&] ( int i, int j, real64 val )
    {
      real32 const localIncrement_p = -val * stack.invDensity * stack.vti_sqrtDelta_Q1[q] * m_q_n[m_elemsToNodes( k, j )];
      stack.stiffnessVectorLocal_p[i] += localIncrement_p;
    ...
    // Missing dz term
    m_finiteElementSpace.template computeRotatedMissingzVolumeTerm( q, stack.tti_tilt_Q1[q], stack.tti_azimuth_Q1[q], stack.xLocal, [&] ( int iVertice, int j, real64 val )
    {
      real32 const localIncrement_p = -val * stack.invDensity * stack.vti_delta_sqrtDelta_Q1[iVertice] * m_q_n[m_elemsToNodes( k, j )];
      stack.stiffnessVectorLocal_p[q] += localIncrement_p;
    } );

    // Missing dxy term
    m_finiteElementSpace.template computeRotatedMissingxyVolumeTerm( q, stack.tti_tilt_Q1[q], stack.tti_azimuth_Q1[q], stack.xLocal, [&] ( int iVertice, int j, real64 val )
    {
      real32 const localIncrement_p = -val * stack.invDensity * stack.vti_epsi_Q1[iVertice] * m_p_n[m_elemsToNodes( k, j )];
      stack.stiffnessVectorLocal_p[q] += localIncrement_p;

      real32 const localIncrement_q = -val * stack.invDensity * stack.vti_delta_sqrtDelta_Q1[iVertice] * m_p_n[m_elemsToNodes( k, j )];
...
protected:
  ...
  /// The array containing the tilt angle.
  arrayView1d< real32 const > const m_tti_DofTilt;
  /// The array containing the azimuth angle.
  arrayView1d< real32 const > const m_tti_DofAzimuth;
...
/// The factory used to construct a ExplicitAcousticTTIZhang kernel.
...
#endif //GEOS_PHYSICSSOLVERS_WAVEPROPAGATION_ACOUSTICTTIZHANGWAVEEQUATIONSEMKERNEL_HPP_
