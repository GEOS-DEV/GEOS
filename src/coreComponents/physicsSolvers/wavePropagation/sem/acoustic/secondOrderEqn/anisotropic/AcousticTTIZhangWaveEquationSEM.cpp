...
/**
 * @file AcousticTTIZhangWaveEquationSEM.cpp
 */

#include "AcousticTTIZhangWaveEquationSEM.hpp"
...
AcousticTTIZhangWaveEquationSEM::~AcousticTTIZhangWaveEquationSEM()
{
  // TODO Auto-generated destructor stub
}

void AcousticTTIZhangWaveEquationSEM::initializePreSubGroups()
{
  WaveSolverBase::initializePreSubGroups();
}


void AcousticTTIZhangWaveEquationSEM::registerDataOnMesh( Group & meshBodies )
{
...
void AcousticTTIZhangWaveEquationSEM::postInputInitialization()
{
...
void AcousticTTIZhangWaveEquationSEM::precomputeSourceAndReceiverTerm( MeshLevel & baseMesh, MeshLevel & mesh,
                                                                       string_array const & regionNames )
{
                                                                                                ...
                                                                                                CellElementSubRegion & elementSubRegion )
  {
    GEOS_THROW_IF( elementSubRegion.getElementType() != ElementType::Hexahedron,
                   getDataContext() << ": Invalid type of element, the acoustic TTI Zhang solver is designed for hexahedral meshes only (C3D8), using the SEM formulation",
                   InputError );
    ...
    finiteElement::FiniteElementDispatchHandler< SEM_FE_TYPES >::dispatch3D( fe, [&] ( auto const finiteElement )
    {
      ...
      {
        GEOS_MARK_SCOPE( acousticTTIZhangWaveEquationSEMKernels::PrecomputeSourceAndReceiverKernel );
...
void AcousticTTIZhangWaveEquationSEM::addSourceToRightHandSide( real64 const & time_n, arrayView1d< real32 > const rhs )
{
...
void AcousticTTIZhangWaveEquationSEM::initializePostInitialConditionsPreSubGroups()
{
  GEOS_MARK_FUNCTION;
  {
  ...
  if( m_usePML )
  {
    AcousticTTIZhangWaveEquationSEM::initializePML(); //TODO:
  }
...
real64 AcousticTTIZhangWaveEquationSEM::computeTimeStep( real64 & dtOut )
{
  // TODO: adapt to TTI
  GEOS_ERROR( "This option (Time Step computation) is not supported" );
...
void AcousticTTIZhangWaveEquationSEM::applyFreeSurfaceBC( real64 time, DomainPartition & domain )
{
  GEOS_MARK_FUNCTION;
  FieldSpecificationManager & fsManager = FieldSpecificationManager::getInstance();
  FunctionManager const & functionManager = FunctionManager::getInstance();
...
void AcousticTTIZhangWaveEquationSEM::initializePML()
{
  GEOS_ERROR( "This option (PML) is not supported" );
  return;
}
...
void AcousticTTIZhangWaveEquationSEM::applyPML( real64 const GEOS_UNUSED_PARAM( time ), DomainPartition & GEOS_UNUSED_PARAM( domain ) )
{
  GEOS_ERROR( "This option is not supported yet" );
  return;
}
...
real32 AcousticTTIZhangWaveEquationSEM::getGlobalMinWavespeed( MeshLevel & mesh, string_array const & regionNames )
{
...
void AcousticTTIZhangWaveEquationSEM::prepareNextTimestep( MeshLevel & mesh )
{
  NodeManager & nodeManager = mesh.getNodeManager();
...
void AcousticTTIZhangWaveEquationSEM::computeUnknowns( real64 const & time_n,
                                                       real64 const & dt,
                                                       DomainPartition & GEOS_UNUSED_PARAM( domain ),
                                                       MeshLevel & mesh,
                                                       string_array const & regionNames,
                                                       bool const isForward )
{
  ...
  if( !m_usePML )
  {
    ...
    //Leap Frog scheme for TTI is the same as VTI
...
void AcousticTTIZhangWaveEquationSEM::synchronizeUnknowns( real64 const & time_n,
                                                           real64 const & dt,
                                                           DomainPartition & domain,
                                                           MeshLevel & mesh,
                                                           string_array const & )
...
REGISTER_CATALOG_ENTRY( PhysicsSolverBase, AcousticTTIZhangWaveEquationSEM, string const &, dataRepository::Group * const )
