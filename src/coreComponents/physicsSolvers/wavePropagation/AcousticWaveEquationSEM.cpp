...
void AcousticWaveEquationSEM::registerDataOnMesh( Group & meshBodies )
{
                                                    ...
                                                    arrayView1d< string const > const & )
  {
    ...
    if(m_useModeq)
      {
	nodeManager.registerField< fields::ForcingRHSSecondDerivative >( this->getName() );
      }
...
// Modified Equation
void AcousticWaveEquationSEM::addSourceToRightHandSide( integer const & cycleNumber, arrayView1d< real32 > const rhs, arrayView1d< real32 > const rhsSecondDerivative )
{
  arrayView2d< localIndex const > const sourceNodeIds = m_sourceNodeIds.toViewConst();
  arrayView2d< real64 const > const sourceConstants   = m_sourceConstants.toViewConst();
  arrayView1d< localIndex const > const sourceIsAccessible = m_sourceIsAccessible.toViewConst();
