...
void AcousticROMFrechet::registerDataOnMesh( Group & meshBodies )
{
                                                    ...
                                                    arrayView1d< string const > const & )
  {
    ...
    nodeManager.registerField< fields::Pressure_nm1,
			       fields::Pressure_n,
			       fields::Pressure_np1,
			       fields::PressureFrechet_nm1,
			       fields::PressureFrechet_n,
			       fields::PressureFrechet_np1,
    ...
    //nodeManager.getField< fields::PressureFrechet_nm1 >().resizeDimension< 1 >(size);
    nodeManager.getField< fields::PressureFrechet_n >().resizeDimension< 1 >(m_orderFrechet+1);
    //nodeManager.getField< fields::PressureFrechet_n >().resizeDimension< 1 >(size);
    nodeManager.getField< fields::PressureFrechet_np1 >().resizeDimension< 1 >(m_orderFrechet+1);
    //nodeManager.getField< fields::PressureFrechet_np1 >().resizeDimension< 1 >(size);
...
void AcousticROMFrechet::postProcessInput()
{
...
void AcousticROMFrechet::precomputeSourceAndReceiverTerm( MeshLevel & mesh,
                                                               arrayView1d< string const > const & regionNames )
{
                                                                                        ...
                                                                                        CellElementSubRegion & elementSubRegion )
  {
    ...
    finiteElement::FiniteElementDispatchHandler< SEM_FE_TYPES >::dispatch3D( fe, [&] ( auto const finiteElement )
    {
      ...
      {
        GEOS_MARK_SCOPE( acousticROMFrechetKernels::PrecomputeSourceAndReceiverKernel );
                                                                                            ...
                                                                                            CellElementSubRegion & elementSubRegion )
	{
	  ...
	  forAll< EXEC_POLICY >( solverTargetNodesSet.size(), [=] GEOS_HOST_DEVICE ( localIndex const n )
	  {
	    ...
	    if( freeSurfaceNodeIndicator[a] != 1 )
	    {
	      if( f==0 )
	      {
		rhs[a] = (-p_np1[a] + 2.0*p_n[a] - p_nm1[a]) *  massFrechet[a] / dt2;
	      }
	      else
	      {
		rhs[a] = (f+1)*(-pf_np1[a][f-1] + 2.0*pf_n[a][f-1] - pf_nm1[a][f-1]) * massFrechet[a] / dt2;
	      }
                                                   ...
                                                   arrayView1d< string const > const & )
{
  ...
  fieldsToBeSync.addFields( FieldLocation::Node, { fields::PressureFrechet_np1::key() } );
						      ...
						      arrayView2d< WaveSolverBase::wsCoordType const, nodes::REFERENCE_POSITION_USD > const X)
{
  ...
  for( int j=0; j<totCount; ++j )
  {
    ...
    GEOS_THROW_IF( !wf1,
                   ...
                   InputError );
    q1V.move( MemorySpace::host, true );
    ...
    for(int iq=jf;iq>0;--iq)
    {
      ...
      GEOS_THROW_IF( !wf2,
                     ...
                     InputError );
      q2V.move( MemorySpace::host, true );
    ...
    if (rank==0)
    {
      std::cout<<"Order = "<<m_selectionOrder[0][j]<<",  Vector = "<<m_selectionOrder[1][j]<<",  Val = "<<val_all<<",  jf = "<<jf<<",  Success = "<<success<<std::endl;
    }
	...
	stiffnessVector_qV[a] = 0.0;
      ...
      q1V.move( MemorySpace::host, false );
...
REGISTER_CATALOG_ENTRY( SolverBase, AcousticROMFrechet, string const &, dataRepository::Group * const )
