...
double myFunc(double x, double y, double z) { return x; }
...
void ReactionDiffusionFEM::RegisterDataOnMesh(ManagedGroup *const MeshBodies) {
...
real64 ReactionDiffusionFEM::ExplicitStep(real64 const &time_n,
                                          real64 const &dt,
                                          const int cycleNumber,
...
void ReactionDiffusionFEM::ImplicitStepSetup(
    ...
    DofManager &dofManager, ParallelMatrix &matrix, ParallelVector &rhs,
    ParallelVector &solution) {
  // Computation of the sparsity pattern
  SetupSystem(domain, dofManager, matrix, rhs, solution);
}

void ReactionDiffusionFEM::ImplicitStepComplete(real64 const &time_n,
                                                real64 const &dt,
                                                DomainPartition *const domain) {
}

void ReactionDiffusionFEM::SetupDofs(DomainPartition const *const domain,
                                     DofManager &dofManager) const {
  dofManager.addField(m_fieldName, DofManager::Location::Node,
                      DofManager::Connectivity::Elem);
}

void ReactionDiffusionFEM::AssembleSystem(real64 const time_n, real64 const dt,
                                          DomainPartition *const domain,
          ...
          for (localIndex k = 0; k < elementSubRegion->size(); ++k) {
            if (elemGhostRank[k] < 0) {
              ...
              for (localIndex q = 0; q < n_q_points; ++q) {
                ...
                for (localIndex a = 0; a < numNodesPerElement; ++a) {
                  elemDofIndex[a] = dofIndex[elemToNodes(k, a)];
                  real64 Na = feDiscretization->m_finiteElement->value(a, q);
                  rhs(a) += detJ[k][q] * Na * myFunc(Xq, Yq, Zq);
                  real64 diffusion = 1;
