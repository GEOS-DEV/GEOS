...
LaplaceFEM::LaplaceFEM(const std::string &name, ManagedGroup *const parent)
...
real64 LaplaceFEM::SolverStep(real64 const &time_n, real64 const &dt,
...
real64 LaplaceFEM::ExplicitStep(real64 const &time_n, real64 const &dt,
                                const int cycleNumber,
                                DomainPartition *const domain) {
  return dt;
}

void LaplaceFEM::ImplicitStepSetup(real64 const &time_n, real64 const &dt,
                                   DomainPartition *const domain,
...
void LaplaceFEM::ImplicitStepComplete(real64 const &time_n, real64 const &dt,
                                      DomainPartition *const domain) {}
...
void LaplaceFEM::AssembleSystem(real64 const time_n, real64 const dt,
                                DomainPartition *const domain,
    ...
    elementRegion->forElementSubRegionsIndex<CellElementSubRegion>(
        [&](localIndex const esr,
          ...
          localIndex const numNodesPerElement =
              integer_conversion<int>(elemsToNodes.size(1));

          arrayView2d<localIndex const> const &elemNodes =
              elementSubRegion->nodeList();
            ...
            if (elemGhostRank[k] < 0) {
              ...
              for (localIndex q = 0; q < n_q_points; ++q) {
                ...
                for (localIndex a = 0; a < numNodesPerElement; ++a) {
                  ...
                  real64 diffusion = 0.40528473456; // 4/pi² for testing
...
void LaplaceFEM::ApplyBoundaryConditions(real64 const time_n, real64 const dt,
                                         DomainPartition *const domain,
                                         DofManager const &dofManager,
...
REGISTER_CATALOG_ENTRY(SolverBase, LaplaceFEM, std::string const &,
                       ManagedGroup *const)
} // namespace geosx
