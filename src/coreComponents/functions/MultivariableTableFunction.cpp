...
void MultivariableTableFunction::initializeFunctionFromFile( string const & filename )
{
  ...
  GEOSX_THROW_IF_IF( !file, catalogName() << " " << getName() << ": could not read input file " << filename, InputError );
  ...
  file >> numDims;
  GEOSX_THROW_IF_IF( !file, "Can`t read number of table dimensions", InputError );
  file >> numOps;
  GEOSX_THROW_IF_IF( !file, "Can`t read number of interpolatored operators", InputError );

  // assume no more than 10 dimensions
  GEOSX_THROW_IF_IF_LT_MSG( numDims, 1, catalogName() << " " << getName() << ": positive integer value expected", InputError );
  GEOSX_THROW_IF_IF_GT_MSG( numDims, 10, catalogName() << " " << getName() << ": maximum 10 dimensions expected", InputError );

  // assume no more than 100 operators
  GEOSX_THROW_IF_IF_LT_MSG( numOps, 1, catalogName() << " " << getName() << ": positive integer value expected", InputError );
  GEOSX_THROW_IF_IF_GT_MSG( numOps, 100, catalogName() << " " << getName() << ": maximum 100 operators expected", InputError );
  ...
  {
    file >> axisPoints[i];
    GEOSX_THROW_IF_IF( !file, catalogName() << " " << getName() << ": can`t read the number of points for axis " + std::to_string( i ), InputError );
    GEOSX_THROW_IF_IF_LE_MSG( axisPoints[i], 1, catalogName() << " " << getName() << ": minimum 2 discretization point per axis are expected", InputError );
    file >> axisMinimums[i];
    GEOSX_THROW_IF_IF( !file, catalogName() << " " << getName() << ": can`t read minimum value for axis " + std::to_string( i ), InputError );
    file >> axisMaximums[i];
    GEOSX_THROW_IF_IF( !file, catalogName() << " " << getName() << ": can`t read maximum value for axis " + std::to_string( i ), InputError );
    GEOSX_THROW_IF_IF_LT_MSG( axisMaximums[i], axisMinimums[i], catalogName() << " " << getName() << ": maximum axis value is expected to be larger than minimum", InputError );
  ...
  GEOSX_THROW_IF_IF_GT_MSG( numPointsTotal * numOps, pointStorageMemoryLimitGB * 1024 * 1024 * 1024 / 8, catalogName() << " " << getName() <<
  ...
  for( auto i = 0; i < numPointsTotal; i++ )
  {
    for( auto j = 0; j < numOps; j++ )
    {
      ...
      GEOSX_THROW_IF_IF( !file, catalogName() << " " << getName() << ": table file is shorter than expected", InputError );
    }
  }
  real64 value;

  file >> value;
  GEOSX_THROW_IF_IF( file, catalogName() << " " << getName() << ": table file is longer than expected", InputError );
...
void MultivariableTableFunction::initializeFunction()
{
  ...
  GEOSX_THROW_IF_IF_NE_MSG( m_numDims, m_axisMinimums.size(), catalogName() << " " << getName() <<
                         ": single minimum value is expected for each of " + std::to_string( m_numDims ) + "dimensions",
                         InputError );
  GEOSX_THROW_IF_IF_NE_MSG( m_numDims, m_axisMaximums.size(), catalogName() << " " << getName() <<
                         ": single maxumum value is expected for each of " + std::to_string( m_numDims ) + "dimensions",
                         InputError );
  GEOSX_THROW_IF_IF_NE_MSG( m_numDims, m_axisPoints.size(), catalogName() << " " << getName() <<
  ...
  GEOSX_THROW_IF_IF_NE_MSG( globalIndex( numTablePoints ) * m_numOps, m_pointData.size(), catalogName() << " " << getName() <<
  ...
  GEOSX_THROW_IF_IF_GT_MSG( numTableHypercubes * m_numVerts * m_numOps, hypercubeStorageMemoryLimitGB * 1024 * 1024 * 1024 / 8, catalogName() << " " << getName() <<
