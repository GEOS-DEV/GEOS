...
checkRelativeErrorFormat( const char *,
                          const char *,
                          const char *,
                          const char *,
                          real64 const v1,
                          ...
                          real64 const absTol )
{
  ...
  if( delta > absTol && delta > relTol * value )
  {
    return ::testing::AssertionFailure()
      ...
      << " relative error: " << delta / value << " (" << v1 << " vs " << v2
      << "),"
...
::testing::AssertionResult
checkRelativeErrorFormat( char const * a,
                          char const * b,
                          char const * c,
                          real64 const v1,
                          real64 const v2,
                          real64 const relTol )
{
  return checkRelativeErrorFormat( a,
                                   b,
                                   c,
                                   "DEFAULT_ABS_TOL",
                                   v1,
...
void
checkRelativeError( real64 const v1, real64 const v2, real64 const relTol )
{
  EXPECT_PRED_FORMAT3( checkRelativeErrorFormat, v1, v2, relTol );
}

void
checkRelativeError( real64 const v1,
                    real64 const v2,
                    real64 const relTol,
                    real64 const absTol )
{
...
void
compareMatrixRow( ROW_INDEX const rowNumber,
                  VALUE const relTol,
                  VALUE const absTol,
                  localIndex const length1,
                  COL_INDEX const * const indices1,
                  VALUE const * const values1,
                  localIndex const length2,
                  COL_INDEX const * const indices2,
                  VALUE const * const values2 )
{
  ...
  for( localIndex j1 = 0, j2 = 0; j1 < length1 && j2 < length2; ++j1, ++j2 )
  {
    while( j1 < length1 && j2 < length2 && indices1[j1] != indices1[j2] )
    {
      ...
      {
        ADD_FAILURE() << "column " << indices1[j1]
                      << ") in matrix 1 does not have a match";
      }
      while( j2 < length2 && indices2[j2] < indices1[j1] )
      {
        ADD_FAILURE() << "column " << indices2[j2]
                      << ") in matrix 2 does not have a match";
      }
    }
...
compareMatrixRow( ROW_INDEX const rowNumber,
                  VALUE const relTol,
                  VALUE const absTol,
                  arraySlice1d< COL_INDEX const > indices1,
                  arraySlice1d< VALUE const > values1,
                  arraySlice1d< COL_INDEX const > indices2,
                  arraySlice1d< VALUE const > values2 )
{
  ASSERT_EQ( indices1.size(), values1.size() );
  ASSERT_EQ( indices2.size(), values2.size() );

  compareMatrixRow( rowNumber,
                    relTol,
                    absTol,
                    indices1.size(),
                    indices1.dataIfContiguous(),
                    values1.dataIfContiguous(),
                    indices2.size(),
                    indices2.dataIfContiguous(),
                    values2.dataIfContiguous() );
}

template< typename MATRIX >
void
compareMatrices( MATRIX const & matrix1,
...
void
compareLocalMatrices( CRSMatrixView< T const, COL_INDEX const > const & matrix1,
                      ...
                      real64 const absTol = DEFAULT_ABS_TOL )
{
  ...
  for( localIndex i = 0; i < matrix1.numRows(); ++i )
  {
    compareMatrixRow( i,
                      relTol,
                      absTol,
                      matrix1.getColumns( i ),
                      matrix1.getEntries( i ),
                      matrix2.getColumns( i ),
                      matrix2.getEntries( i ) );
  }
}
