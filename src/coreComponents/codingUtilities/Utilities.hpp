 ...
 */
template <typename T>
GEOSX_FORCE_INLINE GEOSX_HOST_DEVICE constexpr bool isEqual(T const val1,
                                                            T const val2,
                                                            T const relTol = 0.0)
{
  T const absTol = (relTol > 10 * NumericTraits<T>::eps)
    ? relTol * (fabs(val1) + fabs(val2)) * 0.5
    : 0.0;
 ...
 */
template <typename T>
GEOSX_FORCE_INLINE GEOSX_HOST_DEVICE constexpr bool isZero(
  T const val,
  T const tol = NumericTraits<T>::eps)
{
  return -tol <= val && val <= tol;
}

template <typename T>
GEOSX_FORCE_INLINE GEOSX_HOST_DEVICE constexpr bool isOdd(T x)
{
  static_assert(std::is_integral<T>::value,
                "Not meaningful for non-integral types");
  return (x & 1);
}

template <typename T>
GEOSX_FORCE_INLINE GEOSX_HOST_DEVICE constexpr bool isEven(T x)
{
  return !isOdd(x);
}
...
bool executeOnMapValue(mapBase<T1, T2, SORTED> const& Map,
                       const T1& key,
                       LAMBDA&& lambda)
{
  bool rval = false;
...
{
  T_VALUE rvalue;
  typename mapBase<T_KEY, T_VALUE, SORTED>::const_iterator iter =
    theMap.find(key);
  if(iter == theMap.end())
  {
    rvalue = failValue;
...
GEOSX_HOST_DEVICE GEOSX_FORCE_INLINE void copy(localIndex const N,
                                               VEC1 const& v1,
                                               VEC2 const& v2)
{
...
GEOSX_HOST_DEVICE GEOSX_FORCE_INLINE void applyChainRule(localIndex const N,
                                                         MATRIX const& dy_dx,
                                                         VEC1 const& df_dy,
                                                         VEC2&& df_dx)
{
...
template <typename MATRIX, typename VEC1, typename VEC2>
GEOSX_HOST_DEVICE GEOSX_FORCE_INLINE void applyChainRuleInPlace(localIndex const N,
                                                                MATRIX const& dy_dx,
                                                                VEC1&& df_dxy,
                                                                VEC2&& work)
{
