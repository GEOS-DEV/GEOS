...
namespace PVTProps
{
  ...
  EvalArgs2D XYTable::Value(const EvalArgs2D& x, const EvalArgs2D& y) const 
  {
    ...
    EvalArgs2D out = m_value[xIndices[0] ][yIndices[0] ] * (1.0 - xWeight) * (1.0 - yWeight) + m_value[xIndices[0] ][yIndices[1] ] * (1.0 - xWeight) * yWeight + m_value[xIndices[1] ][yIndices[1] ] * xWeight * yWeight + m_value[xIndices[1] ][yIndices[0] ] * xWeight * (1.0 - yWeight); 
