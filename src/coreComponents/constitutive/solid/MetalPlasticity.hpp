...
/**
 * @class MetalPlasticityUpdates
 *
 * Class to provide elastic isotropic material updates that may be
 * called from a kernel function.
 */
class MetalPlasticityUpdates : public ElasticIsotropicUpdates
{
public:
  /**
   * @brief Constructor
   ...
   * @param[in] density    The ArrayView holding the density for each quadrature point.
   * @param[in] wavespeed    The ArrayView holding the wavespeed for each quadrature point.
   * @param[in] disableInelasticity Flag to disable plasticity for inelastic models
   */
  MetalPlasticityUpdates( arrayView1d< real64 > const & yieldStrength,
  ...
  MetalPlasticityUpdates() = delete;

  /// Default copy constructor
  MetalPlasticityUpdates( MetalPlasticityUpdates const & ) = default;

  /// Default move constructor
  MetalPlasticityUpdates( MetalPlasticityUpdates && ) = default;

  /// Deleted copy assignment operator
  MetalPlasticityUpdates & operator=( MetalPlasticityUpdates const & ) = delete;

  /// Deleted move assignment operator
  MetalPlasticityUpdates & operator=( MetalPlasticityUpdates && ) =  delete;
...
GEOS_HOST_DEVICE
inline
void MetalPlasticityUpdates::getElasticStiffness( localIndex const k,
                                            localIndex const q,
                                            real64 ( & stiffness )[6][6] ) const
{
  GEOS_UNUSED_VAR( q );
...
GEOS_HOST_DEVICE
inline
void MetalPlasticityUpdates::getElasticStrain( localIndex const k,
                                         localIndex const q,
                                         real64 ( & elasticStrain)[6] ) const
{
  ...
  GEOS_ERROR( "getElasticStrain not implemented for MetalPlasticity model" );
}


GEOS_HOST_DEVICE
inline
void MetalPlasticityUpdates::smallStrainNoStateUpdate_StressOnly( localIndex const k,
                                                            localIndex const q,
                                                            real64 const ( &totalStrain )[6],
                                                            real64 ( & stress )[6] ) const
{
  ...
  GEOS_ERROR( "smallStrainNoStateUpdate_StressOnly is not implemented for MetalPlasticity model" );
}


GEOS_HOST_DEVICE
inline
void MetalPlasticityUpdates::smallStrainNoStateUpdate( localIndex const k,
                                                 localIndex const q,
                                                 real64 const ( &totalStrain )[6],
                                                 real64 ( & stress )[6],
                                                 real64 ( & stiffness )[6][6] ) const
...
GEOS_HOST_DEVICE
inline
void MetalPlasticityUpdates::smallStrainUpdate_StressOnly( localIndex const k,
                                                     localIndex const q,
                                                     real64 const & timeIncrement,
                                                     real64 const ( &strainIncrement )[6],
                                                     real64 ( & stress )[6] ) const
{
  ...
  GEOS_UNUSED_VAR( stress );
  GEOS_ERROR( "SmallStrainUpdate_StressOnly is not implemented for MetalPlasticity model" );
}
...
GEOS_HOST_DEVICE
inline
void MetalPlasticityUpdates::smallStrainUpdate( localIndex const k,
                                          localIndex const q,
                                          real64 const & timeIncrement,
                                          real64 const ( &strainIncrement )[6],
                                          real64 ( & stress )[6],
...
GEOS_HOST_DEVICE
GEOS_FORCE_INLINE
void MetalPlasticityUpdates::computePlasticStrainIncrement ( localIndex const k,
                                                       localIndex const q,
                                                       const real64 timeIncrement,
                                                       real64 const ( &strainIncrement )[6],
                                                       real64 const ( &stressIncrement )[6],
...
/**
 * @class MetalPlasticity
 *
 * Class to provide an elastic isotropic material response.
 */
class MetalPlasticity : public ElasticIsotropic
{
public:

  /// Alias for MetalPlasticityUpdates
  using KernelWrapper = MetalPlasticityUpdates;
   ...
   */
  MetalPlasticity( string const & name,
             Group * const parent );

  /**
   * Default Destructor
   */
  virtual ~MetalPlasticity() override;
  ...
  static constexpr auto m_catalogNameString = "MetalPlasticity";
  ...
  /**
   * @brief Create a instantiation of the MetalPlasticityUpdate class
   *        that refers to the data in this.
   * @param includeState Flag whether to pass state arrays that may not be needed for "no-state" updates
   * @return An instantiation of MetalPlasticityUpdate.
   */
  MetalPlasticityUpdates createKernelUpdates( bool const includeState = true ) const
  {
    if( includeState )
    {
      return MetalPlasticityUpdates( m_yieldStrength,
