...
/**
 *  @file ElasticPressureDependnt.hpp
 */
...
/**
 * @class ElasticIsotropicPressureUpdates
 *
 * Class to provide elastic isotropic material updates that may be
 * called from a kernel function.
 */
class ElasticIsotropicPressureUpdates : public SolidBaseUpdates
{
public:
   ...
   *                   point.
   */
  ElasticIsotropicPressureUpdates( arrayView1d< real64 const > const & refPressure,
                                   arrayView1d< real64 const > const & refStrainVol,
                                   arrayView1d< real64 const > const & recompressibilityIndex,
                                   arrayView1d< real64 const > const & shearModulus,
                                   arrayView3d< real64, solid::STRESS_USD > const & newStress,
                                   arrayView3d< real64, solid::STRESS_USD > const & oldStress ):
    SolidBaseUpdates( newStress, oldStress ),
    m_refPressure( refPressure),
    m_recompressibilityIndex( recompressibilityIndex),
    m_refStrainVol( refStrainVol),
    m_shearModulus( shearModulus )
  {}

  /// Deleted default constructor
  ElasticIsotropicPressureUpdates() = delete;

  /// Default copy constructor
  ElasticIsotropicPressureUpdates( ElasticIsotropicPressureUpdates const & ) = default;

  /// Default move constructor
  ElasticIsotropicPressureUpdates( ElasticIsotropicPressureUpdates && ) = default;

  /// Deleted copy assignment operator
  ElasticIsotropicPressureUpdates & operator=( ElasticIsotropicPressureUpdates const & ) = delete;

  /// Deleted move assignment operator
  ElasticIsotropicPressureUpdates & operator=( ElasticIsotropicPressureUpdates && ) =  delete;
...
protected:
  ...
  arrayView1d< real64 const > const m_recompressibilityIndex;
...
{
   // NOTE: need current pressure. from old_stress?
   // For doxygen: state-dependent quantity based on new_stress
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::getElasticStrain( localIndex const k,
                                                localIndex const q,
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::smallStrainUpdate( localIndex const k,
                                                 localIndex const q,
                                                 real64 const ( & strainIncrement )[6],
                                                 real64 ( & stress )[6]) const
{
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::smallStrainNoStateUpdate( localIndex const k,
                                                        localIndex const q,
                                                        real64 const ( & totalStrain )[6],
                                                        real64 ( & stress )[6],
                                                        real64 ( & stiffness )[6][6] ) const
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::SmallStrainNoState( localIndex const k,
                                                        real64 const ( &voigtStrain )[ 6 ],
                                                        real64 ( & stress )[ 6 ] ) const
{
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::SmallStrain( localIndex const k,
                                                 localIndex const q,
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::HypoElastic( localIndex const k,
                                                 localIndex const q,
                                                 real64 const ( &Ddt )[ 6 ],
...
GEOSX_HOST_DEVICE
GEOSX_FORCE_INLINE
void ElasticIsotropicPressureUpdates::HyperElastic( localIndex const k,
                                                  real64 const (&FmI)[3][3],
                                                  real64 ( & stress )[ 6 ] ) const
{
  real64 const C1 = 0.5 * m_shearModulus[k];
 ...
 */
class ElasticIsotropicPressure : public SolidBase
{
public:
   ...
   */
  ElasticIsotropicPressure( string const & name, Group * const parent );

  /**
   * Default Destructor
   */
  virtual ~ElasticIsotropicPressure() override;
  ...
  ElasticIsotropicUpdates createKernelUpdates( bool const includeState = true ) const
  {
    if( includeState )
    {
      return ElasticIsotropicPressureUpdates( m_bulkModulus,
                                      m_shearModulus,
                                      m_newStress,
                                      m_oldStress );
    }
