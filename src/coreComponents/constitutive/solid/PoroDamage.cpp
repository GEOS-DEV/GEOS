...
PoroDamage< BASE >::PoroDamage( string const & name, Group * const parent ):
  BASE( name, parent ),
  m_damage(),
  ...
  m_criticalStrainEnergy()
{
  this->registerWrapper( viewKeyStruct::damageString, &m_damage )->
    ...
    setDescription( "Material PoroDamage Variable" );
...
template< typename BASE >
void PoroDamage< BASE >::postProcessInput()
{
  BASE::postProcessInput();
}

template< typename BASE >
void PoroDamage< BASE >::allocateConstitutiveData( dataRepository::Group * const parent,
...
typedef PoroDamage< ElasticIsotropic > PoroDamageElasticIsotropic;
typedef PoroDamage< PoroElastic < ElasticIsotropic> > PoroDamagePoroElasticIsotropic;
  
REGISTER_CATALOG_ENTRY( ConstitutiveBase, PoroDamageElasticIsotropic, string const &, Group * const )
REGISTER_CATALOG_ENTRY( ConstitutiveBase, PoroDamagePoroElasticIsotropic, string const &, Group * const )
