...
/**
 *  @file NonlinearElasticDuncanChangEB.cpp
 */
...
NonlinearElasticDuncanChangEB::NonlinearElasticDuncanChangEB( std::string const & name, Group * const parent ):
...
NonlinearElasticDuncanChangEB::~NonlinearElasticDuncanChangEB()
{}


void
NonlinearElasticDuncanChangEB::DeliverClone( string const & name,
                                      Group * const parent,
                                      std::unique_ptr<ConstitutiveBase> & clone ) const
{
  if( !clone )
  {
    clone = std::make_unique<NonlinearElasticDuncanChangEB>( name, parent );
  }
  SolidBase::DeliverClone( name, parent, clone );
  NonlinearElasticDuncanChangEB * const newConstitutiveRelation = dynamic_cast<NonlinearElasticDuncanChangEB *>(clone.get());
...
void NonlinearElasticDuncanChangEB::AllocateConstitutiveData( dataRepository::Group * const parent,
...
void NonlinearElasticDuncanChangEB::PostProcessInput()
{
...
void NonlinearElasticDuncanChangEB::StateUpdatePoint( localIndex const k,
                                               localIndex const q,
                                               R2SymTensor const & D,
                                               R2Tensor const & Rot,
...
REGISTER_CATALOG_ENTRY( ConstitutiveBase, NonlinearElasticDuncanChangEB, std::string const &, Group * const )
}
} /* namespace geosx */
