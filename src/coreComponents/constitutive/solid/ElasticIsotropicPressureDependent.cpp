...
void ElasticIsotropicPressureDependent::postInputInitialization()
{
  ...
  GEOS_THROW_CTX_IF( m_defaultRecompressionIndex <= 0,
                     getFullName() << ": Non-positive recompression index detected " << m_defaultRecompressionIndex,
  ...
  GEOS_THROW_CTX_IF( poisson < 0,
                     getFullName() << ": Elastic parameters lead to negative Poisson ratio at reference pressure ",
                     InputError, getDataContext() );
