...
public:
  ...
  operator=(
    LinearElasticIsotropicUpdates const & ) = delete;

  /// Deleted move assignment operator
  LinearElasticIsotropicUpdates &
  operator=( LinearElasticIsotropicUpdates && ) = delete;
   ...
   */
  GEOSX_HOST_DEVICE inline virtual void
  GetStiffness(
    localIndex const k,
  ...
  void
  GetStiffness( localIndex const k, array2d< real64 > & c ) const
  {
    real64 const G = m_shearModulus[k];
    real64 const Lame = m_bulkModulus[k] - 2.0 / 3.0 * G;
  ...
  GEOSX_HOST_DEVICE
  virtual void
  HyperElastic( localIndex const k,
...
GEOSX_FORCE_INLINE
void
LinearElasticIsotropicUpdates::SmallStrainNoState(
  localIndex const k,
  real64 const ( &voigtStrain )[6],
...
GEOSX_FORCE_INLINE
void
LinearElasticIsotropicUpdates::SmallStrain( localIndex const k,
                                            localIndex const q,
                                            real64 const ( &voigtStrainInc )[6] ) const
{
  real64 const lambda = m_bulkModulus[k] - 2.0 / 3.0 * m_shearModulus[k];
  real64 const volStrain =
    ( voigtStrainInc[0] + voigtStrainInc[1] + voigtStrainInc[2] );
  real64 const TwoG = 2.0 * m_shearModulus[k];

  m_stress( k, q, 0 ) =
    m_stress( k, q, 0 ) + TwoG * voigtStrainInc[0] + lambda * volStrain;
  m_stress( k, q, 1 ) =
    m_stress( k, q, 1 ) + TwoG * voigtStrainInc[1] + lambda * volStrain;
  m_stress( k, q, 2 ) =
    m_stress( k, q, 2 ) + TwoG * voigtStrainInc[2] + lambda * volStrain;
...
GEOSX_FORCE_INLINE
void
LinearElasticIsotropicUpdates::HypoElastic( localIndex const k,
                                            localIndex const q,
                                            real64 const ( &Ddt )[6],
...
GEOSX_FORCE_INLINE
void
LinearElasticIsotropicUpdates::HyperElastic( localIndex const k,
                                             real64 const ( &FmI )[3][3],
                                             real64 ( &stress )[6] ) const
{
  ...
  real64 const detFm1 = FmI[0][0] + FmI[1][1] + FmI[2][2] -
    ...
    FmI[0][0] *
      ( FmI[1][1] - FmI[1][2] * FmI[2][1] + FmI[2][2] + FmI[1][1] * FmI[2][2] );

  real64 const p = -2 * D1 * ( detFm1 + 1.0 ) * detFm1;
  real64 devB[6] = {
    1 / 3 *
      ( 2 * FmI[0][0] * ( 2 + FmI[0][0] ) - FmI[1][1] * ( 2 + FmI[1][1] ) -
        FmI[2][2] * ( 2 + FmI[2][2] ) + 2 * FmI[0][1] * FmI[0][1] +
        2 * FmI[0][2] * FmI[0][2] - FmI[1][0] * FmI[1][0] -
        FmI[1][2] * FmI[1][2] - FmI[2][0] * FmI[2][0] - FmI[2][1] * FmI[2][1] ),
    1 / 3 *
      ( -FmI[0][0] * ( 2 + FmI[0][0] ) + 2 * FmI[1][1] * ( 2 + FmI[1][1] ) -
        FmI[2][2] * ( 2 + FmI[2][2] ) - FmI[0][1] * FmI[0][1] -
        FmI[0][2] * FmI[0][2] + 2 * FmI[1][0] * FmI[1][0] +
        2 * FmI[1][2] * FmI[1][2] - FmI[2][0] * FmI[2][0] - FmI[2][1] * FmI[2][1] ),
    1 / 3 *
      ( -FmI[0][0] * ( 2 + FmI[0][0] ) - FmI[1][1] * ( 2 + FmI[1][1] ) +
        2 * FmI[2][2] * ( 2 + FmI[2][2] ) - FmI[0][1] * FmI[0][1] -
        FmI[0][2] * FmI[0][2] - FmI[1][0] * FmI[1][0] - FmI[1][2] * FmI[1][2] +
        2 * FmI[2][0] * FmI[2][0] + 2 * FmI[2][1] * FmI[2][1] ),
    FmI[1][2] + FmI[1][0] * FmI[2][0] + FmI[2][1] + FmI[1][1] * FmI[2][1] +
      FmI[1][2] * FmI[2][2],
    FmI[0][2] + FmI[2][0] + FmI[0][0] * FmI[2][0] + FmI[0][1] * FmI[2][1] +
      FmI[0][2] * FmI[2][2],
    FmI[0][1] + FmI[1][0] + FmI[0][0] * FmI[1][0] + FmI[0][1] * FmI[1][1] +
      FmI[0][2] * FmI[1][2] };
...
public:
   ...
   */
  void
  setDefaultBulkModulus( real64 const bulkModulus )
  {
   ...
   */
  void
  setDefaultShearModulus( real64 const shearModulus )
  {
   ...
   */
  LinearElasticIsotropicUpdates
  createKernelUpdates( bool const includeState = true ) const
  {
    if( includeState )
    {
    ...
    {
      return LinearElasticIsotropicUpdates(
        m_bulkModulus,
        m_shearModulus,
  ...
  UPDATE_KERNEL
  createDerivedKernelUpdates( PARAMS &&... constructorParams )
  {
