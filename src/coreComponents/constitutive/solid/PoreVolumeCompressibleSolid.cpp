...
{
PoreVolumeCompressibleSolid::PoreVolumeCompressibleSolid( std::string const & name,
                                                          Group * const parent ) :
...
void
PoreVolumeCompressibleSolid::DeliverClone(
  string const & name,
  Group * const parent,
...
void
PoreVolumeCompressibleSolid::AllocateConstitutiveData(
...
void
PoreVolumeCompressibleSolid::PostProcessInput()
{
  if( m_compressibility < 0.0 )
  {
    string const message = "An invalid value of fluid bulk modulus (" +
      std::to_string( m_compressibility ) + ") is specified";
    GEOSX_ERROR( message );
  }
...
void
PoreVolumeCompressibleSolid::StateUpdatePointPressure( real64 const & pres,
                                                       localIndex const k,
                                                       localIndex const q )
{
...
void
PoreVolumeCompressibleSolid::StateUpdateBatchPressure(
