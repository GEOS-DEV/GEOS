...
/*
        <CompositionalTwoPhaseFluidPhillipsBrine
            ...
            componentNames="{ CH4, CO2, N2, H2O }"
            componentCriticalPressure="{ 4.59920e+06, 7.37730e+06, 3.39580e+06, 2.20640e+07 }"
            componentCriticalTemperature="{ 1.90564e+02, 3.04128e+02, 1.26192e+02, 6.47096e+02 }"
            componentCriticalVolume="{ 9.86278e-05, 9.41185e-05, 8.94142e-05, 5.59480e-05 }"
            componentAcentricFactor="{ 1.14200e-02, 2.23940e-01, 3.72000e-02, 3.44300e-01 }"
            componentMolarWeight="{ 1.60425e-02, 4.40095e-02, 2.80134e-02, 1.80153e-02 }"
            componentBinaryCoeff="{
                { 0.0000, 0.0000, 0.0000, 0.4850 },
                { 0.0000, 0.0000, 0.0000, 0.1896 },
                { 0.0000, 0.0000, 0.0000, 0.4778 },
                { 0.4850, 0.1896, 0.4778, 0.0000 }
            }"
            pressureCoordinates="{
            ...
            temperatureCoordinates="{ 277.15, 293.65, 310.15, 326.65, 343.15, 359.65, 376.15, 392.65, 409.15, 425.65, 442.15 }"
...
public:
  ...
  void testFlash( FlashData< NC > const & data )
  {
    ...
    std::array<real64,4> kv{ 1.364671e+03, 1.595669e+02, 1.718535e+02, 6.450368e-02 };
    for (int i = 0; i < 4; i++) kValues(0,i) = kv[i];
...
INSTANTIATE_TEST_SUITE_P(
  ...
  ::testing::ValuesIn<FlashData<4>>({
    ...
    {4.7343674e+06, 403.29381020, { 0.718369, 0.0399971, 5.58409e-14, 0.241633 }, 0.99999969, 0.00000031, { 0.00000000, 0.02572570, 0.97427430 }, { 0.00000000, 0.99724814, 0.00275186 }},
  })
);
/*
INSTANTIATE_TEST_SUITE_P(
