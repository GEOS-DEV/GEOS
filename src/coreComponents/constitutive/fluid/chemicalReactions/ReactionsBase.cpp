...
/
void ReactionsBase::computeLog10ActCoefBDotModel( real64 const temperature,
...
{
  ReactionsBase::allocateConstitutiveData( parent, numConstitutivePointsPerParentIndex );
...
// Find whether H+ and H2O are specified as basis species
  ...
  GEOSX_ERROR_IF( HplusNotFound, "ReactionsBase: H+ is not specified in basisSpeciesNames" );
  GEOSX_ERROR_IF( H2OFound, "ReactionsBase: H2O cannot be specified in basisSpeciesNames" );
...
void ReactionsBase::resizeFields( localIndex const size )	//what size is being passed here? I don't understand it but I have kept it as is. 
{
  localIndex const NBasis = m_numPrimarySpecies;		
  localIndex const NDependent = m_numSecSpecies;
...
void ReactionsBase::ComputeLogActCoef( real64 const pressure,
...
void ReactionsBase::KernelWrapper::Compute( real64 const pressure,
                                            real64 const temperature,
...
REGISTER_CATALOG_ENTRY( ReactionsBase, EquilibriumReaction, string const &, string_array const &, string_array const &, string_array const &, array1d< real64 > const & )
