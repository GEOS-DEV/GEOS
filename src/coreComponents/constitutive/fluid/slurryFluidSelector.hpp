...
void constitutiveUpdatePassThru(SlurryFluidBase const& fluid, LAMBDA&& lambda)
{
  ConstitutivePassThruHandler<ProppantSlurryFluid>::Execute(
    fluid,
    std::forward<LAMBDA>(lambda));
}

template <typename LAMBDA>
void constitutiveUpdatePassThru(SlurryFluidBase& fluid, LAMBDA&& lambda)
{
  ConstitutivePassThruHandler<ProppantSlurryFluid>::Execute(
    fluid,
    std::forward<LAMBDA>(lambda));
