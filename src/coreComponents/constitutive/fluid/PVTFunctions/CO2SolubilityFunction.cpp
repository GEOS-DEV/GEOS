...
namespace internal
{
  ...
  real64 logF(real64 const & T, real64 const & P, real64 const & V_r)
  {
    ...
    real64 const log_f = Z - 1 - log(Z) + (acoef[0] + acoef[1]/T_r/T_r + acoef[2]/T_r/T_r/T_r)/V_r + (acoef[3] + acoef[4]/T_r/T_r + acoef[5]/T_r/T_r/T_r)/2.0/V_r/V_r + (acoef[6] + acoef[7]/T_r/T_r + acoef[8]/T_r/T_r/T_r)/4.0/V_r/V_r/V_r/V_r + (acoef[9] + acoef[10]/T_r/T_r + acoef[11]/T_r/T_r/T_r)/5.0/V_r/V_r/V_r/V_r/V_r + acoef[12]/2.0/T_r/T_r/T_r/acoef[14] * (acoef[13] + 1.0 - (acoef[13] + 1.0 + acoef[14]/V_r/V_r) * exp(-acoef[14]/V_r/V_r));
  ...
  FlashModel( inputPara[1], componentNames, componentMolarWeight)
{
  ...
  for(localIndex i = 0; i < componentNames.size(); ++i)
  {
    ...
    if(stringutilities::streq(componentNames[i], "Water") || stringutilities::streq(componentNames[i], "water"))
    {
      m_waterIndex = i;
      notFound = 0;
      break;
  ...
  for(localIndex i = 0; i < phaseNames.size(); ++i)
  {
    ...
    if(stringutilities::streq(phaseNames[i], "CO2") || stringutilities::streq(phaseNames[i], "co2") || stringutilities::streq(phaseNames[i], "gas") || stringutilities::streq(phaseNames[i], "Gas"))
    {
      m_phaseGasIndex = i;
      notFound = 0;
      break;
  ...
  for(localIndex i = 0; i < phaseNames.size(); ++i)
  {
    ...
    if(stringutilities::streq(phaseNames[i], "Water") || stringutilities::streq(phaseNames[i], "water") || stringutilities::streq(phaseNames[i], "Liquid") || stringutilities::streq(phaseNames[i], "liquid"))
    {
...
void CO2SolubilityFunction::MakeTable(string_array const & inputPara)
{
  ...
  internal::CalculateCO2Solubility(pressures, temperatures, m, solubilities);
