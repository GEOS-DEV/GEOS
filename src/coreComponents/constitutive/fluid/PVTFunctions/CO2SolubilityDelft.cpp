...
real64 CO2SolubilityDelftFunction( string const & name,
...
void calculateCO2SolubilityDelft( string const & functionName,
                             real64 const & tolerance,
                             ...
                             array1d< real64 > const & values )
{
  ...
  for( localIndex i = 0; i < nPressures; ++i )
  {
    ...
    for( localIndex j = 0; j < nTemperatures; ++j )
    {
      ...
      real64 const V_r = CO2SolubilityDelftFunction( functionName, tolerance, T, P, &co2EOS );
                                           ...
                                           FunctionManager & functionManager )
{
  ...
  calculateCO2SolubilityDelft( functionName, tolerance, tableCoords, salinity, values );
...
CO2SolubilityDelft::CO2SolubilityDelft( string const & name,
                  ...
                  componentMolarWeight )
{
  GEOSX_THROW_IF_NE_MSG( phaseNames.size(), 2,
                         "The CO2SolubilityDelft model is a two-phase model",
                         InputError );
  GEOSX_THROW_IF_NE_MSG( componentNames.size(), 2,
                         "The CO2SolubilityDelft model is a two-component model",
                         InputError );
  ...
  m_CO2SolubilityDelftTable = makeSolubilityTable( inputParams, m_modelName, FunctionManager::getInstance() );
}

CO2SolubilityDelft::KernelWrapper CO2SolubilityDelft::createKernelWrapper() const
{
  return KernelWrapper( m_componentMolarWeight,
                        *m_CO2SolubilityDelftTable,
                        m_CO2Index,
                        m_waterIndex,
                        m_phaseGasIndex,
                        m_phaseLiquidIndex );
...
REGISTER_CATALOG_ENTRY( FlashModelBase, CO2SolubilityDelft, string const &, string_array const &, string_array const &, string_array const &, array1d< real64 > const & )
