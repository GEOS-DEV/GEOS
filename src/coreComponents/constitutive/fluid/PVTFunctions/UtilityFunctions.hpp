...
template <typename T, int Dim> class EvalArgs
{
public:
  EvalArgs()
  {
  ...
  bool operator!=(const EvalArgs &arg) const { return !(*this == arg); }

  bool operator>(const EvalArgs &arg) const { return this->m_var > arg.m_var; }
  ...
  bool operator==(const T &arg) const { return this->m_var == arg; }

  bool operator!=(const T &arg) const { return !(*this == arg); }
...
public:
  ...
  real64_array &XArray() { return m_x; }

  real64_array &YArray() { return m_y; }

  real64_array2d &ValueArray() { return m_value; }

  virtual string const &TableName() const { return m_tableName; }

  virtual EvalArgs1D Value(EvalArgs1D const &) const { return 0; }
...
public:
  ...
  real64_array &ValueArray() { return m_value; }
...
private:
  template <class T> T GetValue(T const &x) const;
