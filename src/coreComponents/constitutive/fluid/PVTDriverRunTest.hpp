...
void PVTDriver::runTest( FLUID_TYPE & fluid, arrayView2d< real64 > const & table )
{
                        ...
                        [numPhases, numSteps, kernelWrapper, table, composition] GEOSX_HOST_DEVICE ( localIndex const i )
  {
    ...
    {
      kernelWrapper.update( i, 0, table( n, static_cast< localIndex >(PRES) ), table( n, static_cast< localIndex >(TEMP) ), composition[i] );
      table( n, static_cast< localIndex >(TEMP) + 1 ) = kernelWrapper.totalDensity()( i, 0 );

      for( integer p = 0; p < numPhases; ++p )
      {
        table( n, static_cast< localIndex >(TEMP) + 2 + p ) = kernelWrapper.phaseFraction()( i, 0, p );
        table( n, static_cast< localIndex >(TEMP) + 2 + p + numPhases ) = kernelWrapper.phaseDensity()( i, 0, p );
        table( n, static_cast< localIndex >(TEMP) + 2 + p + 2 * numPhases ) = kernelWrapper.phaseViscosity()( i, 0, p );
      }
    }
  } );
