...
{
  static std::map< string, PVTPackage::PHASE_TYPE > const PHASE_TYPES =
  {
    { "gas", PVTPackage::PHASE_TYPE::GAS },
    { "oil", PVTPackage::PHASE_TYPE::OIL },
    { "water", PVTPackage::PHASE_TYPE::LIQUID_WATER_RICH }
  };
  auto const it = PHASE_TYPES.find( name );
  GEOSX_ERROR_IF( it == PHASE_TYPES.end(), "Fluid phase not supported by PVTPackage: " << name );
  return it->second;
}
...
void MultiFluidPVTPackageWrapperUpdate::Compute( real64 pressure,
                                                 ...
                                                 arraySlice1d< real64, 0 > const & dTotalDensityDGlobalCompFraction ) const
{
