...
void BlackOilFluid::checkTableConsistency() const
{
  ...
  GEOS_THROW_CTX_IF( m_PVTO.undersaturatedPressure[m_PVTO.numSaturatedPoints - 1].size() <= 1,
  ...
  for( integer i = 0; i < m_PVTO.numSaturatedPoints - 1; ++i )
  {
    ...
    GEOS_THROW_CTX_IF( ( m_PVTO.Rs[i + 1] - m_PVTO.Rs[i] ) <= 0,
                       GEOS_FMT( "{}: Rs must increase with Pb in {}", getFullName(), m_tableFiles[m_phaseOrder[PT::OIL]] ),
                       InputError, getDataContext() );
    // Bo must increase with Pb
    GEOS_THROW_CTX_IF( ( m_PVTO.saturatedBo[i + 1] - m_PVTO.saturatedBo[i] ) <= 0,
    ...
    GEOS_THROW_CTX_IF( ( m_PVTO.saturatedViscosity[i + 1] - m_PVTO.saturatedViscosity[i] ) >= 0,
  ...
  for( integer i = 0; i < m_PVTO.numSaturatedPoints; ++i )
  {
    ...
    {
      // Pressure
      GEOS_THROW_CTX_IF( ( m_PVTO.undersaturatedPressure[i][j + 1] - m_PVTO.undersaturatedPressure[i][j] ) <= 0,
                         GEOS_FMT( "{}: P must decrease in undersaturated region in {}", getFullName(), m_tableFiles[m_phaseOrder[PT::OIL]] ),
                         InputError, getDataContext() );
      // Bo must decrease with P
      GEOS_THROW_CTX_IF( ( m_PVTO.undersaturatedBo[i][j + 1] - m_PVTO.undersaturatedBo[i][j] ) >= 0,
      ...
      GEOS_THROW_CTX_IF( ( m_PVTO.undersaturatedViscosity[i][j + 1] - m_PVTO.undersaturatedViscosity[i][j] ) < -1e-10,
