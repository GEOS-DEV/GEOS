                                     ...
                                     arraySlice1d< real64, USD2 > const & vapourComposition )
{
  ...
  for( localIndex iterationCount = 0; iterationCount < maxIterations; ++iterationCount )
  {
    ...
    bool const rrStatus = RachfordRice::solve( kVapourLiquid.toSliceConst(), composition, presentComponents, vapourPhaseMoleFraction );

#if !defined(GEOS_DEVICE_COMPILE)
    GEOS_WARNING_IF( !rrStatus,
                     GEOS_FMT( "Rachford-Rice solver failed to converge at pressure {:.5e}, temperature {:.3f} and composition ",
