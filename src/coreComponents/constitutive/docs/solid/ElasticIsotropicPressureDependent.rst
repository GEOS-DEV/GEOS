...
:math:`\epsilon_{v}^e` and :math:`\epsilon_{s}^e`are the volumetrix and deviatoric components of the the strain tensor. 
:math:`\epsilon_{v0}` and :math:`p_0` are the initial volumetric strain and initial pressure. :math:`C_r` denotes the elastic compressibility index,
...
The following attributes are supported.  Note that two elastic constants :math:`C_r` and :math:`mu`, in addition to the initial volumetrix strain and initial pressure need to be provided.  The "default" keyword in front of certain properties indicates that this
