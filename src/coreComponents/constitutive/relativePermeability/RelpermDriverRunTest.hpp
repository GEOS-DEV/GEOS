...
//specific to Hysteresis
    ...
    RelpermDriver::runTest( RELPERM_TYPE & relperm, arrayView3d< real64 > const & table  )
    {
                             ...
                             [numPhases, kernelWrapper, saturation, table, offset ] GEOSX_HOST_DEVICE (localIndex const i )
        {
            for( integer n = 0; n < saturation.size(1); ++n )
            {
                ...
                for( integer p = 0; p < numPhases; ++p )
                {
                    ...
                    if( kernelWrapper.phaseHasHysteresis[p] )
//                    table( n, 0, offset + numPhases + 1 + p ) = kernelWrapper.relperm_h()( i, 0, p );
                }
            }
        } );
