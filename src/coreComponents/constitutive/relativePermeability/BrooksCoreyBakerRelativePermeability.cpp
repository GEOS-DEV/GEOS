  ...
  RelativePermeabilityBase( name, parent )
{
  ...
  registerWrapper( viewKeyStruct::waterOilRelPermExponentString,
                   &m_waterOilRelPermExponent )
    ->setApplyDefaultValue( 1.0 )
    ->setInputFlag( InputFlags::OPTIONAL )
    ->setDescription(
      "Rel perm power law exponent for the pair (water phase, oil phase) at "
      "residual gas saturation" );

  registerWrapper( viewKeyStruct::waterOilRelPermMaxValueString,
                   &m_waterOilRelPermMaxValue )
    ->setApplyDefaultValue( 0.0 )
    ->setInputFlag( InputFlags::OPTIONAL )
    ->setDescription(
      "Maximum rel perm value for the pair (water phase, oil phase) at "
      "residual gas saturation" );

  registerWrapper( viewKeyStruct::gasOilRelPermExponentString,
                   &m_gasOilRelPermExponent )
    ->setApplyDefaultValue( 1.0 )
    ->setInputFlag( InputFlags::OPTIONAL )
    ->setDescription(
      "Rel perm power law exponent for the pair (gas phase, oil phase) at "
      "residual water saturation" );

  registerWrapper( viewKeyStruct::gasOilRelPermMaxValueString,
                   &m_gasOilRelPermMaxValue )
    ->setApplyDefaultValue( 0.0 )
    ->setInputFlag( InputFlags::OPTIONAL )
    ->setDescription(
      "Maximum rel perm value for the pair (gas phase, oil phase) at residual "
      "water saturation" );
}
...
void
BrooksCoreyBakerRelativePermeability::DeliverClone(
  string const & name,
  Group * const parent,
  std::unique_ptr< ConstitutiveBase > & clone ) const
{
  ...
  BrooksCoreyBakerRelativePermeability & relPerm =
    dynamicCast< BrooksCoreyBakerRelativePermeability & >( *clone );
...
void
BrooksCoreyBakerRelativePermeability::PostProcessInput()
{
  ...
  GEOSX_ERROR_IF( m_phaseOrder[PhaseType::OIL] < 0,
                  "BrooksCoreyBakerRelativePermeability: reference oil phase "
                  "has not been defined and must be included in model" );
...
#define COREY_CHECK_INPUT_LENGTH( data, expected, attr )                             \
  ...
  {                                                                                  \
    GEOSX_ERROR(                                                                     \
      "BrooksCoreyBakerRelativePermeability: invalid number of entries in "          \
  ...
  {
    COREY_CHECK_INPUT_LENGTH( m_waterOilRelPermExponent,
                              2,
                              viewKeyStruct::waterOilRelPermExponentString )
    COREY_CHECK_INPUT_LENGTH( m_waterOilRelPermMaxValue,
                              2,
                              viewKeyStruct::waterOilRelPermMaxValueString )
  }

  if( m_phaseOrder[PhaseType::GAS] >= 0 )
  {
    COREY_CHECK_INPUT_LENGTH( m_gasOilRelPermExponent,
                              2,
                              viewKeyStruct::gasOilRelPermExponentString )
    COREY_CHECK_INPUT_LENGTH( m_gasOilRelPermMaxValue,
                              2,
                              viewKeyStruct::gasOilRelPermMaxValueString )
  }
...
#undef COREY_CHECK_INPUT_LENGTH
  ...
  for( localIndex ip = 0; ip < NP; ++ip )
  {
    GEOSX_ERROR_IF(
      ...
      "BrooksCoreyBakerRelativePermeability: invalid phase min volume fraction "
      "value: "
        << m_phaseMinVolumeFraction[ip] );
    m_volFracScale -= m_phaseMinVolumeFraction[ip];
  }
  GEOSX_ERROR_IF( m_volFracScale < 0.0,
                  "BrooksCoreyBakerRelativePermeability: sum of min volume "
                  "fractions exceeds 1.0" );
  ...
  for( localIndex ip = 0; ip < 2; ++ip )
  {
    if( m_phaseOrder[PhaseType::WATER] >= 0 )
    {
      GEOSX_ERROR_IF( m_waterOilRelPermExponent[ip] < 0.0,
                      "BrooksCoreyBakerRelativePermeability: invalid water-oil "
                      "exponent value: "
                        << m_waterOilRelPermExponent[ip] );
      GEOSX_ERROR_IF(
        m_waterOilRelPermMaxValue[ip] < 0.0 || m_waterOilRelPermMaxValue[ip] > 1.0,
        "BrooksCoreyBakerRelativePermeability: invalid maximum value: "
          << m_waterOilRelPermMaxValue[ip] );
    }

    if( m_phaseOrder[PhaseType::GAS] >= 0 )
    {
      GEOSX_ERROR_IF(
        m_gasOilRelPermExponent[ip] < 0.0,
        "BrooksCoreyBakerRelativePermeability: invalid gas-oil exponent value: "
          << m_gasOilRelPermExponent[ip] );
      GEOSX_ERROR_IF(
        m_gasOilRelPermMaxValue[ip] < 0.0 || m_gasOilRelPermMaxValue[ip] > 1.0,
        "BrooksCoreyBakerRelativePermeability: invalid maximum value: "
          << m_gasOilRelPermMaxValue[ip] );
    }
  }

  if( m_phaseOrder[PhaseType::WATER] >= 0 && m_phaseOrder[PhaseType::GAS] >= 0 )
  {
    real64 const mean = 0.5 *
      ( m_gasOilRelPermMaxValue[GasOilPairPhaseType::OIL] +
