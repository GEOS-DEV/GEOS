...
/**
 * @file PistonPressureMPMEvent.cpp
 */

#include "PistonPressureMPMEvent.hpp"
...
namespace geos
{
  ...
  PistonPressureMPMEvent::PistonPressureMPMEvent( const string & name,
                                                    Group * const parent ) :
  ...
  PistonPressureMPMEvent::~PistonPressureMPMEvent() 
  {}

  void PistonPressureMPMEvent::postInputInitialization()
  {
    GEOS_ERROR_IF( m_boreholeRadius < 0.0   , "Piston radius must be positive!");

    GEOS_LOG_RANK_0( "PistonPressureEvent: " << 
                     "Time=" << m_time << ", " << 
                     "Interval=" << m_interval << ", " << 
  ...
  REGISTER_CATALOG_ENTRY( MPMEventBase, PistonPressureMPMEvent, string const &, Group * const )
