 ...
 * tensor.
 */
template <int T_dim> class R1TensorT : public TensorBaseT<T_dim>
{
public:
  ...
  template <int USD>
  GEOSX_HOST_DEVICE CONSTEXPR_WITH_NDEBUG inline R1TensorT& operator=(
    LvArray::ArraySlice<realT const, 1, USD, std::ptrdiff_t> const& src)
  {
  ...
  template <int USD>
  GEOSX_HOST_DEVICE constexpr inline R1TensorT& operator+=(
    LvArray::ArraySlice<realT, 1, USD, std::ptrdiff_t> const& src)
  {
    return (*this) +=
      reinterpret_cast<LvArray::ArraySlice<realT const, 1, USD, std::ptrdiff_t> const&>(
             src);
  }
  ...
  GEOSX_HOST_DEVICE inline realT operator()(const int i) const
  {
    return this->t_data[i];
  }

  /// non-const access to data
  GEOSX_HOST_DEVICE inline realT& operator()(const int i)
  {
    return this->t_data[i];
  }

  /// const access to data
  GEOSX_HOST_DEVICE GEOSX_FORCE_INLINE realT operator[](const int i) const
  {
    return this->t_data[i];
  }

  /// non-const access to data
  GEOSX_HOST_DEVICE GEOSX_FORCE_INLINE realT& operator[](const int i)
  {
    return this->t_data[i];
  }
  ...
  friend inline GEOSX_HOST_DEVICE R1TensorT<T_dim> Cross(const R1TensorT<T_dim>& a,
                                                         const R1TensorT<T_dim>& b)
  {
    R1TensorT<T_dim> c;
    c.Cross(a, b);
    return c;
...
template <> inline R1TensorT<2>::R1TensorT(realT x, realT y) : TensorBaseT<2>()
{
  this->t_data[0] = x;
  this->t_data[1] = y;
}
...
// throw a compile time error.
template <>
inline R1TensorT<3>::R1TensorT(realT x, realT y, realT z) : TensorBaseT<3>()
{
  this->t_data[0] = x;
  this->t_data[1] = y;
  this->t_data[2] = z;
...
template <int T_dim> void R1TensorT<T_dim>::print(std::ostream& os) const
{
  for(int i = 0; i < T_dim; ++i) os << (*this)(i) << '\t';
}
 ...
 */
template <int T_dim>
GEOSX_HOST_DEVICE inline R1TensorT<T_dim>& R1TensorT<T_dim>::operator=(
  const realT& rhs)
{
  TensorBaseT<T_dim>::operator=(rhs);
  return *this;
 ...
 */
template <int T_dim>
GEOSX_HOST_DEVICE inline realT R1TensorT<T_dim>::L2_Norm(void) const
{
  realT norm = 0.0;
 ...
 */
template <int T_dim> inline realT R1TensorT<T_dim>::Sum(void) const
{
  realT sum = 0.0;

  for(int i = 1; i <= T_dim; ++i) sum += this->t_data[i - 1];
 ...
 */
template <int T_dim>
inline void R1TensorT<T_dim>::AiBi(const R1TensorT<T_dim>& A,
                                   const R1TensorT<T_dim>& B)
{
  for(int i = 0; i < T_dim; i++) this->t_data[i] = A.t_data[i] * B.t_data[i];
}
 ...
 */
template <int T_dim>
GEOSX_HOST_DEVICE inline void R1TensorT<T_dim>::Cross(const R1TensorT<T_dim>& a,
                                                      const R1TensorT<T_dim>& b)
{
  if(T_dim == 3)
  {
