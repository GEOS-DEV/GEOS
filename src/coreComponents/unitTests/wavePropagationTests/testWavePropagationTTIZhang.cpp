  ...
  <Problem>
    ...
    <Events
      ...
      <PeriodicEvent
        ...
        target="/Solvers/acousticTTIZhangSolver"/>
      <PeriodicEvent
...
class AcousticTTIZhangWaveEquationSEMTest : public ::testing::Test
{
public:

  AcousticTTIZhangWaveEquationSEMTest():
...
protected:
  ...
  GeosxState state;
  AcousticTTIZhangWaveEquationSEM * propagator;
};

real64 constexpr AcousticTTIZhangWaveEquationSEMTest::time;
real64 constexpr AcousticTTIZhangWaveEquationSEMTest::dt;
real64 constexpr AcousticTTIZhangWaveEquationSEMTest::eps;

TEST_F( AcousticTTIZhangWaveEquationSEMTest, SeismoTrace )
{

  DomainPartition & domain = state.getProblemManager().getDomainPartition();
  propagator = &state.getProblemManager().getPhysicsSolverManager().getGroup< AcousticTTIZhangWaveEquationSEM >( "acousticTTIZhangSolver" );
  real64 time_n = time;
  // run for 1s (10 steps)
  for( int i=0; i<10; i++ )
  {
  ...
  // retrieve seismo
  arrayView2d< real32 > const pReceivers = propagator->getReference< array2d< real32 > >( AcousticTTIZhangWaveEquationSEM::viewKeyStruct::pressureNp1AtReceiversString() ).toView();
