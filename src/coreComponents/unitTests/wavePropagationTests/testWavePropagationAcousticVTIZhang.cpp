...
#include "physicsSolvers/wavePropagation/sem/acoustic/secondOrderEqn/anisotropic/AcousticVTIZhangWaveEquationSEM.hpp"
...
class AcousticVTIZhangWaveEquationSEMTest : public ::testing::Test
{
public:

  AcousticVTIZhangWaveEquationSEMTest():
...
protected:
  ...
  GeosxState state;
  AcousticVTIZhangWaveEquationSEM * propagator;
};

real64 constexpr AcousticVTIZhangWaveEquationSEMTest::time;
real64 constexpr AcousticVTIZhangWaveEquationSEMTest::dt;
real64 constexpr AcousticVTIZhangWaveEquationSEMTest::eps;

TEST_F( AcousticVTIZhangWaveEquationSEMTest, SeismoTrace )
{

  DomainPartition & domain = state.getProblemManager().getDomainPartition();
  propagator = &state.getProblemManager().getPhysicsSolverManager().getGroup< AcousticVTIZhangWaveEquationSEM >( "acousticVTIZhangSolver" );
  real64 time_n = time;
  // run for 1s (10 steps)
  for( int i=0; i<10; i++ )
  {
  ...
  // retrieve seismo
  arrayView2d< real32 > const pReceivers = propagator->getReference< array2d< real32 > >( AcousticVTIZhangWaveEquationSEM::viewKeyStruct::pressureNp1AtReceiversString() ).toView();
