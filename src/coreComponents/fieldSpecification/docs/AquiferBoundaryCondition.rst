...
For more complex meshes, sch as those imported using the **VTKMesh**, using a **Box** to perform a face selection is challenging. We recommend using a tagged ``PhysicalEntity`` in the .msh file instead. This physical entity will be used to locate the face set.
