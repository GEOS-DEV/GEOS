  ...
  m_max()
{
  ...
  registerWrapper( viewKeyStruct::radialCoordsString(), &(m_vertices[0]) ).
    ...
    setDescription( "Radial coordinates of each mesh block vertex" );

  registerWrapper( viewKeyStruct::tangentCoordsString(), &(m_vertices[1]) ).
    setInputFlag( InputFlags::REQUIRED ).
    setSizedFromParent( 0 ).
    setDescription( "Tangent coordinates of each mesh block vertex" );

  registerWrapper( viewKeyStruct::axialCoordsString(), &(m_vertices[2]) ).
    setInputFlag( InputFlags::REQUIRED ).
    setSizedFromParent( 0 ).
    setDescription( "Axial coordinates of each mesh block vertex" );

  registerWrapper( viewKeyStruct::radialElemsString(), &(m_nElems[0]) ).
    setInputFlag( InputFlags::REQUIRED ).
    setSizedFromParent( 0 ).
    setDescription( "Number of elements in the radial direction within each mesh block" );

  registerWrapper( viewKeyStruct::tangentElemsString(), &(m_nElems[1]) ).
    setInputFlag( InputFlags::REQUIRED ).
    setSizedFromParent( 0 ).
    setDescription( "Number of elements in the tangent direction within each mesh block" );

  registerWrapper( viewKeyStruct::axialElemsString(), &(m_nElems[2]) ).
    setInputFlag( InputFlags::REQUIRED ).
    setSizedFromParent( 0 ).
    setDescription( "Number of elements in the axial direction within each mesh block" );

  registerWrapper( viewKeyStruct::radialBiasString(), &(m_nElemBias[0]) ).
    setApplyDefaultValue( 1.0 ).
    setSizedFromParent( 0 ).
    setInputFlag( InputFlags::OPTIONAL ).
    setDescription( "Bias of element sizes in the radial direction within each mesh block (dr_left=(1+b)*R/nr, dr_right=(1-b)*R/nr)" );

  // m_nElemBias[1] is a place holder for tangent bias for potential use case

  registerWrapper( viewKeyStruct::axialBiasString(), &(m_nElemBias[2]) ).
    setApplyDefaultValue( 1.0 ).
    setSizedFromParent( 0 ).
    setInputFlag( InputFlags::OPTIONAL ).
    setDescription( "Bias of element sizes in the axial direction within each mesh block (da_left=(1+b)*L/na, da_right=(1-b)*L/na)" );
...
void InternalWellboreGenerator::postProcessInput()
{
  ...
  {
    GEOSX_ERROR( "InternalWellboreGenerator: incorrect element type!" );
  }

  {
    ...
    if( failFlag )
    {
      GEOSX_ERROR( "vertex/element mismatch InternalWellboreGenerator::ReadXMLPost()" );
    }
    ...
    if( failFlag )
    {
      GEOSX_ERROR( "element/bias mismatch InternalWellboreGenerator::ReadXMLPost()" );
    }
  }
  ...
  if( LvArray::integerConversion< long >( m_elementType.size()) != m_numElePerBox.size())
  {
    ...
    {
      GEOSX_ERROR( "InternalWellboreGenerator: The number of element types is inconsistent with the number of total block." );
    }
  }
...

  {
    ...
    if( numBlocks != m_regionNames.size() )
    {
      ...
      {
        GEOSX_ERROR( "Incorrect number of regionLayout entries specified in InternalWellboreGenerator::ReadXML()" );
      }
    }
  }
 ...
 * @param node_size
 */
void InternalWellboreGenerator::getElemToNodesRelationInBox( const string & elementType,
                                                         const int index[],
                                                         const int & iEle,
                                                         int nodeIDInBox[],
                                                         const int node_size )
