...
#ifndef GEOSX_TENSOR_DET
#define GEOSX_TENSOR_DET
...
auto Determinant(const Tensor &J)
{
   ...
   {
      GEOSX_ABORT_KERNEL("Not yet supported.");
      // T det{};
      // Foreach<0>(J,[&](int row){
...
auto Determinant(const Tensor &J)
{
   GEOSX_ABORT_KERNEL("Not yet supported.");
   using T = get_tensor_value_type<Tensor>;
   // GEOSX_SHARED T shared_mem[get_tensor_batchsize];
   // auto batch_id = 0; // <-- GEOSX_THREAD_ID(z); ?
