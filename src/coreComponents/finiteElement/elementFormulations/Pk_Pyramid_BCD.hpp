...
public:
  ...
  GEOS_HOST_DEVICE
  GEOS_FORCE_INLINE
  static void calcModal(int i, int j, int k, real64 const (&X)[3], real64 (& PsiX))
  {
    real64 x = X[0];
    real64 y = X[1];
    real64 z = X[2];
    ...
    if (z > 1.0 || z < 0.0) {
      ...
      real64 P_k = boost::math::jacobi(k, 2.0 * max_ij+2.0, 0.0, chi);
      
      PsiX = P_i * P_j * std::pow(1.0 - z, max_ij) * P_k;
  ...
  GEOS_HOST_DEVICE
  GEOS_FORCE_INLINE
  static void calcN(real64 const (&X)[3], real64 (&N)[numNodes])
  {
