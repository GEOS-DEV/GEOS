...
#include "physicsPackages/fluidFlow/ThermalSinglePhaseFVMKernels.hpp"
#include "physicsPackages/fluidFlow/StabilizedSinglePhaseFVMKernels.hpp"

#include "physicsPackages/fluidFlow/SinglePhaseProppantFluxKernels.hpp"
#include "physicsPackages/multiphysics/poromechanicsKernels/SinglePhasePoromechanicsEmbeddedFractures.hpp"
#include "physicsPackages/multiphysics/poromechanicsKernels/ThermalSinglePhasePoromechanicsEmbeddedFractures.hpp"
#include "physicsPackages/multiphysics/poromechanicsKernels/SinglePhasePoromechanicsConformingFractures.hpp"
#include "physicsPackages/multiphysics/poromechanicsKernels/ThermalSinglePhasePoromechanicsConformingFractures.hpp"
                                                      ...
                                                      arrayView1d< real64 const > const & localRhs )
{
  ...
  PhysicsPackageBaseKernels::NormType const normType = BASE::getNonlinearSolverParameters().normType();
                                                                      ...
                                                                      arrayView1d< string const > const & regionNames )
  {
                                                     ...
                                                     ElementSubRegionBase const & subRegion )
    {
      ...
      SingleFluidBase const & fluid = PhysicsPackageBase::getConstitutiveModel< SingleFluidBase >( subRegion, fluidName );
      ...
      if( m_isThermal )
      {
        ...
        CoupledSolidBase const & solid = PhysicsPackageBase::getConstitutiveModel< CoupledSolidBase >( subRegion, solidName );
        ...
        SolidInternalEnergy const & solidInternalEnergy = PhysicsPackageBase::getConstitutiveModel< SolidInternalEnergy >( subRegion, solidInternalEnergyName );
  ...
  if( m_isThermal )
  {
    ...
    if( getLogLevel() >= 1 && logger::internal::rank == 0 )
    {
      std::cout << GEOS_FMT( "        ( R{} ) = ( {:4.2e} )        ( Renergy ) = ( {:4.2e} )",
                             FlowPackageBase::coupledSolverAttributePrefix(), globalResidualNorm[0], globalResidualNorm[1] );
    }
  }
  else
  {
    ...
    if( getLogLevel() >= 1 && logger::internal::rank == 0 )
    {
      std::cout << GEOS_FMT( "        ( R{} ) = ( {:4.2e} )", FlowPackageBase::coupledSolverAttributePrefix(), residualNorm );
    }
  }
  return residualNorm;
}
...
REGISTER_CATALOG_ENTRY( PhysicsPackageBase, SinglePhaseFVMProppant, string const &, Group * const )
typedef SinglePhaseFVM<> SinglePhaseFVM;
REGISTER_CATALOG_ENTRY( PhysicsPackageBase, SinglePhaseFVM, string const &, Group * const )
}
} /* namespace geos */
