...
public:
   ...
   */
  virtual const std::type_info &get_typeid() const { return typeid(*this); }
   ...
   */
  template <typename T = Group> T *RegisterGroup(std::string const &name)
  {
   ...
   */
  template <typename T> static T group_cast(Group *group)
  {
   ...
   * @return      A pointer to @p T that refers to the @p group
   */
  template <typename T> static T group_cast(Group const *group)
  {
    return dynamicCast<T>(group);
  }
   ...
   */
  template <typename T> T group_cast() { return dynamicCast<T>(this); }
   ...
   * @return      A pointer to \p T that refers to this object
   */
  template <typename T> T group_cast() const { return dynamicCast<T>(this); }
   ...
   */
  template <typename T = Group> T *GetGroup(localIndex index)
  {
   ...
   */
  template <typename T = Group> T const *GetGroup(localIndex index) const
  {
   ...
   * @return A pointer to @p T that refers to the sub-group
   */
  template <typename T = Group> T *GetGroup(string const &name)
  {
   ...
   */
  template <typename T = Group> T const *GetGroup(string const &name) const
  {
   ...
   */
  template <typename T = Group> T &getGroupReference(string const &key)
  {
   ...
   * @return A pointer to @p T that refers to the sub-group
   */
  template <typename T = Group> T *GetGroup(subGroupMap::KeyIndex const &key)
  {
   ...
   * @return A pointer to @p T that refers to the sub-group
   */
  template <typename T = Group> T *GetGroupByPath(string const &path)
  {
    return const_cast<T *>(
   ...
   */
  subGroupMap &GetSubGroups() { return m_subGroups; }

  /**
   * @brief Get the subgroups object
   * @return a reference to const that points to the sub-group map.
   */
  subGroupMap const &GetSubGroups() const { return m_subGroups; }
   ...
   * @param[in] lambda  the functor to call
   */
  template <typename LAMBDA> void forWrappers(LAMBDA lambda)
  {
   ...
   */
  template <typename LAMBDA> void forWrappers(LAMBDA lambda) const
  {
   ...
   */
  wrapperMap const &wrappers() const { return m_wrappers; }

  /**
   * @copydoc wrappers() const
   */
  wrapperMap &wrappers() { return m_wrappers; }
   ...
   */
  template <typename T> Wrapper<T> *getWrapper(char const *const key)
  {
    return getWrapper<T>(string(key));
  }
   ...
   */
  template <typename T, typename LOOKUP_TYPE>
  traits::ViewTypeConst<T> getReference(LOOKUP_TYPE const &lookup) const
  {
   ...
   */
  template <typename T>
  traits::ViewTypeConst<T> getReference(char const *const name) const
  {
   ...
   */
  template <typename T> T &getReference(char const *const name)
  {
    return getReference<T>(string(name));
  }
   ...
   */
  integer sizedFromParent() const { return m_sizedFromParent; }
...
template <typename T>
T *Group::RegisterGroup(std::string const &name, std::unique_ptr<T> newObject)
{
...
template <typename T>
T *Group::RegisterGroup(std::string const &name, T *newObject)
{
...
template <typename T>
Wrapper<T> *Group::registerWrapper(std::string const &name, T *newObject)
{
...
template <typename T> T const *Group::GetGroupByPath(string const &path) const
{
