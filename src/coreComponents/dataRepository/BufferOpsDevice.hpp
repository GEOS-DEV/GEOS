...
typename std::enable_if<can_memcpy<T>, localIndex>::type PackByIndexDevice(
  buffer_unit_type*& buffer,
  ArrayView<T const, NDIM, USD> const& var,
...
localIndex PackByIndexDevice(buffer_unit_type*& GEOSX_UNUSED_PARAM(buffer),
                             T const& GEOSX_UNUSED_PARAM(var),
...
typename std::enable_if<can_memcpy<T>, localIndex>::type UnpackDevice(
  buffer_unit_type const*& buffer,
  ArrayView<T, NDIM, USD> const& var);

//------------------------------------------------------------------------------
template <typename T>
localIndex UnpackDevice(buffer_unit_type const*& GEOSX_UNUSED_PARAM(buffer),
                        T& GEOSX_UNUSED_PARAM(var))
{
  GEOSX_ERROR("Trying to unpack data type ("
              << LvArray::system::demangleType<T>()
              << ") on device but type is not packable.");
  return 0;
}
...
typename std::enable_if<can_memcpy<T>, localIndex>::type UnpackByIndexDevice(
  buffer_unit_type const*& buffer,
  ArrayView<T, NDIM, USD> const& var,
  T_INDICES const& indices);
...
localIndex UnpackByIndexDevice(buffer_unit_type const*& GEOSX_UNUSED_PARAM(buffer),
                               T& GEOSX_UNUSED_PARAM(var),
                               T_INDICES const& GEOSX_UNUSED_PARAM(indices))
{
  GEOSX_ERROR("Trying to unpack data type ("
