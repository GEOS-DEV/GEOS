...
namespace testing
{
template <typename T> void checkNoSizeMethod(T const& var)
{
  EXPECT_EQ(wrapperHelpers::size(var), 1);
}

template <typename T> void checkSizeMethod(T const& var)
{
...
template <typename ARRAY> class PopulateMCArray : public ::testing::Test
{
public:
...
TYPED_TEST(PopulateMCArray, test) { this->test(); }

template <typename ARRAY> class AverageOverSecondDim : public ::testing::Test
{
public:
  static_assert(traits::is_array<ARRAY>, "T must be an LvArray::Array!");
  using T = typename ARRAY::value_type;
...
TYPED_TEST(AverageOverSecondDim, test) { this->test(); }
