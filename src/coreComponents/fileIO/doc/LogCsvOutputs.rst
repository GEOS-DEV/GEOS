...
- **Umpire Memory Pool (reserved / % over total):**
  ...
  The name of the memory allocator. Allocators are components that manage memory, allocating and releasing blocks according to the program’s needs.
...
- **Sum over ranks:**
  ...
  The total sum of watermarks (KB, MB, or GB) for this allocator across all ranks. This shows how much memory has been allocated in total by this allocator across the whole application. It also displays the total percentage of memory occupied.
...
- **Immobile phase mass (metric 2) [kg or mol]:**
  ...
  For each phase, mass or number of moles that do not move in the simulation (for example due to trapping, viscosity, or pressure thresholds). See footnote [3]_.
