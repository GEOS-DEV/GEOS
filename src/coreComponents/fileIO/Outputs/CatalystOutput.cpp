...
bool CatalystOutput::execute(real64 const time_n, real64 const /*dt*/,
  ...
  if (isInTransit) {
    ...
    bool isMultiMesh =
        (channel["type"].as_char8_str() == std::string("multimesh"));
    conduit::index_t nMeshes =
        (isMultiMesh ? channel["data"].number_of_children() : 1);

    for (conduit::index_t iMesh = 0; iMesh < nMeshes; ++iMesh) {
      auto& mesh =
          (isMultiMesh ? channel["data"].child(iMesh) : channel["data"]);
      localIndex_array numberOfNodesPerRank(commSize);

      MpiWrapper::allGather(static_cast<localIndex>(mesh["coordsets"]
                                                        .child(0)["values"]
                                                        .child(0)
                                                        .dtype()
                                                        .number_of_elements()),
      ...
      globalIndex offsetPoints =
          std::accumulate(numberOfNodesPerRank.begin(),
                          numberOfNodesPerRank.begin() + thisRank, 0);

      globalIndex totalNumberOfNodes =
          std::accumulate(numberOfNodesPerRank.begin() + thisRank,
                          numberOfNodesPerRank.end(), offsetPoints);
      ...
      mesh["coordsets/coords/points_count"].set_uint64(
          numberOfNodesPerRank[thisRank]);

      // WARNING
      // what follows on the elements only works if all the MPI partitions have
      // the same number of corresponding subRegions. I do not know if this is
      // the case. If it is not, we expose ourselves to hanging MPI
      // communication here.
      auto& topologies = mesh["topologies"];
      if (topologies.number_of_children() != 1) {
        GEOS_ERROR(
            "CatalystOutput: execute: can only accept one topology per mesh");
      }
      meshLevel.getElemManager()
          .forElementSubRegionsComplete<CellElementSubRegion>(
              [&](localIndex, localIndex, ElementRegionBase const& region,
                  CellElementSubRegion const& subRegion) {
                std::string const topologyName =
                    region.getName() + "-" + subRegion.getName();
                if (topologyName == topologies.child(0).name()) {
                  auto& topology = topologies[topologyName];

                  localIndex_array numberOfElementsPerRank(commSize);

                  MpiWrapper::allGather(subRegion.size(),
                                        numberOfElementsPerRank);

                  globalIndex offsetElements = std::accumulate(
                      numberOfElementsPerRank.begin(),
                      numberOfElementsPerRank.begin() + thisRank, 0);

                  globalIndex totalNumberOfElements = std::accumulate(
                      numberOfElementsPerRank.begin() + thisRank,
                      numberOfElementsPerRank.end(), offsetElements);

                  topology["cells_shape"].set_uint64(totalNumberOfElements);
                  topology["cells_start"].set_uint64(offsetElements);
                  topology["cells_count"].set_uint64(
                      numberOfElementsPerRank[thisRank]);
                }
              });
    }
  }
