...
std::unique_ptr< CommandLineOptions > parseCommandLineOptions( int argc, char * * argv )
{
  ...
  const option::Descriptor usage[] =
  {
    ...
    { ERRORS, 0, "e", "errorsOutput", Arg::nonEmpty, "\t-e, \t Output path for the errors file" },
    { DATAHIERARCHY, 0, "h", "hierarchyOutput", Arg::nonEmpty, "\t-h, \t Output path for the datahierarchy file" },
  ...
  for( int ii=0; ii<parse.optionsCount(); ++ii )
  {
    ...
    switch( opt.index() )
    {
      ...
      case ERRORS:
      {
        commandLineOptions->errorsPath = opt.arg;
      }
      break;
      case DATAHIERARCHY:
      {
        commandLineOptions->hierarchyPath = opt.arg;
      }
      break;
    }
  }
