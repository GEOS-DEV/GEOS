...
void ProblemManager::parseInputFile()
{
  ...
  xmlWrapper::xmlResult const xmlResult = xmlDocument.loadFile( inputFileName.c_str(), true );
...
void ProblemManager::parseInputString( string const & xmlString )
{
  // Load preprocessed xml file
  xmlWrapper::xmlDocument xmlDocument;
  xmlWrapper::xmlResult xmlResult = xmlDocument.loadBuffer( xmlString.c_str(), xmlString.length(), true );
