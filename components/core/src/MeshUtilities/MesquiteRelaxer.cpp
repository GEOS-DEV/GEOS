...
void MesquiteRelaxer::RelaxMesh( PhysicalDomainT& domain )
{
  ...
  // TODO: EBH: Have the user select the solver type for the mesh relaxation. For now, just hard code something that works
  //            most of the time.
  ...
  // Overwrite the reference nodal positions based on the movement from the Mesquite library
  for (localIndex a = 0; a < numNodes; ++a)
  {
    xyzIdx = 3*a;
