...
def form_block_value(suffix):
	#loading pore vols
	pv_tbl = pd.read_csv('./postt/class09_pv'+suffix+'.csv')
	pv = (pv_tbl['vol']*pv_tbl['poro']).to_numpy()

	#read hdf5 timeHistory
	fr = h5py.File(os.getcwd() + '/rhoHistory'+suffix+'.hdf5','r');
	fs = h5py.File(os.getcwd() + '/satHistory'+suffix+'.hdf5','r');
	fx = h5py.File(os.getcwd() + '/xcompHistory'+suffix+'.hdf5','r');
...
ecl = pd.read_csv('./docs/ECLSLB_hyst.csv') 
