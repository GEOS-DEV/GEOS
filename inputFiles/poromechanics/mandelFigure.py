...
def main():
    ...
    for i in range(len(t)):
        ...
        # ax.legend(bbox_to_anchor=(0.02, 0.6), loc='center left', borderaxespad=0., fontsize=fsize * 0.7)
    ...
    pressure_analytical = np.zeros([len(t), len(xd_numerical)])
    for i in range(len(t)):
        for j in range(len(xd_numerical)):
            pressure_analytical[i][j] = mandelAnalyticalSolution.computePressure(xd_numerical[j], t[i]) / p0

    fig, ax = plt.subplots()
    cmap = plt.get_cmap("tab10")

    for i in range(len(t)):
        ax.semilogy(xd_numerical,
                    abs(pressure_analytical[i][:] - pressure_numerical[i][:])/abs(pressure_analytical[i][:]),
                    color = cmap(i),
                    marker = 'o',
                    linestyle = 'none',
                    alpha = 0.6,
                    label = 't = ' + str(t[i]) + 's',
                    lw = lw)
        ax.set_title('Pressure relative errors')
        ax.set_xlim(0, 1)
        # ax.set_ylim(0, 1.2)
        ax.set_xlabel('Normalized Distance, '
                      r'$x$/$a$', size=fsize, weight="bold")
    ...
    for i in range(len(t)):
        ...
        ax.set_title('Displacement profiles')
        # ax.legend(loc='upper left', fontsize=fsize * 0.7)
        ax.legend(loc='best')
        ax.grid(True)
        ax.xaxis.set_tick_params(labelsize=fsize)
        ax.yaxis.set_tick_params(labelsize=fsize)
