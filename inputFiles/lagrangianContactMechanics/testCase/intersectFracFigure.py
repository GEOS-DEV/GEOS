...
def main():
    ...
    xmlFilePath = "ContactMechanics_TFrac_vtkinput_base.xml"
    ...
    for i in range(0, len(zcord)):
        if abs(ycord[i] - 1.) < 0.01:
            xlist.append(xcord[i] - 1.0)
    ...
    apertlist = []
    for i in range(0, len(zcord)):
        if abs(xcord[i] - 1.) < 0.01:
            ylist.append(ycord[i] + 49.0)
